...
void Jiki(ACTION *pAct) {
  ...
  if (pYobi->cnt3 <= 0) {
    ...
    if (mouse.onceState & MOUSE_LEFT_CRICK) {
      ...
      // MakeJikiTama( pAct->x,            pAct->y -16,  0, -8, Rnd( 0, BMP_FILE
      // - 2 ) );

      // MakeJikiTama( pAct->x,            pAct->y -16,  0, -8, Rnd( 31554,
      // 31557 ) ); // �ԣǣ� MakeJikiTama( pAct->x,            pAct->y -16,  0,
      // -8, Rnd( 31000, 31069 ) ); // ��󥹥��`
      //            MakeJikiTama( pAct->x,            pAct->y -16,  0, -8, Rnd(
      //            30100, 30135 ) ); // �У�
      // MakeJikiTama( pAct->x + 10, pAct->y -16, 0, -8, Rnd( 0, BMP_FILE - 1 )
      // ); MakeJikiTama( pAct->x, pAct->y -16, 0, -8, 29 ); MakeJikiTama(
      // pAct->x - 10, pAct->y -16, 0, -8, 29 ); MakeJikiTama( pAct->x + 10,
      // pAct->y -16, 0, -8, 29 );
      pYobi->cnt3 = 5;
#if 0
            for( i = 0; i < (int)256; i++ ){
...
void enemy_missile(ACTION *a0) {
  gemini(a0); // �Ƅ�
  // ������ʤ�
  if (ATR_H_POS(a0) >= lpDraw->xSize || ATR_H_POS(a0) < -32 ||
      ATR_V_POS(a0) > lpDraw->ySize || ATR_V_POS(a0) < -32)
    DeathAction(a0); // �K��
}
...
ACTION *MakeBg(void) {
  ...
  // SetRect( &Rc[ pAct->bmpNo ], pYobi->rcX, pYobi->rcY, pYobi->rcX +
  // lpDraw->xSize, pYobi->rcY + lpDraw->ySize );
