...
static int SaStockFontBuffer(lua_State *L) {
  ...
  lua_pushinteger(
      L, StockFontBuffer(winX, winY, fontPrio, color, (char *)data, hitFlag));
  return 1;
}
...
static int Lua_TeaDecryption(lua_State *L) {
  ...
  size_t l;
  const char *str = luaL_checklstring(L, 1, &l);
  extern long tea_decrypt(long *v, long n, long *k);
  ...
  if (err == 0) {
    ...
    fread(temp, 1, len, fp);
    fclose(fp);
    tea_decrypt((long *)temp, len / 4, (long *)_LUA_KEY_);
  }
...
static int Lua_getStringFromIndexWithDelim_body(lua_State *L) {
  extern BOOL getStringFromIndexWithDelim_body(
      char *src, char *delim, int index, char *buf, int buflen);
  char temp[2048];
  memset(temp, 0, 2024);
  size_t l;
  const char *str = luaL_checklstring(L, 1, &l);
  ...
  getStringFromIndexWithDelim_body((char *)str, (char *)delim, index, temp,
                                   2024);
  lua_pushstring(L, temp);
  return 1;
}
...
static const luaL_Reg Lssprotolib[] = {{"windows", windows_send},
                                       {NULL, NULL}};
