...
#endif
void initConnectServer(void) { connectServerCounter = 0; }
...
int connectServer(void) {
  ...
  if (connectServerCounter == 0) {
    ...
    sin_server.sin_addr.s_addr =
        inet_addr(hostname); /* accept only dot notaion  */
    if (sin_server.sin_addr.s_addr == -1) {
      ...
      memcpy((void *)&sin_server.sin_addr.s_addr, h->h_addr,
             sizeof(struct in_addr));
    }
    // Non blocking Connect
    int ret =
        connect(sockfd, (struct sockaddr *)&sin_server, sizeof(sin_server));
    if (ret == SOCKET_ERROR) {
      if (WSAGetLastError() != WSAEWOULDBLOCK) {
        count = 0;
        closesocket(sockfd);
...
#ifdef __NEW_CLIENT
      hPing =
          CreateThread(NULL, 0, PingFunc, &sin_server.sin_addr, 0, &dwPingID);
#endif
    }
...
void lssproto_S_recv(int fd, char *data) {
  ...
  case 'K': {
    ...
    if (kubun == 0) {
      ...
      if (kubun == 1) {
        ...
        pet[no].changeNameFlag =
            getIntegerToken(data, S_DELIM, 19); // 0x00040000
        ...
        for (; mask > 0; mask <<= 1) {
          if (kubun & mask) {
            if (mask == 0x00000002) {
              ...
              pet[no].maxSkill =
                  getIntegerToken(data, S_DELIM, i); // 0x00020000
              i++;
            } else if (mask == 0x00040000) {
              pet[no].changeNameFlag =
                  getIntegerToken(data, S_DELIM, i); // 0x00040000
              i++;
            } else if (mask == 0x00080000) {
              getStringToken(data, S_DELIM, i, sizeof(name) - 1,
                             name); // 0x00080000
  ...
  case 'S': {
    ...
    for (i = 0; i < MAX_PROFESSION_SKILL; i++) {
      ...
      profession_skill[i].skill_level =
          getIntegerToken(data, S_DELIM, 7 + count);
...
#ifdef DEBUGPUSH
  char msg[800];
  sprintf_s(msg, "佋???????????????FL%d %d,%d-%d,%d (%ud/%ud)", fl, x1, y1, x2,
            y2, tileSum, partsSum);
  PUSH(msg);
#endif
...
void lssproto_M_recv(int fd, int fl, int x1, int y1, int x2, int y2,
                     char *data) {
  char showString[512], floorName[32], tilestring[18192], partsstring[18192],
      eventstring[18192], tmp[100];
...
void lssproto_C_recv(int fd, char *data) {
  int i, j, id, x, y, dir, graNo, level, nameColor, walkable, height, classNo,
      money, charType, charNameColor;
  char bigtoken[2048], smalltoken[2048], name[2048], freeName[2048], info[1024],
      fmname[2048], petname[1024];
  ...
  *titlestr = 0;
  ...
  for (i = 0;; i++) {
    ...
    case 1: // OBJTYPE_CHARA
      ...
      //            getStringToken(bigtoken, '|', 20, sizeof(smalltoken) - 1,
      //            smalltoken); profession_exp = atoi(smalltoken);
...
#ifdef _CHAR_PROFESSION // WON ADD 人物职业
//    #ifdef _GM_IDENTIFY        // Rog ADD GM识别
//                setPcParam(name, freeName, level, petname, petlevel,
//                nameColor, walkable, height, profession_class,
//                profession_level, profession_exp, profession_skill_point ,
//                gm_name); setPcParam(name, freeName, level, petname, petlevel,
//                nameColor, walkable, height, profession_class,
//                profession_level, profession_skill_point , gm_name);
//    #else
//                setPcParam(name, freeName, level, petname, petlevel,
//                nameColor, walkable, height, profession_class,
//                profession_level, profession_exp, profession_skill_point);
#ifdef _ALLDOMAN // Syu ADD 排行榜NPC
        setPcParam(name, freeName, level, petname, petlevel, nameColor,
                   walkable, height, profession_class, profession_level,
                   profession_skill_point, herofloor);
#else
        setPcParam(name, freeName, level, petname, petlevel, nameColor,
                   walkable, height, profession_class, profession_level,
...
#ifdef _CHAR_PROFESSION // WON ADD 人物职业
#ifdef _GM_IDENTIFY     // Rog ADD GM识别
        setNpcCharObj(id, graNo, x, y, dir, fmname, name, freeName, level,
                      petname, petlevel, nameColor, walkable, height, charType,
                      profession_class, gm_name);
#else
#ifdef _NPC_PICTURE
        setNpcCharObj(id, graNo, x, y, dir, fmname, name, freeName, level,
                      petname, petlevel, nameColor, walkable, height, charType,
                      profession_class, picture);
#else
        setNpcCharObj(id, graNo, x, y, dir, fmname, name, freeName, level,
                      petname, petlevel, nameColor, walkable, height, charType,
                      profession_class);
...
#ifdef _CHAR_PROFESSION // WON ADD 人物职业
#ifdef _GM_IDENTIFY     // Rog ADD GM识别
      setNpcCharObj(id, graNo, x, y, dir, "", name, "", level, petname,
                    petlevel, nameColor, 0, height, 2, 0, "");
#else
#ifdef _NPC_PICTURE
      setNpcCharObj(id, graNo, x, y, dir, "", name, "", level, petname,
                    petlevel, nameColor, 0, height, 2, 0, 0);
#else
      setNpcCharObj(id, graNo, x, y, dir, "", name, "", level, petname,
                    petlevel, nameColor, 0, height, 2, 0);
#endif
...
#ifdef _STREET_VENDOR
    if (act == 41)
      strncpy_s(szStreetVendorTitle, sizeof(szStreetVendorTitle), smalltoken,
                sizeof(szStreetVendorTitle));
    else
#endif
    {
      ...
      if (pc.ptAct == NULL ||
        ...
        if (act == 41) {
          if (pc.iOnStreetVendor == 1) {
            memset(pc.ptAct->szStreetVendorTitle, 0,
                   sizeof(pc.ptAct->szStreetVendorTitle));
            sprintf_s(pc.ptAct->szStreetVendorTitle,
                      sizeof(pc.ptAct->szStreetVendorTitle), "%s",
                      szStreetVendorTitle);
...
#ifdef _STREET_VENDOR
      if (act == 41) {
        memset(ptAct->szStreetVendorTitle, 0,
               sizeof(ptAct->szStreetVendorTitle));
        strncpy_s(ptAct->szStreetVendorTitle, szStreetVendorTitle,
                  sizeof(szStreetVendorTitle));
      }
#endif
      changeCharAct(ptAct, x, y, dir, act, effectno, effectparam1,
                    effectparam2);
    }
  }
}
...
void lssproto_TK_recv(int fd, int index, char *message, int color) {
  ...
  if (id[0] == 'P') {
...
#endif
      if (strlen(szToken) > 1) {
        if (strcmp(szToken, "TK") == 0)
          InitSelectChar(message, 0);
        else if (strcmp(szToken, "TE") == 0)
    ...
    if (getStringToken(msg, '|', 1, sizeof(TK) - 1, TK) == 0) {
      if (strcmp(TK, "TK") == 0)
        InitSelectChar(msg, 0);
      else if (strcmp(TK, "TE") == 0)
        InitSelectChar(msg, 1);
    } else {
      char temp[] = "告诉你：";
      char *found;
...
void createNewCharStart(void) {
  ...
  if (bNewServer) {
    ...
    lssproto_CreateNewChar_send(
        sockfd, selectPcNo + 多人物当前页数 * 2, newCharacterName,
...
void lssproto_EV_recv(int fd, int seqno, int result) {
  ...
  } else if (eventEnemySendId == seqno) {
    if (result == 0) {
...
void lssproto_I_recv(int fd, char *data) {
  ...
  for (j = 0;; j++) {
    ...
    if (getStringToken(data, '|', no + 2, sizeof(name) - 1, name) ==
        1) // 道具名
      break;
    makeStringFromEscaped(name);
    if (strlen(name) == 0) {
...
void lssproto_PME_recv(int fd, int objindex, int graphicsno, int x, int y,
  ...
  else
  {
    ...
    // ?????????????
    if (setReturnPetObj(id, graNo, x, y, dir, name, freeName, level, nameColor,
                        walkable, height, charType)) {
      switch (no) {
      case 0:
...
void lssproto_B_recv(int fd, char *command) {
  ...
  if (*(command + 1) == 'C') {
    ...
    BattleStatusWritePointer =
        (BattleStatusWritePointer + 1) & (BATTLE_BUF_SIZE - 1);
  }
...
void lssproto_PETST_recv(int fd, int petarray, int result) {
  if (petarray < 0 || petarray >= 5)
    return;
  pc.selectPetNo[petarray] = result;
  BattlePetStMenCnt--;
  if (BattlePetStMenCnt < 0)
    BattlePetStMenCnt = 0;
...
void lssproto_SKUP_recv(int fd, int point) { StatusUpPoint = point; }
...
void lssproto_MSG_recv(int fd, int aindex, char *text, int color) {
  ...
  getStringToken(
      text, '|', 1,
      sizeof(MailHistory[aindex].dateStr[MailHistory[aindex].newHistoryNo]) - 1,
      MailHistory[aindex].dateStr[MailHistory[aindex].newHistoryNo]);
  // ???
  getStringToken(
      text, '|', 2,
      sizeof(MailHistory[aindex].str[MailHistory[aindex].newHistoryNo]) - 1,
  ...
  if (noReadFlag != -1) {
    // ?????????????
    MailHistory[aindex].noReadFlag[MailHistory[aindex].newHistoryNo] =
        noReadFlag;
    // ??????????
    MailHistory[aindex].petLevel[MailHistory[aindex].newHistoryNo] =
        getIntegerToken(text, '|', 4);
    // ???????
    getStringToken(
        text, '|', 5,
        sizeof(MailHistory[aindex].petName[MailHistory[aindex].newHistoryNo]),
        MailHistory[aindex].petName[MailHistory[aindex].newHistoryNo]);
    // ?????????????
    makeStringFromEscaped(
        MailHistory[aindex].petName[MailHistory[aindex].newHistoryNo]);
    // ???????????????
    MailHistory[aindex].itemGraNo[MailHistory[aindex].newHistoryNo] =
        getIntegerToken(text, '|', 6);
    // ????????????
...
void lssproto_FM_recv(int fd, char *data) {
  ...
  if (strcmp(FMType1, "S") == 0) {
    if (strcmp(FMType2, "F") == 0) // 家族列表
      initFamilyList(data);
    else if (strcmp(FMType2, "D") == 0) // 家族详细
    ...
    if (strcmp(FMType2, "G") == 0)
      BankmanInit(data);
    else if (strcmp(FMType2, "I") == 0) {
      // getStringToken(data, '|', 3, sizeof( FMType3 ) - 1, FMType3 );
      // ItemmanInit(data );
      // initItemman(data );
    } else if (strcmp(FMType2, "T") == 0)
...
DWORD WINAPI PingFunc(LPVOID param) {
  ...
  if (sockRaw != INVALID_SOCKET) {
    iRead = setsockopt(sockRaw, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,
                       sizeof(timeout));
    if (iRead != SOCKET_ERROR) {
      timeout = 1000;
      iRead = setsockopt(sockRaw, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout,
                         sizeof(timeout));
      if (iRead != SOCKET_ERROR) {
        ...
        icmp_hdr->i_cksum = 0;
        icmp_hdr->i_seq = 0;
        memset(icmp_data + sizeof(IcmpHeader), 'E',
        ...
        while (1) {
          ...
          iWrote = sendto(sockRaw, icmp_data, datasize, 0,
          ...
          while (1) {
            iRead = recvfrom(sockRaw, recvbuf, MAX_PACKET, 0,
...
void lssproto_IC_recv(int fd, int x, int y) {
  m_bt = true;
  setCharMind(pc.ptAct, SPR_cracker);
  m_bt = false;
}
#endif
...
void lssproto_CS_recv(int fd, int deltimes) { delaytimes = deltimes; }
int lssproto_getdelaytimes() {
  if (delaytimes < 0)
    delaytimes = 0;
  return delaytimes;
}
void lssproto_setdelaytimes(int delays) { delaytimes = delays; }
#endif
  ...
  if ((pc.etcFlag & PC_ETCFLAG_CHAT_SAVE) && !bCloseFile) {
    ...
    for (int i = 0; i < 6; i++) {
      if (pc.etcFlag & Channel[i]) {
        if (pSaveChatDataFile[i] == NULL) {
          sprintf_s(szFileName, "chat\\%c%02d%02d%02d.TXT", ChannelType[i],
                    (nowTime.tm_year % 100), nowTime.tm_mon + 1,
                    nowTime.tm_mday);
          if ((pSaveChatDataFile[i] = fopen(szFileName, "a")) == NULL)
            continue;
        }
      } else {
        if (pSaveChatDataFile[i] != NULL) {
    ...
    for (int i = 0; i < 6; i++) {
      if (KindOfChannel == ChannelType[i]) {
        if (pSaveChatDataFile[i] != NULL)
          fprintf(pSaveChatDataFile[i], "[%02d:%02d:%02d]%s\n", nowTime.tm_hour,
                  nowTime.tm_min, nowTime.tm_sec, msg);
      }
    }
  } else
    bCloseFile = 1;
...
void lssproto_JOBDAILY_recv(int fd, char *data) {
  ...
  while (getStringToken(data, '#', i, sizeof(getdata) - 1, getdata) != 1) {
    while (getStringToken(getdata, '|', j, sizeof(perdata) - 1, perdata) != 1) {
      switch (j) {
      case 1:
        jobdaily[i - 1].JobId = atoi(perdata);
        break;
      case 2:
        strcpy(jobdaily[i - 1].explain, perdata);
        break;
      case 3:
        strcpy(jobdaily[i - 1].state, perdata);
        break;
      default:
        StockChatBufferLine("每笔资料内参数有错误", FONT_PAL_RED);
        break;
      }
      perdata[0] = '\0';
      j++;
...
void lssproto_S2_recv(int fd, char *data) {
  ...
  if (getStringToken(data, '|', 3, sizeof(szMessage) - 1, szMessage) == 1)
    return;
...
void lssproto_Firework_recv(int fd, int nCharaindex, int nType,
                            int nActionNum) {
  ACTION *pAct;
...
void lssproto_TheaterData_recv(int fd, char *pData) {
  ...
  case E_DATA_TYPE_MOVE: // 移动
    camMapToGamen((float)(HIWORD(iData) * GRID_SIZE),
                  float(LOWORD(iData) * GRID_SIZE), &fX, &fY);
  ...
  case E_DATA_TYPE_NPC: // 产生或是消失或更改临时NPC
    ...
    if (iData >= 0 && iData < 5) {
      ...
      getStringToken(pData, '|', 3, sizeof(szMessage) - 1,
                     szMessage); // 取出指令
      if (atoi(szMessage) == 1) {
        getStringToken(pData, '|', 4, sizeof(szMessage) - 1,
                       szMessage); // 取出图号
        iSprNum = atoi(szMessage);
        getStringToken(pData, '|', 5, sizeof(szMessage) - 1,
                       szMessage); // 取出座标
        iGX = atoi(szMessage);
        getStringToken(pData, '|', 6, sizeof(szMessage) - 1,
                       szMessage); // 取出座标
        iGY = atoi(szMessage);
        getStringToken(pData, '|', 7, sizeof(szMessage) - 1,
                       szMessage); // 取出动作
        iAction = atoi(szMessage);
        getStringToken(pData, '|', 8, sizeof(szMessage) - 1,
                       szMessage); // 取出方向
        iDir = atoi(szMessage);
        camMapToGamen((float)iGX * GRID_SIZE, (float)iGY * GRID_SIZE, &fX, &fY);
        if (pc.pActNPC[iData] == NULL) {
          pc.pActNPC[iData] =
              MakeAnimDisp((int)fX, (int)fY, iSprNum, ANIM_DISP_THEATER_NPC);
...
void lssproto_MagiccardDamage_recv(int fd, int position, int damage,
...
void lssproto_PetSkins_recv(char *data) {
  char *str =
      "宠物栏位置|当前使用皮肤图号|总皮肤数|皮肤图号|说明|皮肤图号|说明|...";
}
