...
void picture_play(ACTION *a0) {
  ...
  if (a0->picture != 0) {
    ...
    if (picture1 == 1) { // 不断变身
      ...
      if (g_bUseAlpha)
        a0->anim_chr_no = 101805;
      else
        a0->anim_chr_no = 101858;
      a0->picture =
          (((1) << 24) & 0xff000000) | (((picture2) << 16) & 0x00ff0000) |
          (((picture3) << 8) & 0x0000ff00) | ((picture4) & 0x000000ff);
    }
  }
}
#endif
...
int pattern(ACTION *a0, int anim_spd, int loop_flg) {
  ...
  if (getCharType(a0)) {
    ...
    if (!ATR_PAT_NO(a0) && ATR_PAT_BAK_NO(a0))
      ATR_PAT_NO(a0) = ATR_PAT_BAK_NO(a0);
  }
  ...
  if (ATR_CHR_NO(a0) < SPRSTART) { // 不是sprite
    ...
    if (ATR_CHR_NO(a0) >= CG_ATR_ICON_EARTH_BATTLE &&
        ATR_CHR_NO(a0) <= CG_ATR_ICON_WIND_BATTLE) {
  ...
  for (i = 0; i < SpriteData[chrNo].animSize; i++) {
    if (ATR_CHR_ANG(a0) == ptAnimlist[i].dir &&
        ptAnimlist[i].no == ATR_CHR_ACT(a0))
      break;
  }
  if (i >= SpriteData[chrNo].animSize) {
    ...
    for (int sh_i = 0; sh_i < SpriteData[chrNo].animSize; sh_i++) {
      if (ATR_CHR_ANG(a0) == ptAnimlist[sh_i].dir &&
          ptAnimlist[sh_i].no == ANIM_STAND) {
        i = sh_i;
        break;
      } else {
        i = 0;
  ...
  if (a0->picture != 0) {
    ...
    /*if( (((a0->picture&0xff000000)>>24) & 0x000000ff) == 1
        ...
        && ( a0->anim_chr_no !=
       pictable[(((a0->picture&0x00ff0000)>>16)&0x000000ff)] ) ) )
    ...
    */
    if (a0->anim_chr_no !=
        ...
        (((a0->picture & 0xff000000) >> 24) & 0x000000ff) == 3)
      if (a0->anim_chr_no != 101805 && a0->anim_chr_no != 101858 &&
          a0->anim_chr_no != 101936)
        ATR_CHR_TIM(a0) = 1; // 变身npc速度调最快
  }
#endif
  ...
  if (ptFramelist[ATR_CHR_CNT(a0)].SoundNo != 0) {
    if (ptFramelist[ATR_CHR_CNT(a0)].SoundNo < 10000)
      play_se(ptFramelist[ATR_CHR_CNT(a0)].SoundNo, ATR_H_POS(a0),
              ATR_V_POS(a0));
    else
  ...
  realGetPos(ptFramelist[ATR_CHR_CNT(a0)].BmpNo, &dx,
             &dy); //????????????
