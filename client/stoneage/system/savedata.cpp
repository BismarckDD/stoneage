...
enum {
  ...
  STEREO_FLAG = BA_SEL_PEN + BA_SEL_PEN_SIZE * MAX_PET * MAX_CHARACTER,
...
BOOL saveNowState(void) {
  ...
  if (fwrite(writebuffer, 1, writebufferlen, fp) <
...
BOOL createSaveFile(void) {
  ...
  *((unsigned short *)savedatabuf + SAVEDATA_VER) = SAVEDATA_VERSION;
  ...
  // ??????????
  jEncode((char *)savedatabuf, SAVEDATA_SIZE, 0, writebuffer, &writebufferlen,
          sizeof(writebuffer));
  if ((fp = fopen(SAVEFILE_NAME, "wb+")) == NULL) {
    return FALSE;
  }
