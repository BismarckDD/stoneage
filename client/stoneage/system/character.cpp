...
enum { CHAROBJ_USE_FREE, CHAROBJ_USE_STAY, CHAROBJ_USE_VIEW };
// Robin
...
int charIconOffsetY[12] = {-71, -88, -84, -88, -96, -102,
                           -88, -88, -88, -85, -85, -88};

int battleIconTbl[][2] = {
    {CG_VS_MARK_1A, CG_VS_MARK_1B}, {CG_VS_MARK_2A, CG_VS_MARK_2B},
    {CG_VS_MARK_3A, CG_VS_MARK_3B}, {CG_VS_MARK_4A, CG_VS_MARK_4B},
    {CG_VS_MARK_5A, CG_VS_MARK_5B}, {CG_VS_MARK_6A, CG_VS_MARK_6B},
    {CG_VS_MARK_7A, CG_VS_MARK_7B}, {CG_VS_MARK_8A, CG_VS_MARK_8B},
    {CG_VS_MARK_9A, CG_VS_MARK_9B}, {CG_VS_MARK_10A, CG_VS_MARK_10B}};
...
int offsetYtbl[60] = {
    20,  38,  54,  68,  80,  104, 104, 104, 104, 104, 104, 104, 104, 104, 104,
...
void charProc(ACTION *ptAct) {
  ...
  if (pc.ptAct != ptAct) {
    ...
    if (ptAct->vx == 0 && ptAct->vy == 0 && charObj[no].stockDir != -1 &&
        charObj[no].stockDirX == ptAct->gx &&
        charObj[no].stockDirY == ptAct->gy) {
  ...
  if (pc.bMoveScreenMode)
    camMapToGamen(ptAct->mx - iScreenMoveX * GRID_SIZE,
                  ptAct->my - iScreenMoveY * GRID_SIZE, &mx, &my);
  else
#endif
  ...
  if (ptAct->anim_no == ANIM_HAND || ptAct->anim_no == ANIM_HAPPY ||
      ptAct->anim_no == ANIM_ANGRY || ptAct->anim_no == ANIM_SAD ||
      ptAct->anim_no == ANIM_WALK || ptAct->anim_no == ANIM_STAND ||
  ...
  if (pc.ptAct != ptAct) {
    if (charObj[no].newFoundFlag) {
      ...
      xx += ptAct->x;
      yy += ptAct->y;
      if (0 <= xx && xx + ww <= DEF_APPSIZEX && 0 <= yy &&
          yy + hh <= DEF_APPSIZEY) {
...
void waterAniProc(ACTION *ptAct) {
  ...
  if (20000 <= ptAct->anim_chr_no && ptAct->anim_chr_no <= 24999) {
    ...
    if (!itemOverlapCheck(ptAct->bmpNo, ptAct->gx, ptAct->gy)) {
      ...
      setPartsPrio(ptAct->bmpNo, ptAct->x, ptAct->y, 0, 0, ptAct->mx, ptAct->my,
                   ptAct->dispPrio);
#else
      setCharPrio(ptAct->bmpNo, ptAct->x, ptAct->y, 0, 0, ptAct->mx, ptAct->my
      );
...
void _charMove(ACTION *ptAct) {
  ...
  if (ptAct->vx != 0 || ptAct->vy != 0) {
    ...
    // ?佋?????????????
    if (pointLen2(ptAct->mx, ptAct->my, mx, my) <= vx * vx + vy * vy) {
      ptAct->mx = mx;
      ptAct->my = my;
      ptAct->vx = 0;
      ptAct->vy = 0;
...
void setCharMind(ACTION *ptAct, int MindNo) {
  ...
  ext->ptMindIcon = createCommmonEffectAction(MindNo, ptAct->gx, ptAct->gy, 0,
                                              0, DISP_PRIO_CHAR);
}
...
void setCharFamily(ACTION *ptAct, int MindNo) {
  ...
  ext->ptFamilyIcon = createCommmonEffectAction(MindNo, ptAct->gx, ptAct->gy, 0,
                                                0, DISP_PRIO_CHAR);
}
...
void setCharmFamily(ACTION *ptAct, int MindNo) {
  ...
  ext->ptmFamilyIcon = createCommmonEffectAction(MindNo, ptAct->gx, ptAct->gy,
                                                 0, 0, DISP_PRIO_CHAR);
}
...
void setCharmManor(ACTION *ptAct, int MindNo) {
  ...
  ext->ptmManorIcon = createCommmonEffectAction(MindNo, ptAct->gx, ptAct->gy, 0,
                                                0, DISP_PRIO_CHAR - 1);
}
...
void setNpcNotice(ACTION *ptAct, int MindNo) {
  ...
  ext->ptNoticeIcon = createCommmonEffectAction(MindNo, ptAct->gx, ptAct->gy, 0,
                                                0, DISP_PRIO_CHAR);
}
...
void drawCharStatus(ACTION *ptAct) {
  ...
  if (ext->ptmManorIcon) {
    {
      if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
        ...
        if (getCartoonSize(ptAct, &x1, &y1)) {
          ...
          StockDispBuffer2(ext->ptmManorIcon->x, ext->ptmManorIcon->y,
                           ext->ptmManorIcon->dispPrio,
                           ext->ptmManorIcon->bmpNo, 0);
        }
      }
    }
  }
  ...
  if ((status & CHR_STATUS_BATTLE) != 0) {
    if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
      ...
      if (getCartoonSize(ptAct, &x1, &y1)) {
        ...
        // ??????????????
        setCharPrio(bmpNo, ptAct->x, ptAct->y, 0,
                    ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_)
                                       : -(y1 / 2 + _FANILYTEXIAOY_),
                    ptAct->mx, ptAct->my
        );
      }
    }
  ...
  if ((status & CHR_STATUS_ANGEL) != 0 && ext->ptActAngelMark == NULL) {
    if (g_bUseAlpha)
      ext->ptActAngelMark = createCommmonEffectAction(
          101812, ptAct->gx, ptAct->gy, 0, 0, DISP_PRIO_CHAR);
    else
      ext->ptActAngelMark = createCommmonEffectAction(
          101865, ptAct->gx, ptAct->gy, 0, 0, DISP_PRIO_CHAR);
    ...
    if (getCartoonSize(ptAct, &x1, &y1)) {
      ...
      // ??????????????
      setCharPrio(ext->ptActAngelMark->bmpNo, ext->ptActAngelMark->x,
                  ext->ptActAngelMark->y, 0,
                  ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_)
                                     : -(y1 / 2 + _FANILYTEXIAOY_),
  ...
  if ((status & CHR_STATUS_HELP) != 0) {
    if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
      ...
      if (getCartoonSize(ptAct, &x1, &y1)) {
        ...
        // ??????????????
        setCharPrio(bmpNo, ptAct->x, ptAct->y, 0,
                    ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_ - 20)
                                       : -(y1 / 2 + _FANILYTEXIAOY_ - 20),
                    ptAct->mx, ptAct->my
        );
      }
    }
  ...
  if ((status & CHR_STATUS_LEADER) != 0 && ext->ptActLeaderMark == NULL &&
    ...
    // ??????????????
    ext->ptActLeaderMark = createCommmonEffectAction(
        SPR_leader, ptAct->gx, ptAct->gy, 0, 0, DISP_PRIO_CHAR);
  } else if (((status & CHR_STATUS_LEADER) == 0 ||
              (status & CHR_STATUS_BATTLE) != 0) &&
    ...
    if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
      ...
      if (!ATR_PAT_NO(ext->ptActLeaderMark)) {
        if (ATR_PAT_BAK_NO(ext->ptActLeaderMark)) {
          ATR_PAT_NO(ext->ptActLeaderMark) =
              ATR_PAT_BAK_NO(ext->ptActLeaderMark);
        }
      }
      short x1, y1;
      if (getCartoonSize(ptAct, &x1, &y1)) {
        ...
        // ??????????????
        setCharPrio(ext->ptActLeaderMark->bmpNo, ext->ptActLeaderMark->x,
                    ext->ptActLeaderMark->y, 0,
                    ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_)
                                       : -(y1 / 2 + _FANILYTEXIAOY_),
  ...
  if ((status & CHR_STATUS_USE_MAGIC) != 0 && ext->ptActMagicEffect == NULL) {
    // ????????????
    ext->ptActMagicEffect = createCommmonEffectAction(
        SPR_effect01, ptAct->gx, ptAct->gy + 1, 0, 0, DISP_PRIO_CHAR);
    if (pc.ptAct != NULL && pc.ptAct == ptAct)
      delPcUseMagic();
    else
    ...
    if (ext->drawFukidashiTime > TimeGetTime()) {
      ...
      if (SPR_001em <= ptAct->anim_chr_no && ptAct->anim_chr_no <= SPR_114bw) {
        if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
          ...
          short x1, y1;
          if (getCartoonSize(ptAct, &x1, &y1)) {
            setCharPrio(bmpNo, ptAct->x, ptAct->y, 20,
                        ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_)
                                           : -(y1 / 2 + _FANILYTEXIAOY_),
                        ptAct->mx, ptAct->my
            );
  ...
  extern void PutTitleText(int x, int y, char fontPrio, TITLE_STR str,
                           BOOL hitFlag);
  ...
  if (ext1) {
    if (!ext1->ptStreetVendor) {
      if (ptAct->TitleText.flg == 1) {
        short x1, y1;
        if (getCartoonSize(ptAct, &x1, &y1)) {
          PutTitleText(ptAct->x - ptAct->TitleText.len / 2,
                       ptAct->y - (y1 / 2 + 45) - 12, 0, ptAct->TitleText, 0);
        }
      }
    }
  }
#endif
  if (nameOverTheHeadFlag && ptAct != pc.ptAct &&
      (ptAct->atr & (ACT_ATR_TYPE_PC | ACT_ATR_TYPE_OTHER_PC |
                     ACT_ATR_TYPE_PET | ACT_ATR_TYPE_OTHER)) &&
...
#ifdef _AniCharBubble // Syu ADD 动画层人物吐出气泡
  if (((nowFloor == 817) || (nowFloor == 8007) || (nowFloor == 8101) ||
       (nowFloor == 8100) || (nowFloor == 8027) || (nowFloor == 8028) ||
       (nowFloor == 8029) || nowFloor == 8113 || nowFloor == 8114 ||
       (nowFloor == 8015)) &&
      (popflag == false) && (waittimeflag == false)) {
  ...
  if (((nowFloor != 817) && (nowFloor != 8007) && (nowFloor != 8101) &&
       (nowFloor != 8100) && (nowFloor != 8113) && (nowFloor != 8114) &&
       (nowFloor != 8027) && (nowFloor != 8028) && (nowFloor != 8029) &&
       (nowFloor != 8015)) &&
      (popflag == true)) {
    popflag = false;
    waittimeflag = false;
  ...
  if (ext->ptActEmotion) {
    if (ext->ptActEmotion->mx != ptAct->mx ||
        ...
        ext->ptActEmotion->x != ptAct->x || ext->ptActEmotion->y != ptAct->y) {
      ...
      if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
        ...
        if (getCartoonSize(ptAct, &x1, &y1)) {
          pattern(ext->ptActEmotion, ANM_NOMAL_SPD, ANM_LOOP);
          setCharPrio(ext->ptActEmotion->bmpNo, ext->ptActEmotion->x,
                      ext->ptActEmotion->y, 0,
                      ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_)
                                         : -(y1 / 2 + _FANILYTEXIAOY_),
  ...
  if (ext->ptStreetVendor) {
    ...
    if (pand) {
      ...
      StockDispBuffer(ext->ptStreetVendor->x, ext->ptStreetVendor->y + 35,
                      DISP_PRIO_TILE + 1, 35343, 0);
      StockDispBuffer(ext->ptStreetVendor->x - 10, ext->ptStreetVendor->y + 10,
                      DISP_PRIO_CHAR - 1, 35344, 0);
      StockDispBuffer(ext->ptStreetVendor->x, ext->ptStreetVendor->y - 15,
                      DISP_PRIO_CHAR + 1, 35345, 0);
#else
#ifdef _READ16BITBMP
      StockDispBuffer(ext->ptStreetVendor->x, ext->ptStreetVendor->y - 80,
                      DISP_PRIO_CHAR, OLD_GRAPHICS_START + 58, 10);
#endif
#endif
      StockFontBuffer(ext->ptStreetVendor->x - 65, ext->ptStreetVendor->y - 92,
                      FONT_PRIO_BACK, MyChatBuffer.color,
                      ptAct->szStreetVendorTitle, 0);
    }
  }
#endif
#ifdef _MIND_ICON
  if (ext->ptMindIcon) {
    ...
    // 这里删除掉就可以走动显示
    if (((ptAct->sMindIcon != SPR_asleep) && (ptAct->sMindIcon != SPR_cafe) &&
      ...
      if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
        ...
        if (getCartoonSize(ptAct, &x1, &y1)) {
          ...
          setCharPrio(ext->ptMindIcon->bmpNo, ext->ptMindIcon->x,
                      ext->ptMindIcon->y, 0,
                      ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_)
                                         : -(y1 / 2 + _FANILYTEXIAOY_),
    ...
    {
      {
        ...
        if (getCartoonSize(ptAct, &x1, &y1)) {
          ...
          setCharPrio(ext->ptNoticeIcon->bmpNo, ext->ptNoticeIcon->x - x2,
                      ext->ptNoticeIcon->y, 0, -(y1 / 2 + _FANILYTEXIAOY_ + 10),
  ...
  if (ext->ptFamilyIcon) { // 这里删除掉就可以走动显示
    {
      if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
        ...
        if (getCartoonSize(ptAct, &x1, &y1)) {
          ...
          setCharPrio(ext->ptFamilyIcon->bmpNo, ext->ptFamilyIcon->x - x2,
                      ext->ptFamilyIcon->y, 0, -(y1 / 2 + _FANILYTEXIAOY_) - 2,
                      ext->ptFamilyIcon->mx, ext->ptFamilyIcon->my
          );
#else
          setCharPrio(ext->ptFamilyIcon->bmpNo, ext->ptFamilyIcon->x - x2,
                      ext->ptFamilyIcon->y, 0, -(y1 / 2 + _FANILYTEXIAOY_),
                      ext->ptFamilyIcon->mx, ext->ptFamilyIcon->my
          );
#endif
        }
  ...
  if (ext->ptmFamilyIcon) { // 这里删除掉就可以走动显示
    {
      if (人物屏蔽开关 && ProcNo == PROC_GAME && pc.ptAct != ptAct) {
        ...
        if (getCartoonSize(ptAct, &x1, &y1)) {
          ...
          setCharPrio(ext->ptmFamilyIcon->bmpNo, ext->ptmFamilyIcon->x,
  ...
  if (ext->pActFirework[0]) // 有要放烟火
  {
    ...
    camMapToGamen((float)ext->pActFirework[0]->gx * GRID_SIZE,
                  (float)ext->pActFirework[0]->gy * GRID_SIZE, &fX, &fY);
    ...
    if (pattern(ext->pActFirework[0], ANM_NOMAL_SPD, ANM_NO_LOOP)) {
      ...
      } else if (ext->pActFirework[0]->damage == 2 ||
                 ext->pActFirework[0]->damage == -1)
        ext->pActFirework[0]->damage = -2;
  ...
  if (ext->pActFirework[1]) {
    ...
    camMapToGamen((float)ext->pActFirework[1]->gx * GRID_SIZE,
                  (float)ext->pActFirework[1]->gy * GRID_SIZE, &fX, &fY);
    ...
    if (pattern(ext->pActFirework[1], ANM_NOMAL_SPD, ANM_NO_LOOP)) {
      if (--ext->pActFirework[1]->level >= 0) // 计算还要跑几次
      {
        ...
        if (ext->pActFirework[1]->damage == 3) {
          ...
          ext->pActFirework[1]->gx =
              ext->pActFirework[1]->bufGx[0] + nXY[nDir][0];
          ext->pActFirework[1]->gy =
              ext->pActFirework[1]->bufGy[0] + nXY[nDir][1];
        }
      } else {
...
#ifdef _NPC_MAGICCARD
  {
    ...
    */
    // StockFontBufferExt( 400 , 240-offsetYtbl[iPosition[0]] , FONT_PRIO_FRONT,
    // 6 , "-1" , 0 , 18+iPosition[0] ); StockFontBufferExt( 10 , 240 ,
    // FONT_PRIO_FRONT, 2 , "-8" , 0 , 18 );
...
#ifdef _MOVE_SCREEN
        if (pc.bMoveScreenMode)
          camMapToGamen((float)(Positiontable[iPosition[i]].humanX -
                                iScreenMoveX + ioffsetsx) *
                            GRID_SIZE,
                        (float)(Positiontable[iPosition[i]].humanY -
                                iScreenMoveY + ioffsetsy) *
                            GRID_SIZE,
                        &fX, &fY);
        else
        // camMapToGamen((float)Positiontable[iPosition[i]].humanX * GRID_SIZE,
        // (float)Positiontable[iPosition[i]].humanY * GRID_SIZE, &fX, &fY);
#endif
          camMapToGamen(
              (float)(Positiontable[iPosition[i]].humanX + ioffsetsx) *
                  GRID_SIZE,
              (float)(Positiontable[iPosition[i]].humanY + ioffsetsy) *
                  GRID_SIZE,
              &fX, &fY);
        // StockFontBuffer( (int)fX+30 , (int)fY-20-offsetYtbl[iOffsetY[i]] ,
        // FONT_PRIO_FRONT, color , buf , 0 ); StockFontBufferExt( (int)fX-20 ,
        // (int)fY , FONT_PRIO_FRONT, color , buf , 0 , 36); StockFontBufferExt(
        // (int)fX-10 , (int)fY-20-offsetYtbl[iOffsetY[i]] , FONT_PRIO_FRONT,
        // color , buf , 0 , 36); StockFontBufferExt( (int)fX-10 , (int)fY-20 ,
        // FONT_PRIO_FRONT, color , buf , 0 , 18+iOffsetY[i]);
        // StockFontBufferExt( (int)fX-80+offsetYtbl[iOffsetY[i]] , (int)fY-50 ,
        // FONT_PRIO_FRONT, color , buf , 0 , 18+fonttbl[iOffsetY[i]]);
        // StockFontBufferExt( (int)fX+4 , (int)fY-iOffsetY[i] ,
        // FONT_PRIO_FRONT, color , buf , 0 , 18+fonttbl[iOffsetY[i]]);
        StockFontBuffer((int)fX + 4, (int)fY - iOffsetY[i], FONT_PRIO_FRONT,
                        color, buf, 0);
      }
    }

    if (ext->pActMagiccard[0]) {
      camMapToGamen((float)ext->pActMagiccard[0]->gx * GRID_SIZE,
                    (float)ext->pActMagiccard[0]->gy * GRID_SIZE, &fX, &fY);
    ...
    if (ext->pActMagiccard[1] && ext->pActMagiccard[0] == NULL) {
      camMapToGamen((float)ext->pActMagiccard[1]->gx * GRID_SIZE,
                    (float)ext->pActMagiccard[1]->gy * GRID_SIZE, &fX, &fY);
    ...
    if (ext->pActMagiccard[2] && ext->pActMagiccard[0] == NULL &&
        ext->pActMagiccard[1] == NULL) {
      camMapToGamen((float)ext->pActMagiccard[2]->gx * GRID_SIZE,
                    (float)ext->pActMagiccard[2]->gy * GRID_SIZE, &fX, &fY);
    ...
    if (ext->pActMagiccard[3] && ext->pActMagiccard[0] == NULL &&
        ext->pActMagiccard[1] == NULL && ext->pActMagiccard[2] == NULL) {
      camMapToGamen((float)ext->pActMagiccard[3]->gx * GRID_SIZE,
                    (float)ext->pActMagiccard[3]->gy * GRID_SIZE, &fX, &fY);
  ...
  if (((ptAct->picture >> 24) & 0x000000ff) == 2 && ext->ptActPicture == NULL) {
    if (g_bUseAlpha)
      ext->ptActPicture = createCommmonEffectAction(
          101812, ptAct->gx, ptAct->gy, 0, 0, DISP_PRIO_CHAR);
    else
      ext->ptActPicture = createCommmonEffectAction(
          101865, ptAct->gx, ptAct->gy, 0, 0, DISP_PRIO_CHAR);
  } else if (((ptAct->picture >> 24) & 0x000000ff) != 2 &&
             ext->ptActPicture != NULL) {
    // ??????????????
    DeathAction(ext->ptActAngelMark);
    ext->ptActAngelMark = NULL;
  } else if (((ptAct->picture >> 24) & 0x000000ff) == 2 &&
             ext->ptActPicture != NULL) {
    int pictable[9] = {100388, 100841, 101178, 100854, 101570,
                       100353, 101759, 101424, 101489};
    if (ptAct->anim_chr_no ==
        pictable[((ptAct->picture & 0x00ff0000) >> 16) & 0x000000ff]) {
      short x1, y1;
      if (getCartoonSize(ptAct, &x1, &y1)) {
        ...
        // ??????????????
        setCharPrio(ext->ptActPicture->bmpNo, ext->ptActPicture->x,
                    ext->ptActPicture->y, 0,
                    ptAct->sFamilyIcon ? -(y1 / 2 + _OTHERTEXIAOY_)
                                       : -(y1 / 2 + _FANILYTEXIAOY_),
                    ext->ptActPicture->mx, ext->ptActPicture->my
        );
      }
    }
  }
...
// ???????? /////////////////////////////////////////////////
ACTION *createCommmonEffectAction(int graNo, int gx, int gy, int anim, int dir,
                                  int prio) {
  ACTION *ptAct;
  float mx, my;
  /* ?????????? */
...
ACTION *createCommmonEffectNoLoop(int graNo, int gx, int gy, int anim, int dir,
                                  int prio) {
  ACTION *ptAct;
  float mx, my;
  /* ?????????? */
...
void changeCharAct(ACTION *ptAct, int x, int y, int dir, int action,
  ...
  case 51:
    ...
    ext->pActFirework[0]->damage =
        effectno; // 借用 damage 来储存 type (effectno = type)
  ...
  case 60: {
    ...
    if (effectparam2 == 3) { // 魔法牌
      ...
      if (effectparam1 == 101120 || effectparam1 == 101628) {
        ext->pActMagiccard[0]->gx =
            Positiontable[effectno].humanX + 1 + x; //.magic1X;
        ext->pActMagiccard[0]->gy =
            Positiontable[effectno].humanY - 1 + y; //.magic1Y;
      } else {
...
#else
int checkCharObjPointNotStatus(int gx, int gy, short type,
                               unsigned short status)
#endif
{
  int i;
...
void setNpcCharObj(int id, int graNo, int gx, int gy, int dir, char *fmname,
                   char *name, char *freeName, int level, char *petname,
                   int petlevel, int nameColor, int walk, int height,
                   int charType, int profession_class, char *gm_name)
#else
#ifdef _NPC_PICTURE
void setNpcCharObj(int id, int graNo, int gx, int gy, int dir, char *fmname,
                   char *name, char *freeName, int level, char *petname,
                   int petlevel, int nameColor, int walk, int height,
                   int charType, int profession_class, int picture)
#else
void setNpcCharObj(int id, int graNo, int gx, int gy, int dir, char *fmname,
                   char *name, char *freeName, int level, char *petname,
                   int petlevel, int nameColor, int walk, int height,
                   int charType, int profession_class)
#endif
#endif
#else
#ifdef _NPC_EVENT_NOTICE
...
#endif
{
  ...
  charObj[no].ptAct->picturetemp =
      ((((picture) & 0x00ff0000) >> 16) & 0x000000ff); //(picture&0x0000ffff);
...
BOOL setReturnPetObj(int id, int graNo, int gx, int gy, int dir, char *name,
                     char *freeName, int level, int nameColor, int walk,
                     int height, int charType) {
  int no;
  // ???????????
  if ((no = searchCharObjId(id)) >= 0)
    // ?????
...
//   ?????
//
void setItemCharObj(int id, int graNo, int gx, int gy, int dir, int classNo,
                    char *info) {
  int no;
  BOOL existFlag = FALSE;
  CHAREXTRA *ext;
...
//   ???
//
void setMoneyCharObj(int id, int graNo, int gx, int gy, int dir, int money,
                     char *info) {
  int no;
  BOOL existFlag = FALSE;
  CHAREXTRA *ext;
...
void restorePtActCharObjAll(void) {
  ...
  for (i = 0; i < MAX_CHAROBJ; i++) {
    if (charObj[i].use != CHAROBJ_USE_FREE && charObj[i].ptAct == NULL) {
      charObj[i].ptAct = createCharAction(charObj[i].graNo, charObj[i].gx,
                                          charObj[i].gy, charObj[i].dir);
      if (charObj[i].ptAct == NULL) {
        continue;
      }
...
void restorePtActCharObj(int id) {
  ...
  if (charObj[no].use != CHAROBJ_USE_FREE && charObj[no].ptAct == NULL) {
    charObj[no].ptAct = createCharAction(charObj[no].graNo, charObj[no].gx,
                                         charObj[no].gy, charObj[no].dir);
    if (charObj[no].ptAct == NULL) {
      // ???????????????
      return;
    }
...
void initItemOverlapCheck(void) { itemOverlapCheckCnt = 0; }

BOOL itemOverlapCheck(unsigned int bmpNo, int gx, int gy) {
  int i;
  BOOL flag = FALSE;
  for (i = 0; i < itemOverlapCheckCnt; i++) {
    if (itemOverlapTbl[i].bmpNo == bmpNo && itemOverlapTbl[i].gx == gx &&
...
void getPetRoute(ACTION *ptAct) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    gx = ptAct->gx + dx;
    gy = ptAct->gy + dy;
    if ((gx != pe->preGx || gy != pe->preGy) && checkHitMap(gx, gy) == FALSE) {
      break;
    }
    dir += (2 * (i + 1));
    ajustClientDir(&dir);
...
BOOL petMoveProc(ACTION *ptAct) {
  ...
  // ??????????
  // ???8??????
  if (ABS(ptAct->gx - nowGx) >= 13 || ABS(ptAct->gy - nowGy) >= 13 ||
      (pe->createTime + 8000 < TimeGetTime() && ptAct->vx == 0 &&
       ptAct->vy == 0)) {
  ...
  if (pe->ptAct == NULL && pe->createTime + 7200 < TimeGetTime()) {
    pe->ptAct = createCommmonEffectNoLoop(SPR_difence, ptAct->gx, ptAct->gy, 0,
                                          0, ptAct->dispPrio);
  }
  if (pe->ptAct2 == NULL && pe->createTime + 7500 < TimeGetTime()) {
    pe->ptAct2 = createCommmonEffectNoLoop(SPR_difence, ptAct->gx, ptAct->gy, 0,
                                           0, ptAct->dispPrio);
  }
...
BOOL petCircleOutProc(ACTION *ptAct) {
  ...
  if (pe->ptAct == NULL) {
    ...
    pe->angle = 0.0F;
    pe->r = 0;
    createCommmonEffectNoLoop(SPR_hoshi, ptAct->gx, ptAct->gy, 0, 0,
                              ptAct->dispPrio);
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    // ??????????????
    setCharPrio(ptAct->bmpNo, ptAct->x, ptAct->y, 0, 0, ptAct->mx + dx,
                ptAct->my + dy
    );
    angle += 120;
    angle = AdjustDir(angle);
  }
...
BOOL petCircleInProc(ACTION *ptAct) {
  ...
  if (pe->ptAct != NULL) {
    if (pe->ptAct2 == NULL && pe->createTime + 2300 < TimeGetTime()) {
      pe->ptAct2 = createCommmonEffectNoLoop(SPR_hoshi, ptAct->gx, ptAct->gy, 0,
                                             0, ptAct->dispPrio);
    }
    pe->r -= 4;
    pe->angle += 6.0F;
...
// 创建宠物的Action
ACTION *createPetAction(int graNo,
                        int gx,
                        int gy,
                        int dir, // 方向
                        int mode,
...
void 设置静止的动作为站立(void) {
  ...
  for (i; i < MAX_CHAROBJ; i++) {
    if (charObj[i].charType == 256) {
      if (charObj[i].ptAct) {
        if (charObj[i].ptAct->anim_no == 5 || charObj[i].ptAct->anim_no == 2 ||
            charObj[i].ptAct->anim_no == 0 || charObj[i].ptAct->anim_no == 1 ||
