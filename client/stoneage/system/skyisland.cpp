...
void LoadBmpToSurface(int noFrom, LPDIRECTDRAWSURFACE lpsurTo, int w, int h) {
  ...
  if (lpsurTo->Lock(NULL, &ddsd, DDLOCK_WAIT, NULL) == DD_OK) {
    if (realGetImage(noFrom, (unsigned char **)&pRealBinBits, &RealBinWidth,
                     &RealBinHeight)) {
      // realbin档图素要倒着读
      pRealBinBits += (w * (h - 1));
      if (displayBpp == 32) {
...
void SkyIslandLoadBmp() {
  if (sky_island_no) {
    ...
    if (w != LandWidth || h != LandHeight) {
      fMapWidth =
          ((nowFloorGxSize + nowFloorGySize) * SURFACE_WIDTH) >>
          1; // fMapWidth = (nowFloorGxSize+nowFloorGySize)*SURFACE_WIDTH/2;
      fMapHeight =
          ((nowFloorGxSize + nowFloorGySize) * SURFACE_HEIGHT) >>
          1; // fMapWidth = (nowFloorGxSize+nowFloorGySize)*SURFACE_WIDTH/2;
      LandWidth = w;
      LandHeight = h;
      if (lpsurLand) {
    ...
    if (lpsurLand) {
      ...
      if (!lpsurCloud1) {
        ...
        if (lpsurCloud1 = CreateSurface(lpDraw->xSize, lpDraw->ySize, 0,
      ...
      if (!lpsurCloud2) {
        ...
        iCloudX2 = rand() % w;
        iCloudY2 = rand() % h;
        if (lpsurCloud2 = CreateSurface(lpDraw->xSize, lpDraw->ySize, 0,
                                        DDSCAPS_SYSTEMMEMORY)) {
...
#endif
{
  ...
  if (sky_island_no && lpsurCloud1 && lpsurLand && pc.ptAct) {
...
#ifdef _READ16BITBMP
    if (g_bUseAlpha)
      lpDraw->lpBACKBUFFERSYS->BltFast(0, 0, lpsurLand, &rc, DDBLTFAST_WAIT);
#endif
#ifdef _MOON_FAIRYLAND // (不可开) ROG ADD 月之仙境
    if (moonFlag == TRUE)
      return;
    ...
    if ((rc.bottom - rc.top) > 0 && (rc.right - rc.left) > 0) {
      lpDraw->lpBACKBUFFER->BltFast(x, y, lpsurCloud1, &rc,
                                    DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
#ifdef _READ16BITBMP
      if (g_bUseAlpha)
        lpDraw->lpBACKBUFFERSYS->BltFast(
            x, y, lpsurCloud1, &rc, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
#endif
    }
  }
}
...
#endif
{
  ...
  if (bDrawUpCloud) {
    ...
    if (sky_island_no && lpsurCloud2
      ...
      if ((rc.bottom - rc.top) > 0 && (rc.right - rc.left) > 0) {
        lpDraw->lpBACKBUFFER->BltFast(x, y, lpsurCloud2, &rc,
                                      DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
#ifdef _READ16BITBMP
        if (g_bUseAlpha)
          lpDraw->lpBACKBUFFERSYS->BltFast(
              x, y, lpsurCloud2, &rc, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
#endif
      }
    }
  }
