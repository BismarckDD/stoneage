...
#ifdef _READ16BITBMP
  if (g_bUseAlpha)
    lpBattleSurfaceSys->BltFast(0, 0, lpDraw->lpBACKBUFFERSYS, NULL,
                                DDBLTFAST_WAIT);
#endif
}
...
BOOL ProduceAccele(int mode) {
  ...
  if (endFlag == TRUE) {
    ...
    a = 0;
    // ????┤阪?
    if (GameState == GAME_FIELD_TO_ENCOUNT)
      GameState = GAME_ENCOUNT_TO_BATTLE;
    else if (GameState == GAME_ENCOUNT_TO_BATTLE)
...
BOOL Produce4Way(int mode) {
  RECT rect[4] = {
      ...
      {320 + (DISPLACEMENT_X >> 1), 0, 640 + DISPLACEMENT_X,
       240 + (DISPLACEMENT_Y >> 1)},
      {0, 240 + (DISPLACEMENT_Y >> 1), 320 + (DISPLACEMENT_X >> 1),
       480 + DISPLACEMENT_Y},
      {320 + (DISPLACEMENT_X >> 1), 240 + (DISPLACEMENT_Y >> 1),
       640 + DISPLACEMENT_X, 480 + DISPLACEMENT_Y}};
  static int endFlag = -1;
  static int x[4], y[4];
  int d = 4, i;
  // ??????
...
BOOL Produce_bran_small(int ang) {
  ...
  if (bran_flg == 0) { //????
    bran_flg = 1;      //?????
    d0 = 0;
    for (d7 = -240 - (DISPLACEMENT_Y >> 1) + (BRAN_SIZE_Y >> 1);
         d7 < 240 + (DISPLACEMENT_Y >> 1) + (BRAN_SIZE_Y >> 1);
         d7 += BRAN_SIZE_Y) {
      for (d6 = -320 - (DISPLACEMENT_X >> 1) + (BRAN_SIZE_X >> 1);
           d6 < 320 + (DISPLACEMENT_X >> 1) + (BRAN_SIZE_X >> 1);
           d6 += BRAN_SIZE_X) {
        pos_tbl[d0++] = d6;
        pos_tbl[d0++] = d7;
      }
  ...
  //    ClearBackSurface();
  a0 = pos_tbl +
       (((DISPLACEMENT_Y + 480) / BRAN_SIZE_Y) *
  ...
  for (d7 = 480 + DISPLACEMENT_Y - BRAN_SIZE_Y; d7 >= 0; d7 -= BRAN_SIZE_Y) {
    for (d6 = 640 + DISPLACEMENT_X - BRAN_SIZE_X; d6 >= 0; d6 -= BRAN_SIZE_X) {
      ...
      if (gResolutionMode == 1) {
        ...
        lpDraw->lpBACKBUFFER->BltFast(x, y, lpBattleSurface, &rect,
                                      DDBLTFAST_WAIT);
      } else {
        x = ((*a0 * bran_cnt) >> 6) + 320 + (DISPLACEMENT_X >> 1) -
            (BRAN_SIZE_X >> 1);
        y = ((*(a0 + 1) * bran_cnt) >> 6) + 240 + (DISPLACEMENT_Y >> 1) -
            (BRAN_SIZE_Y >> 1);
        lpDraw->lpBACKBUFFER->BltFast(x, y, lpBattleSurface, &rect,
                                      DDBLTFAST_WAIT);
      }
...
BOOL Produce_break_up2(int ang) {
  ...
  for (d7 = 0; d7 < bran_cnt; d7++) {
    ...
    rect.bottom = rect.top + 1;
    lpDraw->lpBACKBUFFER->BltFast(0, lpDraw->ySize - d7, lpBattleSurface, &rect,
                                  DDBLTFAST_WAIT);
  }
  if (ang < 0) {
    bran_cnt -= BREAK_UP;
...
BOOL ProduceCenterPress(int mode) {
  ...
  for (i = 0; i < line; i++) {
    ...
    lpDraw->lpBACKBUFFER->BltFast(0, h - i, lpBattleSurface, &rect1,
                                  DDBLTFAST_WAIT);
    rect2.top = i * h / line + h;
    rect2.bottom = rect2.top + 1;
    lpDraw->lpBACKBUFFER->BltFast(0, i + h, lpBattleSurface, &rect2,
                                  DDBLTFAST_WAIT);
  }
  if (mode == 0) {
    line -= spd;
...
void TitleProduce(void) {
  ...
  // ???????牙
  StockDispBuffer((int)x[8] + DISPLACEMENT_X / 2, (int)y[8], DISP_PRIO_CHAR,
                  CG_TITLE_JSS_LOGO, 0);
  ...
  if (cnt3 < 8) {
    ...
    if (cnt3 < 26) {
      StockDispBuffer((int)x[0] + DISPLACEMENT_X / 2, (int)y[0], DISP_PRIO_CHAR,
                      CG_TITLE_NAME_FLASH, 0);
    } else {
      StockDispBuffer((int)x[0] + DISPLACEMENT_X / 2, (int)y[0], DISP_PRIO_CHAR,
                      CG_TITLE_NAME_FLASH1 + cnt3 - 26, 0);
    }
    cnt3++;
  } else
    StockDispBuffer((int)x[0] + DISPLACEMENT_X / 2, (int)y[0], DISP_PRIO_CHAR,
