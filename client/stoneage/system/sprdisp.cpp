...
int iProcessAniNum;
#endif
void DrawGrayA(LPDIRECTDRAWSURFACE lpSurface, int ox, int oy, int offsetx,
               int offsety, int ow, int oh, bool bLastOne, int iGrayType);
#ifdef _READ16BITBMP
void DrawAlpha(LPDIRECTDRAWSURFACE lpSurface, int ox, int oy, int offsetx,
               int offsety, int ow, int oh, int Alpha, bool bLastOne);
extern int displayBpp;
...
BOOL PutTileBmp(void) {
  ...
  if ((ProcNo == PROC_GAME /* && SubProcNo==3*/) || ProcNo == PROC_BATTLE) {
    ...
    if (fastDrawTile &&
      ...
      if (ProcNo != PROC_BATTLE) {
        ...
        if (lpDraw->lpBACKBUFFER->BltFast(sx, sy, lpBattleSurface, &src,
                                          DDBLTFAST_WAIT) == DDERR_SURFACEBUSY)
          SurfaceBusyFlag = TRUE;
#ifdef _READ16BITBMP
        if (g_bUseAlpha) {
          if (lpDraw->lpBACKBUFFERSYS->BltFast(sx, sy, lpBattleSurfaceSys, &src,
                                               DDBLTFAST_WAIT) ==
              DDERR_SURFACEBUSY)
            SurfaceBusyFlag = TRUE;
        }
  ...
  if (retainbackbuffer) {
    if (lpBattleSurface->BltFast(0, 0, lpDraw->lpBACKBUFFER, NULL,
                                 DDBLTFAST_WAIT) == DDERR_SURFACEBUSY)
      SurfaceBusyFlag = TRUE;
#ifdef _READ16BITBMP
    if (g_bUseAlpha)
      if (lpBattleSurfaceSys->BltFast(0, 0, lpDraw->lpBACKBUFFERSYS, NULL,
                                      DDBLTFAST_WAIT) == DDERR_SURFACEBUSY)
        SurfaceBusyFlag = TRUE;
#endif
  }
...
void PutBmp(void) {
  ...
  case DRAW_BACK_NORMAL:        // 无背景
    ...
    if (!PutTileBmp()) {
      if (fastDrawTile &&
          lpDraw->lpBACKBUFFER->BltFast(
              0, 0, lpBattleSurface, NULL,
              DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY) == DDERR_SURFACEBUSY) {
        SurfaceBusyFlag = TRUE;
        return;
      }
    }
  ...
  for (i = 0; i < DispBuffer.DispCnt; i++, pDispSort++) {
    ...
    if (bmpNo & STOCK_DISP_BUFFER_NO_BMP) {
      if (bmpNo & (STOCK_DISP_BUFFER_BOX | STOCK_DISP_BUFFER_BOX_FILL |
                   STOCK_DISP_BUFFER_LINE)) {
      ...
      continue;
    {
      if (pDispInfo->DrawEffect == 6) {
        ...
        for (lpSurfaceInfo = SpriteInfo[bmpNo].lpSurfaceInfo;
          ...
          sourceRect.left = 0;
          sourceRect.top = 0;
          sourceRect.right = min(SurfaceSizeX, SpriteInfo[bmpNo].width -
                                                   lpSurfaceInfo->offsetX);
          sourceRect.bottom = min(SurfaceSizeY, SpriteInfo[bmpNo].height -
                                                    lpSurfaceInfo->offsetY);

          destinationRect.left =
              lpSurfaceInfo->offsetX * lpDraw->xSize / SpriteInfo[bmpNo].width;
          destinationRect.top =
              lpSurfaceInfo->offsetY * lpDraw->ySize / SpriteInfo[bmpNo].height;
          destinationRect.right = (lpSurfaceInfo->offsetX + sourceRect.right) *
                                  lpDraw->xSize / SpriteInfo[bmpNo].width;
          destinationRect.bottom =
              (lpSurfaceInfo->offsetY + sourceRect.bottom) * lpDraw->ySize /
      ...
      for (lpSurfaceInfo = SpriteInfo[bmpNo].lpSurfaceInfo;
        ...
        if (pDispInfo->DrawEffect == 2) // 饱和处理
          DrawStaturated(lpSurfaceInfo->lpSurface,
                         pDispInfo->x + lpSurfaceInfo->offsetX,
                         pDispInfo->y + lpSurfaceInfo->offsetY,
                         lpSurfaceInfo->offsetX, lpSurfaceInfo->offsetY,
                         SpriteInfo[bmpNo].width, SpriteInfo[bmpNo].height,
                         (lpSurfaceInfo->pNext == NULL ? true : false));
        else if (pDispInfo->DrawEffect == 3 ||
                 pDispInfo->DrawEffect == 4) // 石化和中毒处理
          ...
          DrawGray(lpSurfaceInfo->lpSurface,
                   ...
                   lpSurfaceInfo->offsetX, lpSurfaceInfo->offsetY,
                   SpriteInfo[bmpNo].width, SpriteInfo[bmpNo].height,
                   (lpSurfaceInfo->pNext == NULL ? true : false), 3);
        }
#endif
#ifdef _NEW_COLOR_
        else {
          ...
          if (SpriteInfo[bmpNo].AlphaFlg) {
            ...
            if (lpDraw->lpBACKBUFFER->Lock(NULL, &ddsd, DDLOCK_WAIT, NULL) !=
                DD_OK)
              continue;
            if (lpSurfaceInfo->lpSurface->Lock(NULL, &ddsd2, DDLOCK_WAIT,
                                               NULL) != DD_OK)
              continue;
            ...
            if (displayBpp == 16) {
              ablend_565(lpSurfaceInfo->lpAlphaData, 64, Src, rect.left,
                         rect.top, Srcpitch, Dst, x0, y0,
                         rect.right - rect.left, rect.bottom - rect.top,
                         Dstpitch);
            } else {
              DrawAlpha32((unsigned long *)Dst, x0, y0, Dstpitch,
                          (unsigned long *)Src, rect.left, rect.top,
                          rect.right - rect.left, rect.bottom - rect.top,
                          Srcpitch);
            }
            if (lpSurfaceInfo->lpSurface->Unlock(NULL) != DD_OK)
              continue;
...
#endif
          {
            if (DrawSurfaceFast(pDispInfo->x + lpSurfaceInfo->offsetX,
                                pDispInfo->y + lpSurfaceInfo->offsetY,
                                lpSurfaceInfo->lpSurface) == DDERR_SURFACEBUSY)
              SurfaceBusyFlag = TRUE;
          }
          lpSurfaceInfo->date = SurfaceDate;
        }
...
int StockDispBuffer(int x, int y, UCHAR dispPrio, int bmpNo, BOOL hitFlag) {
  ...
  if (DispBuffer.DispCnt >= DISP_BUFFER_SIZE)
    return -2;

  if (-1 <= bmpNo && bmpNo <= CG_INVISIBLE)
    return -2;
...
int StockDispBufferScaled(int x, int y, UCHAR dispPrio, int bmpNo) {
  ...
  if (id >= 0)
    DispBuffer.DispInfo[id].DrawEffect = 6;
  return id;
}
...
int StockDispBuffer2(int x, int y, UCHAR dispPrio, int bmpNo, BOOL hitFlag)
{
  ...
  pDispInfo->pAct = NULL;
#ifdef _READ16BITBMP
  if (pDispInfo->bmpNo >= OLD_GRAPHICS_START &&
      pDispInfo->bmpNo < MAX_GRAPHICS) {
...
void StockTaskDispBuffer(void) {
  ...
  for (pActLoop = pActTop->pNext; pActLoop != pActBtm;
       /* ?????????? */
    ...
    // ??????????????
    if (-1 <= pActLoop->bmpNo && pActLoop->bmpNo <= CG_INVISIBLE)
      continue;

    // ????????????
    if (pActLoop->deathFlag == TRUE)
      continue;

    /* ???? */
    if (pActLoop->atr & ACT_ATR_HIDE)
      continue;
    ...
    if (pActLoop->atr & ACT_ATR_HIT_BOX_COL1)
      pDispInfo->hitFlag = 7;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_COL2)
      pDispInfo->hitFlag = 8;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_COL3)
      pDispInfo->hitFlag = 9;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_COL4)
      pDispInfo->hitFlag = 10;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_COL5)
      pDispInfo->hitFlag = 11;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_COL6)
      pDispInfo->hitFlag = 12;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_COL7)
      pDispInfo->hitFlag = 13;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_COL8)
      pDispInfo->hitFlag = 14;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_ALL4)
      pDispInfo->hitFlag = 6;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_ALL3)
      pDispInfo->hitFlag = 5;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_ALL2)
      pDispInfo->hitFlag = 4;
    else if (pActLoop->atr & ACT_ATR_HIT_BOX_ALL1)
      pDispInfo->hitFlag = 3;
    else // ????
      if (pActLoop->atr & ACT_ATR_HIT_BOX)
        pDispInfo->hitFlag = 2;
      else // ??
        if (pActLoop->atr & ACT_ATR_HIT)
          pDispInfo->hitFlag = 1;
        else
          pDispInfo->hitFlag = FALSE;
    // 4 石化
    if (pActLoop->status == 4)
      pDispInfo->DrawEffect = 3;
    // 1 毒
    else if (pActLoop->status == 1)
      pDispInfo->DrawEffect = 4;
    else
      pDispInfo->DrawEffect = 0;
#ifdef _PETCOLOR_
    ...
    if (!pDispInfo->DrawEffect)
      if (ProcNo == PROC_BATTLE && !EncountFlag && !vsLookFlag &&
          !eventEnemyFlag && !DuelFlag && !NoHelpFlag) {
        if (pActLoop->actNo == 0) {
          if (pActLoop->level == 1 &&
...
void StockTaskDispBuffer2(int prio1, int prio2) {
  ...
  for (pActLoop = pActTop->pNext; pActLoop != pActBtm;
    ...
    if (pActLoop->atr & ACT_ATR_HIT_BOX)
      pDispInfo->hitFlag = 2;
    else if (pActLoop->atr & ACT_ATR_HIT)
      pDispInfo->hitFlag = 1;
    else
      pDispInfo->hitFlag = FALSE;
    // ?????????????
...
void StockBoxDispBuffer(int x1, int y1, int x2, int y2, UCHAR dispPrio,
  ...
  // ???????
  if (fill == 0)
    col = color | STOCK_DISP_BUFFER_BOX | STOCK_DISP_BUFFER_NO_BMP;
  // ???????
  else if (fill == 1)
    col = color | STOCK_DISP_BUFFER_BOX_FILL | STOCK_DISP_BUFFER_NO_BMP;
  // ?????
  else if (fill == 2)
    col = color | STOCK_DISP_BUFFER_LINE | STOCK_DISP_BUFFER_NO_BMP;
...
void DrawAni() {
  ...
  if (ProcNo != PROC_BATTLE) {
    ...
    if (fastDrawTile &&
        lpDraw->lpBACKBUFFER->BltFast(0, 0, lpBattleSurface, NULL,
    ...
    if (g_bUseAlpha) {
      if (fastDrawTile &&
          lpDraw->lpBACKBUFFERSYS->BltFast(
              0, 0, lpBattleSurfaceSys, NULL,
              DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY) == DDERR_SURFACEBUSY) {
        SurfaceBusyFlag = TRUE;
        return;
      }
...
void DrawGray(LPDIRECTDRAWSURFACE lpSurface, int ox, int oy, int offsetx,
  ...
  if (lpDraw->lpBACKBUFFER->Lock(NULL, &ddsdSource, DDLOCK_WAIT, NULL) != DD_OK)
    return;
  ...
  rect.right = ox + w;
  rect.bottom = oy + h;
  lpDraw->lpBACKBUFFER->BltFast(ox, oy, lpDraw->lpBACKBUFFER, &rect,
                                DDBLTFAST_WAIT);
  return;
}
      ...
      if (displayBpp == 32) {
        for (j = 0; j < h; j++) {
          for (i = 0; i < w; i++) {
            ...
            if (nColorOverlayer != DEF_COLORKEY) {
              SR = (nColorSource & 0xff0000) >>
                   8; // 原本要右移11左移3,简化成右移8
              SG = (nColorSource & 0x00ff00) >>
                   3; // 原本要右移5左移2,简化成右移3
              ...
              //                                SR = (((SR + OR) > 0x00ff ?
              //                                0x00ff:SR+OR) << 8) & 0xf800; SG
              //                                = (((SG + OG) > 0x00ff ?
              //                                0x00ff:SG+OG) << 3) & 0x07e0; SB
              //                                = (((SB + OB) > 0x00ff ?
              //                                0x00ff:SB+OB) >> 3) & 0x001f;
...
#endif
      {
        for (j = 0; j < h; j++) {
          for (i = 0; i < w; i++) {
            ...
            if (nColorOverlayer != DEF_COLORKEY) {
              SR =
                  (nColorSource & 0xf800) >> 8; // 原本要右移11左移3,简化成右移8
...
void DrawAlpha(LPDIRECTDRAWSURFACE lpSurface, int ox, int oy, int offsetx,
  ...
  if (displayBpp == 32) {
    ...
    ptOverLayerDest32 =
        (int *)(ddsdOverlayer.lpSurface) + dy * surfacePitch2 + dx;
  } else
#endif
  {
    surfacePitch1 = ddsdSource.lPitch >> 1;
    ...
    if (gBitRShift == 2) {
      ...
      if (displayBpp == 32) {
        for (j = 0; j < h; j++) {
          ...
          ptOverLayerDest32 =
              (DWORD *)ptOverLayerDest32 + (surfacePitch2 >> 2) - w;
          if (odd) {
...
void DrawAlphaChannel(SURFACE_INFO *surface_info, BYTE *AlphaData, int ox,
  ...
  if (lpDraw->lpBACKBUFFERSYS->Lock(NULL, &ddsdSource, DDLOCK_WAIT, NULL) !=
      DD_OK)
    return;
  if (surface_info->lpSurface->Lock(
          NULL, &ddsdOverlayer, DDLOCK_WAIT | DDLOCK_READONLY, NULL) != DD_OK) {
    ...
    if (displayBpp == 32) {
      for (j = 0; j < h; j++) {
        for (i = 0; i < w; i++) {
          ...
          if (nColorOverlayer != DEF_COLORKEY) {
            SR =
                (nColorSource & 0xff0000) >> 8; // 原本要右移11左移3,简化成右移8
...
void ablend_565(unsigned char *lpAlpha, unsigned int iAlpPitch,
                ...
                unsigned int iDstX, unsigned int iDstY, unsigned int iDstW,
...
// mmx ARGB混合
void DrawAlpha32(unsigned long *lpDst,    // 目标缓冲
  ...
  unsigned long *lpLinearDstBp = (iDstX) + (iDstY * iDstPitch / sizeof(DWORD)) +
                                 lpDst; // base pointer for linear destination
  unsigned long *lpLinearSrcBp = (iSrcX) + (iSrcY * iSrcPitch / sizeof(DWORD)) +
                                 lpSrc; // base pointer for linear source
...
MainLoop:
    ...
    pmullw mm3,mm1; // mm3=dst*(255-srcAlpha)  MM3= 00FF 0058 005C 0058  MM1=008F
                   // 008F FF8E FF8E   MM3=8E71 3128 D708 D8D0
    paddusw mm3,mm2; // mm3=src*srcAlpha+dst*(255-srcAlpha)  MM3=8E71 3128 D708
                    // D8D0    MM2=31E1 0000 0000 0388  MM3=C052 3128 D708 DC58
    psrlw mm3, 8; // mm3=src*srcAlpha/256+dst*(255-srcAlpha)/256   MM3=C052 3128
                // D708 DC58   MM3=00C0 0031 00D7 00DC
    packuswb mm3,mm7; // mm3=packed dst      MM3=00C0 0031 00D7 00DC     MM3=0000
                     // 0000 C031 D7DC
    movd [edi],mm3;
    jmp BeginPixel;
CopySrc:
