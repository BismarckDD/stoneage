...
BOOL InitDirectDraw(void) {
  ...
  if ((hResult = DirectDrawCreate(NULL, &lpDraw->lpDD, NULL)) != DD_OK) {
    hErrorLogFile =
        CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL,
                   CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if ((hResult = DirectDrawCreate((GUID *)DDCREATE_EMULATIONONLY,
                                    &lpDraw->lpDD, NULL)) != DD_OK) {
      MessageBoxNew(hWnd, "DirectDrawCreate Error", "确定",
                    MB_OK | MB_ICONSTOP);
      hErrorLogFile =
          CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL,
                     CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
      sprintf_s(szErrMsg, "DirectDrawCreate error(2):error result (%x)",
                hResult);
      WriteFile(hErrorLogFile, szErrMsg, sizeof(szErrMsg), &dwWriteByte, NULL);
      CloseHandle(hErrorLogFile);
      return FALSE;
  ...
  if ((hResult = lpDraw->lpDD->QueryInterface(
           IID_IDirectDraw2, (LPVOID *)&lpDraw->lpDD2)) != DD_OK) {
...
#undef _BACK_VERSION
  ...
  if (WindowMode) {
    ...
    if (displayBpp != (int)g_OriginalMode.dmBitsPerPel ||
        lpDraw->xSize != (int)g_OriginalMode.dmPelsWidth ||
        lpDraw->ySize != (int)g_OriginalMode.dmPelsHeight) {
      DEVMODE DevMode;
      int done = 0, iMode = 0;
      DevMode.dmSize = sizeof(DEVMODE);
      while (EnumDisplaySettings(NULL, iMode, &DevMode)) {
        if ((int)DevMode.dmBitsPerPel == displayBpp &&
            DevMode.dmPelsWidth == g_OriginalMode.dmPelsWidth &&
            DevMode.dmPelsHeight == g_OriginalMode.dmPelsHeight) {
          ChangeDisplaySettingsEx(NULL, &DevMode, NULL, CDS_UPDATEREGISTRY,
                                  NULL);
          done = 1;
          break;
        }
      ...
      if (done == 0) {
        ...
        while (EnumDisplaySettings(NULL, iMode, &DevMode)) {
          if ((int)DevMode.dmBitsPerPel == displayBpp &&
              (int)DevMode.dmPelsWidth >= lpDraw->xSize &&
              (int)DevMode.dmPelsHeight >= lpDraw->ySize) {
    ...
    if (lpDraw->lpDD2->SetCooperativeLevel(hWnd, DDSCL_NORMAL) != DD_OK) {
      MessageBoxNew(hWnd, "SetCooperativeLevel Error", "确定",
                    MB_OK | MB_ICONSTOP);
      return FALSE;
    }
    ...
    if ((hResult = lpDraw->lpDD2->CreateSurface(
             &lpDraw->ddsd, &lpDraw->lpFRONTBUFFER, NULL)) != DD_OK) {
      MessageBoxNew(hWnd, "主画面处理失败。", "确定", MB_OK | MB_ICONSTOP);
      hErrorLogFile =
          CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL,
                     CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
      sprintf_s(szErrMsg, "Create frontbuffer error(1):error result (%x)",
                hResult);
      WriteFile(hErrorLogFile, szErrMsg, sizeof(szErrMsg), &dwWriteByte, NULL);
      CloseHandle(hErrorLogFile);
      return FALSE;
    ...
    if ((hResult = lpDraw->lpDD2->CreateSurface(
             &lpDraw->ddsd, &lpDraw->lpBACKBUFFER, NULL)) != DD_OK) {
...
#ifdef _READ16BITBMP
    if (g_bUseAlpha) {
      lpDraw->ddsd.ddsCaps.dwCaps =
          DDSCAPS_OFFSCREENPLAIN | DDSCAPS_SYSTEMMEMORY;
      if ((hResult = lpDraw->lpDD2->CreateSurface(
               &lpDraw->ddsd, &lpDraw->lpBACKBUFFERSYS, NULL)) != DD_OK) {
        MessageBoxNew(hWnd, "暂存区处理失败(sys)", "确定", MB_OK | MB_ICONSTOP);
        hErrorLogFile =
            CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL,
                       CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        sprintf_s(szErrMsg, "Create backbuffer error:error result (%x)",
                  hResult);
        WriteFile(hErrorLogFile, szErrMsg, sizeof(szErrMsg), &dwWriteByte,
                  NULL);
        CloseHandle(hErrorLogFile);
        return FALSE;
      }
...
#define _BACK_VERSION
#endif
  {
    if (lpDraw->lpDD2->SetCooperativeLevel(hWnd,
    ...
    lpDraw->lpDD2->SetDisplayMode(lpDraw->xSize, lpDraw->ySize, displayBpp, 0,
                                  0);
    ZeroMemory(&lpDraw->ddsd, sizeof(lpDraw->ddsd));
    lpDraw->ddsd.dwSize = sizeof(lpDraw->ddsd);
    ...
    if ((hResult = lpDraw->lpDD2->CreateSurface(
      ...
      hErrorLogFile =
          CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL,
                     CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
      sprintf_s(szErrMsg, "Create frontbuffer error(2):error result (%x)",
                hResult);
      WriteFile(hErrorLogFile, szErrMsg, sizeof(szErrMsg), &dwWriteByte, NULL);
      CloseHandle(hErrorLogFile);
      return FALSE;
    ...
    if (g_bUseAlpha) {
      ...
      if ((hResult = lpDraw->lpDD2->CreateSurface(
               &lpDraw->ddsd, &lpDraw->lpBACKBUFFERSYS, NULL)) != DD_OK) {
        MessageBoxNew(hWnd, "暂存区处理失败二(sys)", "确定",
                      MB_OK | MB_ICONSTOP);
        hErrorLogFile =
            CreateFile("ErrorLog.txt", GENERIC_WRITE, FILE_SHARE_READ, NULL,
                       CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  ...
  if (displayBpp == 32) {
    ...
    rBitLShift = getBitCount(ddPixelFormat.dwBBitMask) +
                 getBitCount(ddPixelFormat.dwGBitMask); // ?????????
  ...
  if ((lpBattleSurface = CreateSurface(DEF_APPSIZEX, DEF_APPSIZEY, DEF_COLORKEY,
    ...
    if ((lpBattleSurface = CreateSurface(DEF_APPSIZEX, DEF_APPSIZEY,
        ...
        NULL) {
      MessageBoxNew(hWnd, "建立BattleSurface失败！", "确定",
                    MB_OK | MB_ICONSTOP);
      return FALSE;
    }
  }
#ifdef _READ16BITBMP
  if (g_bUseAlpha) {
    if ((lpBattleSurfaceSys =
             CreateSurface(DEF_APPSIZEX, DEF_APPSIZEY, DEF_COLORKEY,
                           DDSCAPS_SYSTEMMEMORY)) == NULL) {
      MessageBoxNew(hWnd, "建立BattleSurface(sys)失败！", "确定",
                    MB_OK | MB_ICONSTOP);
      return FALSE;
    }
  }
...
#ifdef _READ16BITBMP
  if (g_bUseAlpha)
    ClearSurface(lpBattleSurfaceSys);
#endif
...
BOOL InitPalette(void) {
  ...
  if (PalState.flag == FALSE) {
    ...
    if (fp == NULL) {
      MessageBoxNew(hWnd, "调色盘数据读取失败", "Error", MB_OK | MB_ICONSTOP);
      return FALSE;
    } else {
      // 可自由使用的调色盘设定.
      for (i = 16; i < 240; i++) {
  ...
  if (lpDraw->lpPALETTE == NULL) {
    MessageBoxNew(hWnd, "调色盘数据处理失败", "Error", MB_OK | MB_ICONSTOP);
    return FALSE;
  }
  // WON REM
#ifdef _HI_COLOR_16
  if (displayBpp == 8) {
    if (lpDraw->lpFRONTBUFFER->SetPalette(lpDraw->lpPALETTE) != DD_OK) {
      ...
      MessageBoxNew(hWnd, "请使用16bit高彩色或32bit高彩色模示", "Error", MB_OK);
      return FALSE;
    }
  }
#else
...
#ifdef SWITCH_MODE
  if (lpDraw->lpFRONTBUFFER->SetPalette(lpDraw->lpPALETTE) != DD_OK) {
    if (MessageBoxNew(hWnd, "请使用256色显示模示", "确定",
...
void PaletteProc(void) {
  ...
  if (palNoBak != PalState.palNo) {
    ...
    for (i = 16; i < 240; i++) {
      ...
      // ??
      dBlue[i] =
          (float)(pal[i].peBlue - Palette[i].peBlue) / (float)PalState.time;
      dGreen[i] =
          (float)(pal[i].peGreen - Palette[i].peGreen) / (float)PalState.time;
...
#endif
  if (WindowMode == TRUE) {
    lpDraw->lpFRONTBUFFER->Blt(&g_clientRect, lpDraw->lpBACKBUFFER, &g_moveRect,
                               DDBLT_WAIT, NULL);
  } else {
...
LPBITMAPINFO LoadDirectDrawBitmap(char *pFile) {
  ...
  //??????????
  if ((lpBmpInfo = (LPBITMAPINFO)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                                           BmpFileHeader.bfSize)) == NULL) {
...
// ?? ：????????????                                         //
// ?? ：short bxsize           : ?赶怐???巍纰)                       //
//        short bysize           : ?赶怐?c?(巍纰) // DWORD ColorKey :
//        ????(0?255)                //
// ??：? ... ?????????? /  ... NULL                 //
//---------------------------------------------------------------------------//
LPDIRECTDRAWSURFACE CreateSurface(short bxsize, short bysize, DWORD ColorKey,
                                  unsigned int VramOrSysram) {
  DDCOLORKEY ddck;
...
//---------------------------------------------------------------------------//
// ????????????? ? StretchDIBits ?? ?
void DrawBitmapToSurface(LPDIRECTDRAWSURFACE lpSurface, int offsetX,
                         int offsetY, LPBITMAPINFO pBmpInfo) {
  HDC hDcDest;
  // ??????????????????
  lpSurface->GetDC(&hDcDest);
  StretchDIBits(hDcDest, 0, 0,
                // cary
                pBmpInfo->bmiHeader.biWidth, pBmpInfo->bmiHeader.biHeight,
                // SurfaceSizeX, SurfaceSizeY,
                offsetX, offsetY, pBmpInfo->bmiHeader.biWidth,
...
// ????????????? ? memcpy ?? ?****************************/
#ifdef _READ16BITBMP
void DrawBitmapToSurface2(SURFACE_INFO *surface_info,
                          LPDIRECTDRAWSURFACE lpSurfaceSys, int offsetX,
    ...
    if (gResolutionMode == 1) {
      ...
      if (displayBpp == 32) {
        ...
        for (j = 0; j < sizeX; j++) {
          ...
          if (NewColor16Flg == 1) {
            short *pDest3 =
                (short *)((pSource - pRealBinBits) * 2 + pRealBinBits);
            if (pDest3[j] == DEF_COLORKEY) {
              ...
              pDest32[j] =
                  (((pDest3[j] & 0x001F) << 3) | ((pDest3[j] & 0x07E0) << 5) |
                   ((pDest3[j] & 0xF800) << 8) | 0xFF000000);
#ifdef _READ16BITBMP
              pDestSys32[j] =
                  (((pDest3[j] & 0x001F) << 3) | ((pDest3[j] & 0x07E0) << 5) |
                   ((pDest3[j] & 0xF800) << 8) | 0xFF000000);
            ...
            if ((pDest3[j] & 0xff000000) == DEF_COLORKEY ||
                ((pDest3[j] & 0xff) < 3 && ((pDest3[j] >> 16) & 0xff) < 3 &&
                 ((pDest3[j] >> 8) & 0xff) < 3)) {
              pDest32[j] = 0;
        ...
        for (j = 0; j < sizeX; j++) {
          ...
          if (NewColor16Flg == 1) {
            ...
            if ((pDest3[j] & 0xff000000) == DEF_COLORKEY ||
              ...
              pDest2[j] = ((pDest3[j] >> 8) & 0xF8) |
                          ((pDest3[j] >> 5) & 0x7E) | ((pDest3[j] >> 3) & 0x1F);
#ifdef _READ16BITBMP
              pDestSys[j] = ((pDest3[j] >> 8) & 0xF8) |
                            ((pDest3[j] >> 5) & 0x7E) |
                            ((pDest3[j] >> 3) & 0x1F);
#endif
            }
...
void DrawSurfaceFromPalette(SURFACE_INFO *surface_info) {
  ...
  if (lpSurface == NULL)
    return;
  // 如果不为256色补丁
  if (surface_info->colordepth > 0)
    return;
  // 如果调试板未改变并且不是切换调色板过程，那么就返回，使用缓存中的数据
  if (surface_info->palNo == PalState.palNo && PalState.time == 1)
    return;
  ...
  if (displayBpp == 32) {
    ...
    for (int i = 0; i < sizeY; i++) {
      for (int j = 0; j < sizeX; j++) {
        ...
        *(pDest + i * SurfaceSizeX + j) =
            (v == DEF_COLORKEY
                 ? 0
                 : ((highColor32Palette[v] == 0 ? 1 : highColor32Palette[v])));
      }
    }
  } else {
    ...
    for (int i = 0; i < sizeY; i++) {
      for (int j = 0; j < sizeX; j++) {
        ...
        *(pDest + i * SurfaceSizeX + j) =
            (v == DEF_COLORKEY
                 ? 0
                 : (highColorPalette[v] == 0 ? 1 : highColorPalette[v]));
      }
    }
  }
  if (lpSurface->Unlock(NULL) != DD_OK) {
...
void DrawBitmapToSurface2(SURFACE_INFO *surface_info, int offsetX, int offsetY,
                          int sizeX, int sizeY, LPBITMAPINFO pBmpInfo) {
#ifdef _CACHE_SURFACE_
  LPDIRECTDRAWSURFACE lpSurface = surface_info->lpSurface;
  DDSURFACEDESC ddsd;
  char *pDest;   // 目的地指标//
        ...
        if (NewColor16Flg) {
          ...
          for (j = 0; j < sizeX; j++) {
            ...
            if (NewColor16Flg == 1) { // rgb565
              ...
              int *pDest3 =
                  (int *)((pSource - pRealBinBits) * 4 + pRealBinBits);
...
#define COLOR_TO_MTK_COLOR_SIMUL(color)                                        \
  ((((color) >> 19) & 0x1f) << 11) | ((((color) >> 10) & 0x3f) << 5) |         \
      (((color) >> 3) & 0x1f)
              int *pDest3 =
                  (int *)((pSource - pRealBinBits) * 4 + pRealBinBits);
              if ((pDest3[j] & 0xFF000000) >> 24 == DEF_COLORKEY) {
...
void DrawBox(RECT *rect, unsigned char color, BOOL fill) {
  ...
  if (rect->left < 0)
    rect->left = 0;
  if (rect->right >= lpDraw->xSize)
    rect->right = lpDraw->xSize;
  if (rect->top < 0)
    rect->top = 0;
  if (rect->bottom >= lpDraw->ySize)
    rect->bottom = lpDraw->ySize;
...
#ifdef _READ16BITBMP
  if (g_bUseAlpha) {
    if (lpDraw->lpBACKBUFFERSYS->Lock(NULL, &ddsdsys, DDLOCK_WAIT, NULL) !=
        DD_OK) {
  ...
  if (displayBpp == 32) {
    ...
    if (g_bUseAlpha) {
      ...
      pDestSys32 = (int *)ddsdsys.lpSurface + rect->top * surfacePitchsys +
                   rect->left + 1;
    }
#endif
  } else
    ...
    surfacePitch = surfacePitch >> 1;
    pDest2 =
        (short *)ddsd.lpSurface + rect->top * surfacePitch + rect->left + 1;
#ifdef _READ16BITBMP
    if (g_bUseAlpha) {
      surfacePitchsys = surfacePitchsys >> 1;
      pDestSys = (short *)ddsdsys.lpSurface + rect->top * surfacePitchsys +
                 rect->left + 1;
    }
#endif
  } else {
    // 8 Bit Color
    pDest =
        (char *)(ddsd.lpSurface) + rect->top * surfacePitch + rect->left + 1;
  }
  // ???????
  if (fill == FALSE) {
#ifdef _HI_COLOR_32
    if (displayBpp == 32) {
      ...
      surfacePitch = surfacePitch >> 1;
      pDest32 =
          (int *)ddsd.lpSurface + rect->top * surfacePitch + rect->left + 1;
      // ?????????
      ...
      *(pDest32 + bottom - (surfacePitch + surfacePitch) + w - 4) = pixel; // ??
      ...
      if (g_bUseAlpha) {
        ...
        *(pDestSys32 + bottomSys - (surfacePitchsys + surfacePitchsys) + 1) =
            pixel;
        *(pDestSys32 + bottomSys - (surfacePitchsys + surfacePitchsys) + w -
          4) = pixel;
      ...
      *(pDest2 + bottom - (surfacePitch + surfacePitch) + w - 4) = pixel; // ??
      ...
      if (g_bUseAlpha) {
        ...
        *(pDestSys + bottomSys - (surfacePitchsys + surfacePitchsys) + 1) =
            pixel;
        *(pDestSys + bottomSys - (surfacePitchsys + surfacePitchsys) + w - 4) =
            pixel;
...
#ifdef _READ16BITBMP
  if (g_bUseAlpha)
    if (lpDraw->lpBACKBUFFERSYS->Unlock(NULL) != DD_OK)
      return;
#endif
...
void DrawAutoMapping(int x, int y, unsigned char *autoMap, int w, int h) {
  ...
  if (displayBpp == 8) {
    ...
    ptDest =
        tmpPtDest2 + (surfacePitch + 2) * hh / 2 - (surfacePitch - 2) * ww / 2;
    // ????????????????
    *(ptDest - 1) = pc_color;
    *(ptDest) = pc_color;
    *(ptDest + 1) = pc_color;
    ...
    *(int *)(ptDest - surfacePitch) = color;
    ...
    ptDest =
        tmpPtDest2 + (surfacePitch + 4) * hh / 2 - (surfacePitch - 4) * ww / 2;
...
int getAutoMapColor(unsigned int GraphicNo) {
  ...
  int i, j;
  // real.bin????????????????
  if (realGetImage(GraphicNo, (unsigned char **)&graBuf, &width, &height) ==
      FALSE)
    return 0;
  // ??????????
  for (i = 0; i < height; i++) {
  ...
  if (cnt == 0)
    return 0;
  // ??????????????????
  color = getNearestColorIndex(RGB(red / cnt, green / cnt, blue / cnt), Palette,
                               256);
...
int getNearestColorIndex(COLORREF color, PALETTEENTRY *palette, int entry) {
  ...
  int min_index;
  int i;
  mindist = (palette[0].peRed - GetRValue(color)) *
                (palette[0].peRed - GetRValue(color)) +
            (palette[0].peGreen - GetGValue(color)) *
                (palette[0].peGreen - GetGValue(color)) +
            (palette[0].peBlue - GetBValue(color)) *
                (palette[0].peBlue - GetBValue(color));
  min_index = 0;
  for (i = 16; i < entry - 16; i++) {
    distance = (palette[i].peRed - GetRValue(color)) *
                   (palette[i].peRed - GetRValue(color)) +
               (palette[i].peGreen - GetGValue(color)) *
                   (palette[i].peGreen - GetGValue(color)) +
               (palette[i].peBlue - GetBValue(color)) *
                   (palette[i].peBlue - GetBValue(color));
    if (distance < mindist) {
      min_index = i;
      mindist = distance;
...
#ifdef _HI_COLOR_32
              if (displayBpp == 32) {
                *(ptDest32 + surfacePitch * (i + buf->ey) + j + buf->ex) =
                    highColor32Palette[(unsigned char)color];
              } else
#endif
                  if (displayBpp == 16)
                *(ptDest2 + surfacePitch * (i + buf->ey) + j + buf->ex) =
                    highColorPalette[(unsigned char)color];
              else
...
#ifdef _READ16BITBMP
HRESULT DrawSurfaceFast(long bx, long by, LPDIRECTDRAWSURFACE lpSurface,
                        LPDIRECTDRAWSURFACE lpSurfaceSys) {
  long x0, y0;
  long w, h;
  ...
  if (bx >= lpDraw->xSize || bx + w <= 0 || by >= lpDraw->ySize ||
      by + h <= 0) {
    return DD_OK;
  }
  ...
  lpDraw->lpBACKBUFFERSYS->Blt(&rectD, lpSurfaceSys, &rect,
                               DDBLT_KEYSRC | DDBLT_WAIT, NULL);
  return lpDraw->lpBACKBUFFER->Blt(&rectD, lpSurface, &rect,
                                   DDBLT_KEYSRC | DDBLT_WAIT, NULL);
}
#endif
...
// ? ：DD_OK:? //
//---------------------------------------------------------------------------//
HRESULT DrawSurfaceFast2(short bx, short by, RECT *rect,
                         LPDIRECTDRAWSURFACE lpSurface) {
  short x0, y0;
  long w, h;
...
#ifdef _READ16BITBMP
      if (g_bUseAlpha)
        if (SurfaceInfoSys[i].lpSurface->IsLost())
...
#ifdef _READ16BITBMP
  if (g_bUseAlpha)
    lpDraw->lpBACKBUFFERSYS->GetDC(&hDcSys);
#endif
  SetBkMode(hDc, TRANSPARENT);
#ifdef _READ16BITBMP
  if (g_bUseAlpha)
    SetBkMode(hDcSys, TRANSPARENT);
#endif
  HFONT hOldFont = (HFONT)SelectObject(hDc, hFont);
#ifdef _READ16BITBMP
  if (g_bUseAlpha)
    SelectObject(hDcSys, hFont);
#endif
  SetTextColor(hDc, 0);
#ifdef _READ16BITBMP
  if (g_bUseAlpha)
    SetTextColor(hDcSys, 0);
#endif
  for (i = 0; i < FontCnt; i++) {
    if (FontBuffer[i].fontPrio == fontPrio) {
      if (gResolutionMode == 1) {
        TextOut(hDc, FontBuffer[i].x / 2 + 1, FontBuffer[i].y / 2 + 1,
                FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
      } else {
#ifdef _FONT_SIZE
        ...
        TextOut(hDc, FontBuffer[i].x + 1, FontBuffer[i].y + 1,
                FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#ifdef _READ16BITBMP
        if (g_bUseAlpha)
          TextOut(hDcSys, FontBuffer[i].x + 1, FontBuffer[i].y + 1,
                  FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#endif
#else
        if (FontBuffer[i].fontPrio != FONT_PRIO_CHATBUFFER) {
          ...
          while (char *ss = strstr(s, "/")) {
            ...
            TextOut(hDc, FontBuffer[i].x + 1 + x * 7, FontBuffer[i].y + 1, s,
                    len);
#ifdef _READ16BITBMP
            if (g_bUseAlpha) {
              TextOut(hDcSys, FontBuffer[i].x + x * 7, FontBuffer[i].y + 1, s,
                      len);
            }
#endif
            s = ss;
...
#ifdef _READ16BITBMP
              if (g_bUseAlpha)
                TextOut(hDcSys, FontBuffer[i].x + 1 + x * 7,
                        FontBuffer[i].y + 1, s, len + 2);
#endif
            } else {
              TextOut(hDc, FontBuffer[i].x + 1 + 22 + x * 7,
                      FontBuffer[i].y + 1, s, len + 2);
#ifdef _READ16BITBMP
              if (g_bUseAlpha)
                TextOut(hDcSys, FontBuffer[i].x + 1 + 22 + x * 7,
                        FontBuffer[i].y + 1, s, len + 2);
#endif
            }
            if (ss[0] >= 'a' && ss[0] <= 'z') {
...
#ifdef _READ16BITBMP
            if (g_bUseAlpha)
              TextOut(hDcSys, FontBuffer[i].x + 1 + x * 7, FontBuffer[i].y + 1,
                      s, strlen(s));
#endif
          } else {
            TextOut(hDc, FontBuffer[i].x + 1 + 22 + x * 7, FontBuffer[i].y + 1,
                    s, strlen(s));
#ifdef _READ16BITBMP
            if (g_bUseAlpha)
              TextOut(hDcSys, FontBuffer[i].x + 1 + 22 + x * 7,
                      FontBuffer[i].y + 1, s, strlen(s));
#endif
          }
#else
          ...
          if (false) {
            ...
            TextOut(hDc, FontBuffer[i].x + 1 + 22, FontBuffer[i].y + 1,
                    FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#ifdef _READ16BITBMP
            if (g_bUseAlpha)
              TextOut(hDcSys, FontBuffer[i].x + 1 + 22, FontBuffer[i].y + 1,
                      FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#endif
          }
#endif
...
#ifdef _READ16BITBMP
            if (g_bUseAlpha)
              SetTextColor(hDcSys, FontPal[color]);
#endif
            colorFlag = TRUE;
          }
          //???????????????
...
#ifdef _READ16BITBMP
            if (g_bUseAlpha)
              TextOut(hDcSys, FontBuffer[i].x, FontBuffer[i].y,
                      FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#endif
#else
            if (FontBuffer[i].fontPrio != FONT_PRIO_CHATBUFFER) {
...
#ifdef _READ16BITBMP
                if (g_bUseAlpha) {
                  TextOut(hDcSys, FontBuffer[i].x + x * 7, FontBuffer[i].y, s,
                          len);
                }
#endif
              ...
              TextOut(hDc, FontBuffer[i].x + x * 7, FontBuffer[i].y, s,
                      strlen(s));
#ifdef _READ16BITBMP
              if (g_bUseAlpha)
                TextOut(hDcSys, FontBuffer[i].x + x * 7, FontBuffer[i].y, s,
              ...
              while (char *ss = strstr(s, "/")) {
                ...
                if (false) {
                  ...
                  TextOut(hDc, FontBuffer[i].x + 22 + x * 7, FontBuffer[i].y, s,
                          len + 2);
#ifdef _READ16BITBMP
                  if (g_bUseAlpha)
                    TextOut(hDcSys, FontBuffer[i].x + 22 + x * 7,
                            FontBuffer[i].y, s, len + 2);
#endif
                }
...
#ifdef _READ16BITBMP
                if (g_bUseAlpha)
                  TextOut(hDcSys, FontBuffer[i].x + 22 + x * 7, FontBuffer[i].y,
                          s, strlen(s));
#endif
              }
              SetTextColor(hDc, FontPal[color]);
              ...
              if (false) {
                TextOut(hDc, FontBuffer[i].x, FontBuffer[i].y,
                        FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#ifdef _READ16BITBMP
                if (g_bUseAlpha)
                  TextOut(hDcSys, FontBuffer[i].x, FontBuffer[i].y,
                ...
                TextOut(hDc, FontBuffer[i].x + 22, FontBuffer[i].y,
                        FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#ifdef _READ16BITBMP
                if (g_bUseAlpha)
                  TextOut(hDcSys, FontBuffer[i].x + 22, FontBuffer[i].y,
                          FontBuffer[i].str, (int)strlen(FontBuffer[i].str));
#endif
              }
#endif
...
void snapShot(void) {
  ...
  if (displayBpp == 16) {
    ...
    saveBmpFile(fileName, (BYTE *)ddsdDesc.lpSurface, clientPoint.x,
                clientPoint.y, w, h, ddsdDesc.lPitch, rgbpal, 256);
...
// ???????????
BOOL saveBmpFile(const char *filename, BYTE *buf, int x, int y, int width,
                 int height, int srcpitch, RGBQUAD *rgbpal, int colorCnt) {
  HANDLE fh;
  ...
  WriteFile(fh, (void *)(&infoheader), sizeof(BITMAPINFOHEADER), &writtensize,
            NULL);
  WriteFile(fh, (void *)rgbpal, sizeof(RGBQUAD) * (colorCnt), &writtensize,
            NULL);

  for (i = 0; i < height; i++) {
    WriteFile(fh, (void *)(buf + srcpitch * (height + y - 1 - i) + x), width,
              &writtensize, NULL);
...
#ifdef _READ16BITBMP
void Draw16BitmapToSurface2(SURFACE_INFO *surface_info,
                            LPDIRECTDRAWSURFACE lpSurfaceSys, int offsetX,
                            int offsetY, int sizeX, int sizeY,
                            LPBITMAPINFO pBmpInfo) {
  DDSURFACEDESC ddsd;
  ...
  if (surface_info->lpSurface == NULL)
    return;
  if (g_bUseAlpha && lpSurfaceSys == NULL)
    return;
  ...
  if (FAILED(surface_info->lpSurface->Lock(NULL, &ddsd, DDLOCK_WAIT, NULL)))
    return;
  if (g_bUseAlpha) {
  ...
  if (displayBpp == 32) {
    ...
    for (i = 0; i < sizeY; i++) {
      ...
      if (g_bUseAlpha)
        pDestSys32 += surfacePitchSys32 - sizeX;
    }
  } else
#endif
  {
    ...
    if (g_bUseAlpha)
      pDestSys = (WORD *)(ddsdsys.lpSurface);
    pSource = pRealBinBits + offsetY * RealBinWidth + offsetX;
    surfacePitch = ddsd.lPitch >> 1;
    if (g_bUseAlpha)
      surfacePitchSys = ddsdsys.lPitch >> 1;
    ...
    for (i = 0; i < sizeY; i++) {
      ...
      pSource += RealBinWidth - sizeX;
      if (g_bUseAlpha)
        pDestSys += surfacePitchSys - sizeX;
    }
  }
