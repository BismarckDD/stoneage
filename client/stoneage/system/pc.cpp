...
template <typename T> inline void swap(T &T1, T &T2) {
  T TT;
  TT = T1;
  T1 = T2;
  T2 = TT;
...
void swap(char *T1, char *T2) {
  char tmp[256];
  if (strlen(T1) > 255 || strlen(T2) > 255)
    return;
  strcpy(tmp, T1);
  strcpy(T1, T2);
  strcpy(T2, tmp);
...
//    setPcParam(pc.name, pc.freeName, pc.level, pc.ridePetName,
//    pc.ridePetLevel, pc.nameColor, walk, height, pc.profession_class,
//    pc.profession_level, pc.profession_exp, pc.profession_skill_point,
//    pc.ptAct->gm_name); setPcParam(pc.name, pc.freeName, pc.level,
//    pc.ridePetName, pc.ridePetLevel, pc.nameColor, walk, height,
//    pc.profession_class, pc.profession_level, pc.profession_skill_point,
//    pc.ptAct->gm_name); #else setPcParam(pc.name, pc.freeName, pc.level,
//    pc.ridePetName, pc.ridePetLevel, pc.nameColor, walk, height,
//    pc.profession_class, pc.profession_level, pc.profession_exp,
//    pc.profession_skill_point);
#ifdef _ALLDOMAN // (不可开) Syu ADD 排行榜NPC
  setPcParam(pc.name, pc.freeName, pc.level, pc.ridePetName, pc.ridePetLevel,
             pc.nameColor, walk, height, pc.profession_class,
             pc.profession_level, pc.profession_skill_point, pc.herofloor);
#else
  setPcParam(pc.name, pc.freeName, pc.level, pc.ridePetName, pc.ridePetLevel,
             pc.nameColor, walk, height, pc.profession_class,
             pc.profession_level, pc.profession_skill_point);
#endif
//    #endif
#else
  setPcParam(pc.name, pc.freeName, pc.level, pc.ridePetName, pc.ridePetLevel,
             pc.nameColor, walk, height);
#endif
...
void setPcEmotion(int emotion) {
  ...
  ext->ptActEmotion = createCommmonEffectAction(
      emotion, pc.ptAct->gx, pc.ptAct->gy, 0, 0, DISP_PRIO_CHAR);
}
#endif
...
#ifdef _CHAR_PROFESSION // WON ADD 人物职业
//    #ifdef _GM_IDENTIFY        // Rog ADD GM识别
//  void setPcParam(char *name, char *freeName, int level, char *petname, int
//  petlevel, int nameColor, int walk, int height, int profession_class, int
//  profession_level, int profession_exp, int profession_skill_point , char
//  *gm_name)
//    void setPcParam(char *name, char *freeName, int level, char *petname, int
//    petlevel, int nameColor, int walk, int height, int profession_class, int
//    profession_level, int profession_skill_point , char *gm_name) #else void
//    setPcParam(char *name, char *freeName, int level, char *petname, int
//    petlevel, int nameColor, int walk, int height, int profession_class, int
//    profession_level, int profession_exp, int profession_skill_point)
#ifdef _ALLDOMAN // (不可开) Syu ADD 排行榜NPC
void setPcParam(char *name, char *freeName, int level, char *petname,
                int petlevel, int nameColor, int walk, int height,
                int profession_class, int profession_level,
                int profession_skill_point, int herofloor)
#else
void setPcParam(char *name, char *freeName, int level, char *petname,
                int petlevel, int nameColor, int walk, int height,
                int profession_class, int profession_level,
...
// PC???????????????
void setPcParty(void) { pc.status |= CHR_STATUS_PARTY; }

// PC?????????????????
void delPcParty(void) { pc.status &= (~CHR_STATUS_PARTY); }

// PC???????????
void setPcWatch(void) { pc.status |= CHR_STATUS_WATCH; }

// PC???????????????
void delPcWatch(void) { pc.status &= (~CHR_STATUS_WATCH); }
...
// ????????
void setPcUseMagic(void) { pc.status |= CHR_STATUS_USE_MAGIC; }

// ????????
void delPcUseMagic(void) { pc.status &= (~CHR_STATUS_USE_MAGIC); }
...
void delPcAngel(void) { pc.status &= (~CHR_STATUS_ANGEL); }
#endif
...
// PC????? /////////////////////////////////////////////////////////
void changePcAct(int x, int y, int dir, int action, int effectno,
                 int effectparam1, int effectparam2) {
  switch (action) {
  // ??
  case 0:
#if 0
  ...
  case 60: {
    ...
    // changePcAct(int x, int y, int dir, int action,int effectno, int
    // effectparam1, int effectparam2)
...
#endif
  {
    ...
    if (party[i].useFlag != 0) {
      if (party[i].id == pc.id) {
        ...
        // ???????????????????????
        if ((pc.status & CHR_STATUS_LEADER) != 0 && party[i].ptAct != NULL) {
          party[i].ptAct->bufCount = 0;
        }
...
BOOL TalkToNPC(void) {
  ...
  if (nowVx != 0 || nowVy != 0)
    return FALSE;
  if (ABS(nowGx - mouseMapGx) > 2 || ABS(nowGy - mouseMapGy) > 2)
    return FALSE;
...
void addressBookProc(void) {
  ...
  // ??????????
  if (addressBookFlag && ptActAddressBookWin == NULL) {
    // ??????????????
    ptActAddressBookWin =
        MakeWindowDisp(addressBookX, addressBookY, 4, 8, NULL, 1);
    // ??????????
  ...
  // ??????????
  else if (!addressBookFlag && ptActAddressBookWin != NULL) {
...
void addressBookWindow1(void) {
  ...
  if (ptActAddressBookWin->hp >= 1) {
    ...
    addressBookSelectStrItem[0] =
        StockFontBuffer(addressBookX + 56, addressBookY + 352, FONT_PRIO_FRONT,
                        FONT_PAL_YELLOW, "关闭", 2);
  }
}
