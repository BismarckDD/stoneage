...
int util_mkstring(char *buffer, char *value) {
  ...
  strcat_s(buffer, 16384,
           ";"); // It's important to append a SEPARATOR between fields
  ...
  if (stemp) {
    if (stemp != str) {
      ...
      if (pos == 0)
        ...
        wm[index].style[pos].x = getTextLength(wm[index].style[pos - 1].str) +
                                 wm[index].style[pos - 1].x;
...
void PutWinText(int x, int y, char fontPrio, int color, char *str, BOOL hitFlag,
  ...
  for (; i < 30; i++) {
    if (*wmstr[index].style[i].str)
      StockFontBufferExt(x + wmstr[index].style[i].x, y, fontPrio,
                         wmstr[index].style[i].color, wmstr[index].style[i].str,
                         hitFlag, wmstr[index].style[i].size);
  }
}
  ...
  if (stemp) {
    if (stemp != str) {
      ...
      if (pos == 0)
        ...
        wm->style[pos].x =
            getTextLength(wm->style[pos - 1].str) + wm->style[pos - 1].x;
