...
int SaDispatchMessage(int fd, char *encoded) {
  ...
  if (util_GetFunctionFromSlice(&func, &fieldcount)) {
    if (func == LSSPROTO_XYD_RECV) {
...
#ifdef _STONDEBUG__PACKET_
      sprintf_s(datalog, "objindex=%d graphicsno=%d x=%d y=%d dir=%d flg=%d no=%d cdata=%s",
...
#ifdef _STONDEBUG__PACKET_
      sprintf_s(datalog,
                "fl=%d x1=%d y1=%d x2=%d y2=%d tilesum=%d objsum=%d "
                "eventsum=%d data=%s",
...
#ifdef _STONDEBUG__PACKET_
      char datalog[26384];
      sprintf_s(datalog, "fl=%d x1=%d y1=%d x2=%d y2=%d data=%s", fl, x1, y1,
                x2, y2, data);
      PackageLog(func, datalog);
#endif
      ...
      sprintf_s(datalog, "category=%d dx=%d dy=%d data=%s", category, dx, dy,
                data);
      PackageLog(func, datalog);
#endif
      ...
      sprintf_s(datalog, "result=%d havepetindex=%d havepetskill=%d toindex=%d",
      ...
      char datalog[10420];
      sprintf_s(datalog,
                "windowtype=%d buttontype=%d seqno=%d objindex=%d data=%s",
      ...
      extern void RedMemoySetup(int type, int time, int vip, char *name,
                                int index);
      ...
      sprintf_s(datalog, "iCharaindex=%d iType=%d iActionNum=%d", iCharaindex,
                iType, iActionNum);
      PackageLog(func, datalog);
#endif
      ...
      sprintf_s(datalog, "position=%d damage=%d offsetx=%d offsety=%d",
                position, damage, offsetx, offsety);
      PackageLog(func, datalog);
#endif
...
void lssproto_PMSG_send(int fd, int index, int petindex, int itemindex,
...
void lssproto_TK_send(int fd, int x, int y, char *message, int color,
                      int area) {
  char buffer[16384];
  int iChecksum = 0;
...
void lssproto_PS_send(int fd, int havepetindex, int havepetskill, int toindex,
                      char *data) {
  char buffer[16384];
  int iChecksum = 0;
...
void lssproto_WN_send(int fd, int x, int y, int seqno, int objindex, int select,
                      char *data) {
  char buffer[16384];
  int iChecksum = 0;
...
void lssproto_ClientLogin_send(int fd, char *cdkey, char *passwd, char *mac,
                               int selectServerIndex, char *ip)
#else
...
void lssproto_CreateNewChar_send(int fd, int dataplacenum, char *charname,
...
// shan 2002/01/10
void lssproto_PETST_send(int fd, int nPet,
                         int sPet) // sPet  0:休息 1:等待 4:邮件
{
  // cary
  if (dwServer == GS) {
    char buffer[16384];
...
#ifdef _PET_ITEM
// 宠物装备功能
void lssproto_PetItemEquip_send(int fd, int iGx, int iGy, int iPetNo,
                                int iItemNo, int iDestNO) {
  char szBuffer[16384];
  int iChecksum = 0;
...
void lssproto_HostName_send(int fd) {
  ...
  char MACNUM[16] = "";
  sprintf_s(MACNUM, "%2X%2X%2X%2X%2X%2X", addrlist[0].addr[0],
            addrlist[0].addr[1], addrlist[0].addr[2], addrlist[0].addr[3],
            addrlist[0].addr[4], addrlist[0].addr[5]);
