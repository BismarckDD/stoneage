...
主要内容如下：
容量和长度限制
定义角色名、道具名、宠物数量、背包容量、魔法数量、队伍人数和金币上限等，例如 MAX_PET、MAX_ITEM、MAX_PARTY。
装备槽和物品类别
定义头、身体、手臂、腰带、盾、鞋、宠物装备等位置。
功能状态位
PC_ETCFLAG_* 使用一个整数的不同 bit 保存组队、决斗、交易、聊天频道和自动战斗等开关。
核心数据结构
ITEM：道具名称、图号、说明、使用范围、目标类型等
PC：当前玩家的完整客户端状态
PET：宠物属性、技能和装备
MAGIC：人物魔法
PARTY：队伍成员
ADDRESS_BOOK：名片/好友资料
PET_SKILL：宠物技能
PROFESSION_SKILL：职业技能
CHARLISTTABLE：登录界面的角色列表
*/
...
enum { MAGIC_FIELD_ALL, MAGIC_FIELD_BATTLE, MAGIC_FIELD_MAP };
...
enum { PETSKILL_FIELD_ALL, PETSKILL_FIELD_BATTLE, PETSKILL_FIELD_MAP };
...
EXTERN MAGIC magic[MAX_MAGIC];
EXTERN PET pet[MAX_PET];
EXTERN PARTY party[MAX_PARTY];
EXTERN ADDRESS_BOOK addressBook[MAX_ADR_BOOK];
#ifdef __PC_CPP__
EXTERN short partyModeFlag = 0;
EXTERN BOOL addressBookFlag = FALSE;
#else
EXTERN short partyModeFlag;
EXTERN BOOL addressBookFlag;
#endif
EXTERN BATTLE_RESULT_MSG battleResultMsg;
EXTERN PET_SKILL petSkill[MAX_PET][MAX_SKILL];
EXTERN CHARLISTTABLE chartable[MAX_CHARACTER];
EXTERN char newCharacterName[CHAR_NAME_LEN + 1];
...
//    #ifdef _GM_IDENTIFY        // Rog ADD GM识别
//      void setPcParam( char *, char *, int, char *, int, int, int, int, int,
//      int, int, int, char *);
//        void setPcParam( char *, char *, int, char *, int, int, int, int, int,
//        int, int, char *);
//    #else
