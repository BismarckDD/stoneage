...
// for AnalyzeMessage()
enum { ZERO, READHEAD, READLEN, READPARAM, GETHEAD };
  ...
  while (iStrLen-- > 0) {
    if (backslash) {
      ...
      if (';' == (ch = *lpstr++)) {
        ...
        szAnalyzeBuf[indexBuf++] = '\0';
        if ((paramNew = new Parameter) &&
            (paramNew->lpstrParam = new char[indexBuf + 1])) {
          paramNew->next = NULL;
...
DWORD GetCafeNumber() {
  ...
  DWORD Id = 0;
#ifdef _NEW_EARTH
  if (hFileMapping = OpenFileMapping(FILE_MAP_READ | FILE_MAP_WRITE, FALSE,
                                     "vup2u4fu.6")) {
    int *pFlag;

    if (pFlag = (int *)MapViewOfFile(hFileMapping,
  ...
  char szTemp[64];
  if (hFileMapping =
          OpenFileMapping(FILE_MAP_READ | FILE_MAP_WRITE, FALSE, "f; enc ")) {
    if (lpstr = (LPSTR)MapViewOfFile(hFileMapping,
                                     FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0)) {
      strcpy(szTemp, lpstr);
