...
const int ANSI950 = 950;

int MessageBoxNew(HWND hWnd, LPCSTR lpText, LPCSTR lpCaption, UINT uType)
{
  if (ansi_encoding == ANSI950) {
    char tc_text[1024] = { 0 };  // traditional chinese text
    char tc_capt[1024] = { 0 };  // traditional chinese caption
    LCMapString(0x804, 0x4000000, lpText, strlen(lpText), tc_text, 1024);
    LCMapString(0x804, 0x4000000, lpCaption, strlen(lpCaption), tc_capt, 1024);
    return MessageBox(hWnd, GB2312ToBIG5((const char *)tc_text), GB2312ToBIG5((const char *)tc_capt), uType);
  } else {
...
// ?? ：???????                                                 //
//---------------------------------------------------------------------------//
HINSTANCE hInst;            // handle_instance
HWND hWnd;                  // ?????????
int   CmdShow;              // WinMain??????????????
