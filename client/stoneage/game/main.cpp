...
void 获取机器码() {
  ...
  if (m_hMapFile == NULL) {
    ...
    char *str = (char *)MapViewOfFile( // 映射到本进程的地址空间
        m_hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, 0);
    memset(str, 0, 1032);
...
BOOL DetectKey(char *path) {
  return (IsFileExist(path, "QMScript") || IsFileExist(path, "Recorder.exe") ||
          IsFileExist(path, "QMColorActionCtl.ocx") ||
          IsFileExist(path, "ShieldModule.dat") ||
          IsFileExist(path, "qmacro.ini") || IsFileExist(path, "wqm.exe") ||
          IsFileExist(path, "Fairy_Ape") ||
          IsFileExist(path, "LAScriptX.dll") || IsFileExist(path, "mly.dll") ||
          IsFileExist(path, "v5_hook.dll") || IsFileExist(path, "v5_Log.dll") ||
          IsFileExist(path, "v5_Process_Manager.dll"))
             ? TRUE
             : FALSE;
}
...
void KeyDetection() {
  ...
  for (int i = 0; i < (int)(cbNeeded / sizeof(DWORD)); i++) {
    ...
    if (bret) {
      DWORD dwret = GetModuleFileNameEx(hProcess, hMods[0], szProcessName,
...
BOOL IsContainsProcess(char *strProName, BOOL 判断 = 1) {
  ...
  HANDLE hProcessSnap =
      ::CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0); // 创建快照
...
DWORD __forceinline IsInsideVPC_exceptionFilter(LPEXCEPTION_POINTERS ep) {
  ...
  return EXCEPTION_CONTINUE_EXECUTION; // we can safely resume execution since
                                       // we skipped faulty instruction
}
...
void CreateCompatibleDEVMODE(DEVMODE *pdm, DWORD BitsPerPixel, DWORD Width,
...
// WIN32 桌面程序入口
// hInstance: 当前实例句柄
// hPrevInstance: 旧实例句柄，Win32中始终为NULL
// lpCmdLine: 命令行参数
// nCmdShow: 窗口显示方式
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (displayBpp != 32) {
    DEVMODE dm;
    CreateCompatibleDEVMODE(&dm, 32, GetDeviceCaps(hDcDest, HORZRES),
                            GetDeviceCaps(hDcDest, VERTRES),
                            GetDeviceCaps(hDcDest, VREFRESH));
    ...
    CreateProcess(buf, " OpenClient", NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS,
  ...
  // 必须先确定逻辑分辨率，再创建/调整窗口；后续 DirectDraw
  // 初始化会使用这里的尺寸。
  ...
  if (g_OriginalMode.dmSize != 0) {
    ...
    if (DevMode.dmPelsWidth != g_OriginalMode.dmPelsWidth ||
        DevMode.dmPelsHeight != g_OriginalMode.dmPelsHeight ||
        DevMode.dmBitsPerPel != g_OriginalMode.dmBitsPerPel) {
      g_OriginalMode.dmFields = DM_BITSPERPEL | DM_PELSWIDTH | DM_PELSHEIGHT;
      ChangeDisplaySettingsEx(NULL, &g_OriginalMode, NULL, CDS_UPDATEREGISTRY,
                              NULL);
    }
  }
#endif
...
void AnalyzeCmdLine(void) {
  ...
  if (addr = strstr(CmdLine, "spradrnbin:"))
    sprintf_s(sprAdrnBinName, "data/spradrn_%d.bin",
              atoi(addr + strlen("spradrnbin:")));
#ifdef _READ16BITBMP
  if (addr = strstr(CmdLine, "realtrue:"))
    sprintf_s(realtrueBinName, "data/realtrue_%d.bin",
              atoi(addr + strlen("realtrue:")));
  if (addr = strstr(CmdLine, "adrntrue:"))
    sprintf_s(adrntrueBinName, "data/adrntrue_%d.bin",
              atoi(addr + strlen("adrntrue:")));
...
void ChangeWindowMode(void) {
  ...
  if (hWnd == NULL) {
    hWnd = CreateWindowEx(NULL, DEF_APPNAME, DEF_APPNAME, windowStyle,
                          // CW_USEDEFAULT,
                          // CW_USEDEFAULT,
    ...
    // ?????????????
    SetWindowPos(hWnd, HWND_NOTOPMOST, 0, 0, clientRect.right - clientRect.left,
...
#endif
LRESULT CALLBACK WindMsgProc(HWND hWnd, UINT Message, WPARAM wParam,
                             LPARAM lParam) {
  switch (Message) {
#ifdef _REMAKE_20
  case WM_TIMER:
  ...
  case WM_SYSKEYDOWN: // ALT????????
    ...
    if (SurfaceBusyFlag == TRUE) {
      SurfaceBusyFlag = FALSE;
      MessageBoxNew(hWnd, "SurfaceBusyFlag error!", "确定",
                    MB_OK | MB_ICONSTOP);
      RecoverDirectDraw();
    }
    switch (wParam) {
  ...
  case UM_KEYEVENT:
    ...
    if (ToAscii(wParam, (HIWORD(lParam) & (0x00FF | KF_UP)), bKeyState,
                (PWORD)ac, 0) == 1) {
      extern int HOOK_PASSWD_NUM;
      if ((HOOK_PASSWD_NUM < 12) && (wParam != 13) && (wParam != 9) &&
          (wParam != 27))
        HOOK_PASSWD_NUM++;
    }
    if (wParam == 8)
...
void SetgResolutionMode(const int mode) {
  ...
  lpDraw = (DIRECT_DRAW *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                                    (DWORD)sizeof(DIRECT_DRAW));
  if (lpDraw == NULL) {
    MessageBoxNew(hWnd, "HeapAlloc Error ( DIRECT_DRAW )", "确定",
                  MB_OK | MB_ICONSTOP);
    return;
  }
  switch (mode) {
