...
// Global vars
extern int g_iRunEarthQuake; // 地震的状况: 0 --> 没地震, 1 --> 初始化地震, 2
                             // --> 地震中
...
BOOL ReadBattleMap(const int battleMapNo) {
  ...
  if (battleMapNo >= BATTLE_MAP_FILES)
    BattleMapNo = 0;
  else 
  ...
  if (((nowFloor == 817) || (nowFloor == 8007) || (nowFloor == 8101) ||
       ...
       (nowFloor == 8029) || (nowFloor == 8015) || (nowFloor == 151) ||
       (nowFloor == 160) || (nowFloor == 161) ||
       (nowFloor >= 30022 && nowFloor <= 30025) || nowFloor == 8113 ||
       nowFloor == 8114
#ifdef _8BATTLEMAP
       || (nowFloor >= 78 && nowFloor <= 99) || nowFloor == 8107 ||
       (nowFloor >= 8108 && nowFloor <= 8112) ||
       (nowFloor >= 8116 && nowFloor <= 8120) ||
       (nowFloor >= 8124 && nowFloor <= 8126) || nowFloor == 7500 ||
       nowFloor == 7530 || nowFloor == 7600
#endif
       ) &&
      (RandBattleBg == 0)) {
    ...
    if (RandBattleBg == 0)
      RandBattleBg = 29001;
    else if (RandBattleBg == 1)
      RandBattleBg = 29020;
    else if (RandBattleBg == 2)
      RandBattleBg = 29019;
    else if (RandBattleBg == 3)
      RandBattleBg = 29024;
    else if (RandBattleBg == 4)
      RandBattleBg = 29025;

    RandBattleBg = RAND(29047, 29049);
    // 水晶地表战斗地图
    if (nowFloor == 30022)
      RandBattleBg = iCrystalBattleMap[0][RAND(0, 2)];
    else if (nowFloor == 30023)
      RandBattleBg = iCrystalBattleMap[1][RAND(0, 2)];
    else if (nowFloor == 30024)
      RandBattleBg = iCrystalBattleMap[2][RAND(0, 2)];
    else if (nowFloor == 30025)
      RandBattleBg = iCrystalBattleMap[3][RAND(0, 2)];
#ifdef _8BATTLEMAP
    else if ((nowFloor >= 78 && nowFloor <= 99) || nowFloor == 8107)
      RandBattleBg = 29074 + RAND(0, 2);
    else if ((nowFloor >= 8108 && nowFloor <= 8112) ||
             (nowFloor >= 8116 && nowFloor <= 8120) ||
             (nowFloor >= 8124 && nowFloor <= 8126) || nowFloor == 7500 ||
             nowFloor == 7530 || nowFloor == 7600)
      RandBattleBg = 29070 + RAND(0, 3);
#endif
    StockDispBuffer(400, 300, DISP_PRIO_TILE, RandBattleBg, 0);
  } else if (((nowFloor == 817) || (nowFloor == 8007) || (nowFloor == 8101) ||
              (nowFloor == 8100) || (nowFloor == 8027) || (nowFloor == 8028) ||
              (nowFloor == 8029) || (nowFloor == 8015) || (nowFloor == 151) ||
              (nowFloor == 160) || (nowFloor == 161) ||
              (nowFloor >= 30022 && nowFloor <= 30025) || nowFloor == 8113 ||
...
// 特殊地图编号或地震中
  ...
  if ((BattleMapNo >= 148 && BattleMapNo <= 150) || 2 == g_iRunEarthQuake ||
      nowFloor == 817 || nowFloor == 8007 || nowFloor == 8101 ||
      nowFloor == 8100 || nowFloor == 8027 || nowFloor == 8028 ||
      nowFloor == 8029 || nowFloor == 8015 || nowFloor == 8113 ||
      nowFloor == 8114) {
#else
  if ((BattleMapNo >= 148 && BattleMapNo <= 150) || 2 == g_iRunEarthQuake) {
#endif
  ...
  case 0:
    ...
    lpDraw->lpd3dDevice->StretchRect(lpBattleSurface, &rect,
                                     lpDraw->lpOffScrSurface, &destrect,
                                     D3DTEXF_NONE);
#else
    lpDraw->lpBACKBUFFER->BltFast(x0, y0, lpBattleSurface, &rect,
                                  DDBLTFAST_WAIT);
#endif
#ifdef _READ16BITBMP
    if (g_bUseAlpha)
      lpDraw->lpBACKBUFFERSYS->BltFast(x0, y0, lpBattleSurfaceSys, &rect,
                                       DDBLTFAST_WAIT);
#endif
    break;
  case 1:
    ...
    for (d7 = 0; d7 < 480 + DISPLACEMENT_Y; d7 += RASTER_CLEARANCE) {
      ...
      lpDraw->lpd3dDevice->StretchRect(lpBattleSurface, &rect,
                                       ...
                                       D3DTEXF_NONE);
#else
      lpDraw->lpBACKBUFFER->BltFast(0, d7, lpBattleSurface, &rect,
                                    DDBLTFAST_WAIT);
#endif
#ifdef _READ16BITBMP
      if (g_bUseAlpha)
        lpDraw->lpBACKBUFFERSYS->BltFast(0, d7, lpBattleSurfaceSys, &rect,
                                         DDBLTFAST_WAIT);
#endif
      d6++;
    }
  ...
  case 2:
    ...
    for (d7 = 0; d7 < 480 + DISPLACEMENT_Y; d7 += RASTER_CLEARANCE / 2) {
      ...
      lpDraw->lpd3dDevice->StretchRect(lpBattleSurface, &rect,
                                       ...
                                       D3DTEXF_NONE);
#else
      lpDraw->lpBACKBUFFER->BltFast(d0, d7, lpBattleSurface, &rect,
                                    DDBLTFAST_WAIT);
#endif
#ifdef _READ16BITBMP
      if (g_bUseAlpha)
        lpDraw->lpBACKBUFFERSYS->BltFast(d0, d7, lpBattleSurfaceSys, &rect,
                                         DDBLTFAST_WAIT);
#endif
      d6++;
    }
