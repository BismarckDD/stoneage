...
void InitBattleMenu(void) {
  int i;
  // ???
  for (i = 0; i < BATTLE_BUTTON_SUU; i++)
    battleButtonDispNo[i] = -2;
  // 
  for (i = 0; i < BATTLE_MENU_JUJUTSU_FONTS; i++)
    battleMenuJujutsuFontNo[i] = -2;
  // ????
  for (i = 0; i < BATTLE_MENU_ITEM_FONTS; i++)
    battleMenuItemFontNo[i] = -2;
  // ???
...
int CheckBattle1P2P(void) {
  ...
  for (i = 0; i < BATTLKPKPLYAERNUM; i++) {
    ...
    if (p_party[i]->func == NULL)
      continue;
    // ???????
    if (((ATR_EQU *)p_party[i]->pYobi)->pet_ok == TRUE)
      cnt++;
  }
  if (cnt >= 2)
    return 2;
...
void HpMeterDisp(int no) {
  ...
  int graNo;
  // ??????????????
  if (p_party[no]->func == NULL || p_party[no]->hp <= 0)
    return;
  // ?????
  if (p_party[no]->atr & ACT_ATR_TRAVEL)
    return;
  // ??
  if (no == BattleMyNo) {
    // ?
    ...
    // ??
    StockBoxDispBuffer(meterX - 21, meterY - 3,
                       meterX - 21 +
                           (int)(((double)p_party[BattleMyNo]->hp /
                                  (double)p_party[BattleMyNo]->maxHp) *
                                 40.0),
                       meterY - 3, DISP_PRIO_IME2, SYSTEM_PAL_GREEN, 2);
    StockBoxDispBuffer(meterX - 21, meterY - 2,
                       meterX - 21 +
                           (int)(((double)p_party[BattleMyNo]->hp /
                                  (double)p_party[BattleMyNo]->maxHp) *
                                 40.0),
                       meterY - 2, DISP_PRIO_IME2, SYSTEM_PAL_GREEN, 2);
    // ??
#ifdef _BATTLESKILL // (不可开) Syu ADD 战斗技能介面
    StockBoxDispBuffer(meterX - 21, meterY + 1,
                       meterX - 21 +
                           (int)(((double)p_party[BattleMyNo]->mp /
                                  (double)p_party[BattleMyNo]->maxMp) *
                                 40.0),
                       meterY + 1, DISP_PRIO_IME2, SYSTEM_PAL_YELLOW, 2);
    StockBoxDispBuffer(meterX - 21, meterY + 2,
                       meterX - 21 +
                           (int)(((double)p_party[BattleMyNo]->mp /
                                  (double)p_party[BattleMyNo]->maxMp) *
                                 40.0),
                       meterY + 2, DISP_PRIO_IME2, SYSTEM_PAL_YELLOW, 2);
#else
    StockBoxDispBuffer(
        meterX - 21, meterY + 1,
        meterX - 21 +
            (int)(((double)p_party[BattleMyNo]->mp / pc.maxMp) * 40.0),
        meterY + 1, DISP_PRIO_IME2, SYSTEM_PAL_YELLOW, 2);
    StockBoxDispBuffer(
        meterX - 21, meterY + 2,
        meterX - 21 +
      ...
      StockBoxDispBuffer(meterX - 21, meterY + 0,
                         meterX - 21 +
                             (int)(((double)p_party[BattleMyNo]->petHp /
                                    (double)p_party[BattleMyNo]->petMaxHp) *
                                   40.0),
                         meterY + 0, DISP_PRIO_IME2, SYSTEM_PAL_GREEN, 2);
    }
  } else { // ????
    if (no == BattleMyNo + 5)
      graNo = CG_BATTLE_BAR_PET_2;
    else
      graNo = CG_BATTLE_BAR_PET;
    // ?
    ...
    StockBoxDispBuffer(
        meterX - 21, meterY - 1,
        meterX - 21 +
            (int)(((double)p_party[no]->hp / (double)p_party[no]->maxHp) *
                  40.0),
        meterY - 1, DISP_PRIO_IME2, SYSTEM_PAL_GREEN, 2);
    // ?????????
    StockBoxDispBuffer(
      ...
      if (pActWnd->hp > 0) {
        ...
        if (g_bUseAlpha)
          StockDispBuffer(((WINDOW_DISP *)pActWnd->pYobi)->mx,
                          ((WINDOW_DISP *)pActWnd->pYobi)->my + 7,
          ...
          if (HitFontNo == battleMenuJujutsuFontNo[i] &&
            ...
            if (mouse.onceState & MOUSE_LEFT_CRICK) {
              ...
              if (magic[i].field == MAGIC_FIELD_MAP ||
                  magic[i].mp > p_party[BattleMyNo]->mp) {
                play_se(220, 320, 240);
              } else {
                BattleJujutuNo = i;
        ...
        if (pActWnd != NULL) {
          ...
          for (i = 0; i < 5; i++) {
            ...
            if (magic[i].useFlag == TRUE) {
              col = FONT_PAL_WHITE;
              if (magic[i].mp > p_party[BattleMyNo]->mp)
                col = FONT_PAL_RED; // ????
              if (magic[i].field == MAGIC_FIELD_MAP)
                col = FONT_PAL_GRAY; // ?????

              CenteringStr(magic[i].name, moji, MAGIC_NAME_LEN);
              battleMenuJujutsuFontNo[i] =
                  StockFontBuffer(x, y, FONT_PRIO_FRONT, col, moji, 2);
              y += 28;
              flag = TRUE;
            }
          }
          if (flag == FALSE)
            StockFontBuffer(x, y, FONT_PRIO_FRONT, 0, "没有咒术", 0);

          battleMenuJujutsuFontNo[5] = StockDispBuffer(
              ((WINDOW_DISP *)pActWnd->pYobi)->mx, pActWnd->y + 262 + 10,
        ...
        if (bak == FALSE) {
          ...
          for (i = 0; i < BATTLKPKPLYAERNUM; i++) {
            ...
            if (p_party[i]->func == NULL)
              continue;
            // ????
            if (((ATR_EQU *)p_party[i]->pYobi)->group_flg ==
                ((ATR_EQU *)p_party[BattleMyNo]->pYobi)->group_flg)
              continue;
            // ????
            if (i == BattleMyNo)
              continue;
        ...
        if (pActWnd != NULL) {
          ...
          for (i = 0; i < 3; i++) {
            if (i == 道具栏页数) {
              ...
              if (i) {
                ...
                flg = TRUE;
              if (flg) {
                StockDispBuffer(518, 182 + i * 56, DISP_PRIO_IME3, 55110 + i,
                                1);
                if (MakeHitBox(508, 154 + i * 56, 508 + 20, 151 + i * 56 + 60,
                               DISP_PRIO_IME4)) {
                  if (mouse.onceState & MOUSE_LEFT_CRICK) {
                    道具栏页数 = i;
...
#ifdef _MAPITEMNO_
                    if (nowFloor == 12345)
                      StockChatBufferLine("该地图战斗下不允许使用扩展包裹！",
                                          FONT_PAL_RED);
#endif
                  }
                }
              } else
                StockDispBuffer(518, 182 + i * 56, DISP_PRIO_IME3, 55107 + i,
                                1);
            }
          }
#ifdef _MAPITEMNO_
          ...
          for (i = MAX_ITEM - 1; i >= MAX_ITEMSTART; i--) {
            ...
            if (MakeHitBox(ItemBuffer[i].defX - 24, ItemBuffer[i].defY - 24,
                           ItemBuffer[i].defX + 25, ItemBuffer[i].defY + 23,
                           DISP_PRIO_IME3) == TRUE) {
                ...
                if (pc.item[i].useFlag == TRUE) {
                  ...
                  if (pc.item[i].useFlag != TRUE ||
                    ...
                    case ITEM_TARGET_PET: // 战斗中不可更换宠物装备
                      StockChatBufferLine("战斗中不可更换宠物装备",
                                          FONT_PAL_RED);
                      break;
#endif
                    }
        ...
        if (pActWnd != NULL) {
          ...
          for (i = 0; i < 5; i++) {
            ...
            if (pet[i].useFlag == TRUE && pc.selectPetNo[i] == TRUE) {
              ...
              battleMenuPetFontNo[i] =
                  StockFontBuffer(x, y, FONT_PRIO_FRONT, color, moji, 2);

              if (pet[i].earth > 0) {
                if (pet[i].earth > 50)
                  atrGraNo[atrFlag] = CG_ATR_ICON_EARTH_BIG;
                else
                  atrGraNo[atrFlag] = CG_ATR_ICON_EARTH_SML;
                atrFlag++;
              }
              if (pet[i].water > 0) {
                if (pet[i].water > 50)
                  atrGraNo[atrFlag] = CG_ATR_ICON_WATER_BIG;
                else
                  atrGraNo[atrFlag] = CG_ATR_ICON_WATER_SML;
                atrFlag++;
              }
              if (pet[i].fire > 0) {
                if (pet[i].fire > 50)
                  atrGraNo[atrFlag] = CG_ATR_ICON_FIRE_BIG;
                else
                  atrGraNo[atrFlag] = CG_ATR_ICON_FIRE_SML;
                atrFlag++;
              }
              if (pet[i].wind > 0) {
                if (pet[i].wind > 50)
                  atrGraNo[atrFlag] = CG_ATR_ICON_WIND_BIG;
                else
                  atrGraNo[atrFlag] = CG_ATR_ICON_WIND_SML;
                atrFlag++;
              }

              if (atrFlag > 0)
                StockDispBuffer(pActWnd->x + 214, y + 8, DISP_PRIO_IME3,
                                atrGraNo[0], 0);

              if (atrFlag > 1)
                StockDispBuffer(pActWnd->x + 214 + 16, y + 8, DISP_PRIO_IME3,
                                atrGraNo[1], 0);
              y += 26;
#ifdef _NEWFONT_
...
#else
              sprintf_s(moji, "%3d          %4d   %4d", pet[i].level, pet[i].hp,
                        pet[i].maxHp);
              StockFontBuffer(x + 27, y, FONT_PRIO_FRONT, color, moji, 0);
#endif
              y += 25;
              sprintf_s(moji, "%3d", pet[i].ai);
              StockFontBuffer(x + 60, y, FONT_PRIO_FRONT, color, moji, 0);
              y += 36;
              flag = TRUE;
            }
          }
          ...
          if (flag == TRUE) {
            if (pc.battlePetNo != -1) {
              battleMenuPetFontNo[5] =
                  StockDispBuffer(pActWnd->x + 70, pActWnd->y + 367,
                                  DISP_PRIO_IME3, CG_BTL_PET_RETURN_BTN, 2);
              battleMenuPetFontNo[6] =
                  StockDispBuffer(pActWnd->x + 187, pActWnd->y + 367,
                                  DISP_PRIO_IME3, CG_CLOSE_BTN, 2);
            } else {
              battleMenuPetFontNo[6] = StockDispBuffer(
                  ((WINDOW_DISP *)pActWnd->pYobi)->mx, pActWnd->y + 367,
                  DISP_PRIO_IME3, CG_CLOSE_BTN, 2);
            }
          } else {
...
void BattleButtonPPLSKILL(void) {
  if (MenuToggleFlag & JOY_ESC || MenuToggleFlag & JOY_CTRL_E ||
  ...
  if (wonflag == 1) {
...
#ifdef _READ16BITBMP
      if (g_bUseAlpha) {
        if (MakeHitBox(278, 11 + i * 78, 308, 89 + i * 78, DISP_PRIO_IME3) ==
            TRUE) {
          if (mouse.onceState & MOUSE_LEFT_CRICK) {
            SkillWnd2 = i;
            play_se(217, 320, 240);
...
#endif
          if (MakeHitBox(270, 14 + i * 79, 300, 92 + i * 79, DISP_PRIO_IME3) ==
              TRUE) {
        if (mouse.onceState & MOUSE_LEFT_CRICK) {
          SkillWnd2 = i;
          play_se(217, 320, 240);
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 4; j++) {
        //                    StockDispBuffer( x + 138 + j * 60 , y + 228 + i *
        //                    72 , DISP_PRIO_IME3, CG_FIELD_SKILL_TILE , 2 );
      }
    }
...
#ifdef _READ16BITBMP
      if (g_bUseAlpha)
        StockDispBuffer(168, 223, DISP_PRIO_IME3, CG_FIELD_SKILL_ASSIT, 0);
      else
#endif
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          ...
          if (profession_skill[AssitSkill[j + i * 4]].useFlag == 0)
            StockDispBuffer(x + 24 + j * 60, y + 98 + i * 57,
                            DISP_PRIO_IME3 + 1, CG_BTL_BUTTON_CROSS, 0);
          // StockDispBuffer( x + 138 + j * 48 , y + 228 + i * 52 ,
          // DISP_PRIO_IME3, CG_FIELD_SKILL_TILE , 0 );
          AssitSkillBtn[j + i * 4] = StockDispBuffer(
              x + 24 + j * 60, y + 98 + i * 57, DISP_PRIO_IME3 + 1,
              profession_skill[AssitSkill[j + i * 4]].icon, 2);
          sprintf_s(msg2, "%8s", profession_skill[AssitSkill[j + i * 4]].name);
          StockFontBuffer(x + j * 60, y + 110 + i * 57, FONT_PRIO_FRONT,
                          FONT_PAL_WHITE, msg2, 0);
          if (HitDispNo == AssitSkillBtn[j + i * 4]) {
            int use_color = 0;
            if (p_party[BattleMyNo]->mp >=
                    profession_skill[AssitSkill[j + i * 4]].costmp &&
                profession_skill[AssitSkill[j + i * 4]].costmp != 0) {
          ...
          if (mouse.onceState & MOUSE_LEFT_CRICK_UP) {
            if (HitDispNo == AssitSkillBtn[j + i * 4] &&
                profession_skill[AssitSkill[j + i * 4]].useFlag != 0) {
              if (p_party[BattleMyNo]->mp >=
                      profession_skill[AssitSkill[j + i * 4]].costmp &&
                  profession_skill[AssitSkill[j + i * 4]].costmp != 0) {
                wonflag = 0;
                ...
                BattleSetWazaHitBox(
                    profession_skill[AssitSkill[j + i * 4]].target, 1);
              } else if (profession_skill[AssitSkill[j + i * 4]].skillId ==
                             11 &&
                         profession_skill[AssitSkill[j + i * 4]].costmp ==
                             0) { // 嗜血成性不做mp限制
                wonflag = 0;
...
#ifdef _READ16BITBMP
      if (g_bUseAlpha)
        StockDispBuffer(168, 223, DISP_PRIO_IME3, CG_FIELD_SKILL_BATTLE, 0);
      else
#endif
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          ...
          if (profession_skill[BattleSkill[j + i * 4]].useFlag == 0)
            StockDispBuffer(x + 24 + j * 60, y + 98 + i * 57,
                            DISP_PRIO_IME3 + 1, CG_BTL_BUTTON_CROSS, 0);
          if (profession_skill[BattleSkill[j + i * 4]].skillId == 40 &&
              p_party[BattleMyNo]->hp <= 10)
            StockDispBuffer(x + 24 + j * 60, y + 98 + i * 57,
                            DISP_PRIO_IME3 + 1, CG_BTL_BUTTON_CROSS, 0);
          // StockDispBuffer( x + 138 + j * 48 , y + 228 + i * 52 ,
          // DISP_PRIO_IME3, CG_FIELD_SKILL_TILE , 0 );
          BattleSkillBtn[j + i * 4] = StockDispBuffer(
              x + 24 + j * 60, y + 98 + i * 57, DISP_PRIO_IME3 + 1,
              profession_skill[BattleSkill[j + i * 4]].icon, 2);
          ...
          if (HitDispNo == BattleSkillBtn[j + i * 4]) {
            ...
            if (p_party[BattleMyNo]->mp >=
                ...
                profession_skill[BattleSkill[j + i * 4]].costmp != 0) {
              use_color = FONT_PAL_WHITE;
            } else {
              use_color = FONT_PAL_GRAY;
            }
          ...
          if (mouse.onceState & MOUSE_LEFT_CRICK_UP) {
            if (HitDispNo == BattleSkillBtn[j + i * 4] &&
                profession_skill[BattleSkill[j + i * 4]].useFlag != 0) {
              if (p_party[BattleMyNo]->mp >=
                      profession_skill[BattleSkill[j + i * 4]].costmp &&
                  profession_skill[BattleSkill[j + i * 4]].costmp != 0) {
                if (!(profession_skill[BattleSkill[j + i * 4]].skillId == 40 &&
                      p_party[BattleMyNo]->hp <= 10)) { // 濒死攻击限制
                  wonflag = 0;
                  prouseskill = BattleSkill[j + i * 4];
                  BattleSetWazaHitBox(
                      profession_skill[BattleSkill[j + i * 4]].target, 1);
                }
              }
            }
          }
    ...
    case 2:
#ifdef _READ16BITBMP
      if (g_bUseAlpha)
        StockDispBuffer(168, 223, DISP_PRIO_IME3, CG_FIELD_SKILL_ADVSK, 0);
      else
#endif
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          ...
          // StockDispBuffer( x + 138 + j * 48 , y + 228 + i * 52 ,
          // DISP_PRIO_IME3, CG_FIELD_SKILL_TILE , 0 );
          AdvanceSkillBtn[j + i * 4] = StockDispBuffer(
              x + 24 + j * 60, y + 98 + i * 57, DISP_PRIO_IME3 + 1,
              profession_skill[AdvanceSkill[j + i * 4]].icon, 2);
          sprintf_s(msg2, "%8s",
          ...
          if (HitDispNo == AdvanceSkillBtn[j + i * 4]) {
            ...
            sprintf_s(msg, "%s",
                      profession_skill[AdvanceSkill[j + i * 4]].memo);
          }
          if (mouse.onceState & MOUSE_LEFT_CRICK_UP) {
            if (HitDispNo == AdvanceSkillBtn[j + i * 4] &&
                profession_skill[AdvanceSkill[j + i * 4]].useFlag != 0) {
              if (p_party[BattleMyNo]->mp >=
                      profession_skill[AdvanceSkill[j + i * 4]].costmp &&
                  profession_skill[AdvanceSkill[j + i * 4]].costmp != 0
#ifdef _PRO3_ADDSKILL
...
#ifdef _PRO_ABSOLUTE_DEFENSE
              else if ((profession_skill[AdvanceSkill[j + i * 4]].skillId ==
                            75 ||
                        profession_skill[AdvanceSkill[j + i * 4]].skillId ==
                            76 ||
                        profession_skill[AdvanceSkill[j + i * 4]].skillId ==
                            77) &&
                       profession_skill[AdvanceSkill[j + i * 4]].costmp == 0
                ...
                if (mouse.onceState & MOUSE_LEFT_CRICK) {
                  ...
                  if (petSkill[battlePetNoBak][i].field != PETSKILL_FIELD_MAP) {
                    ...
                    if (!((pet[battlePetNoBak].graNo == 101428) &&
                          (petSkill[battlePetNoBak][i].skillId == 600))
#ifdef _PETSKILL_EVOLUTION
                        && !((pet[battlePetNoBak].graNo == 102011 ||
                              pet[battlePetNoBak].graNo == 102012) &&
                             (petSkill[battlePetNoBak][i].skillId == 672))
#endif
                    )
#endif
                    // ??????????????
          ...
          if (pActWnd != NULL) {
            ...
            for (i = 0; i < pet[battlePetNoBak].maxSkill; i++) {
              ...
              if (petSkill[battlePetNoBak][i].useFlag == TRUE) {
                ...
                char *str = "                                                  "
                            "          ";
                battleMenuWazaFontNo[i] =
                    StockFontBuffer(x - 43, y, FONT_PRIO_FRONT, color, str, 2);
#else
                battleMenuWazaFontNo[i] =
                    StockFontBuffer(x - 43, y, FONT_PRIO_FRONT, color, moji, 2);
#endif
              }
              y += 25;
            }
...
void BattleTargetSelect(void) {
  ...
  if (!(MenuToggleFlag & JOY_ESC) && !(MenuToggleFlag & JOY_CTRL_E) &&
      !(MenuToggleFlag & JOY_CTRL_A) && !(menuBtn == 1)) {

    if (pActInfoWnd == NULL) {
      pActInfoWnd = MakeWindowDisp(210, 356, 3, 2, 0, 1);
    } else {
      if (pActInfoWnd->hp > 0) {
        StockFontBuffer(pActInfoWnd->x + 38, pActInfoWnd->y + 28,
                        FONT_PRIO_FRONT, FONT_PAL_YELLOW, "请选择", 0);
    ...
    if ((targetNo = CheckBattelTarget()) != -1) {
      ...
      case BATTLE_WAZA:
        ...
        case PETSKILL_TARGET_ONE_ROW_ALL:
          ...
          if (targetNo >= 0 && targetNo < 5)
            no = BATTLKPKPLYAERNUM + 6;
          if (targetNo >= 5 && targetNo < 10)
            no = BATTLKPKPLYAERNUM + 5;
          if (targetNo >= 10 && targetNo < 15)
            no = BATTLKPKPLYAERNUM + 3;
          if (targetNo >= 15 && targetNo < 20)
            no = BATTLKPKPLYAERNUM + 4;
          sprintf_s(moji, "W|%X|%X", BattleWazaNo, no);
          lssproto_B_send(sockfd, moji);
          play_se(217, 320, 240);
...
static int Ordinal[] = {4 + 15, 2 + 15, 0 + 15, 1 + 15, 3 + 15, 4 + 10, 2 + 10,
                        0 + 10, 1 + 10, 3 + 10, 4 + 5,  2 + 5,  0 + 5,  1 + 5,
                        3 + 5,  4,      2,      0,      1,      3};
// ?????????? ***************************************************/
#ifdef _AI_OTHER
#ifdef _AI_CAPTURE
...
void BattleCntDownDisp(void) {
  ...
  // ???????
  if (BattleCntDownFlag == FALSE)
    return;
#else
  if (BattleCntDownFlag == FALSE || BattleCliTurnNo < 0)
    return;
#endif
  ...
  /*//人物ai
      ...
      if( AI!=AI_NONE && !PauseAI){
          if( battleMenuFlag&BATTLE_MENU && battleTargetSelectFlag){
              if( (BattleIntervalCnt==0 && BattleCntDownRest<=28000) ||
  (BattleIntervalCnt!=0 && BattleIntervalCnt<TimeGetTime())){ BattleIntervalCnt
  = TimeGetTime()+500;
                  // ??????
              ...
              if( (BattleIntervalCnt==0 && BattleCntDownRest<=28000) ||
  BattleIntervalCnt<TimeGetTime()){ int no; int targetNo = GetBattelTarget();
                  char moji[256];
                  switch( petSkill[ battlePetNoBak ][ BattleWazaNo ].target ){
                  case PETSKILL_TARGET_MYSELF:                // ??
                  case PETSKILL_TARGET_OTHER:                    //
  ...
  ??????
                  ...
                  case PETSKILL_TARGET_ALL:    // ?
                      // ???
                      sprintf_s( moji,"W|%X|%X", BattleWazaNo, BATTLKPKPLYAERNUM
  + 2 );
                      // ??
                      if( bNewServer)
                          lssproto_B_send( sockfd, moji );
...
void BattleMenuProc(void) {
  ...
  if (battleMenuFlag & BATTLE_MENU) {
    ...
    if (battleMenuReturn == FALSE) {
      ...
      /*
      #ifndef __AI
                  if( !PauseAI && AI!=AI_NONE && battleButtonBak==-1 &&
      (BattleCntDown-TimeGetTime())<=29500){ DWORD ai = AI; if( ai ==
      AI_SELECT){ if( p_party[ BattleMyNo ]->maxHp*3/10 < p_party[ BattleMyNo
      ]->hp) ai = AI_ATTACK; else if( p_party[ BattleMyNo ]->maxHp*15/100 <
      p_party[ BattleMyNo ]->hp) ai = AI_GUARD; else ai = AI_ESCAPE;
                      }
...
#ifdef _BATTLESKILL // (不可开) Syu ADD 战斗技能介面
    battleButtonDispNo[0] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BATTLE_ATT_UP - battleButtonFlag[0], 2);
    battleButtonDispNo[1] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BATTLE_JUJUTSU_UP - battleButtonFlag[1], 2);
    battleButtonDispNo[2] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BATTLE_CAPTURE_UP - battleButtonFlag[2], 2);
    battleButtonDispNo[3] = StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                                            CG_BATTLE_HELP_UP - helpFlag, 2);
    battleButtonDispNo[4] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BATTLE_DEF_UP - battleButtonFlag[4], 2);
    battleButtonDispNo[5] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BATTLE_ITEM_UP - battleButtonFlag[5], 2);
    battleButtonDispNo[6] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BATTLE_PET_UP - battleButtonFlag[6], 2);
    battleButtonDispNo[7] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BATTLE_ESCAPE_UP - battleButtonFlag[7], 2);
    battleButtonDispNo[9] = StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                                            CG_BATTLE_PROSKL_UP - wonflag, 2);
    StockDispBuffer(buttonX, buttonY, DISP_PRIO_MENU, CG_BATTLE_PANEL, 0);
#else
    battleButtonDispNo[0] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BTL_BUTTON_ATTACK_UP + battleButtonFlag[0], 2);
    battleButtonDispNo[1] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BTL_BUTTON_JUJUTU_UP + battleButtonFlag[1], 2);
    battleButtonDispNo[2] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BTL_BUTTON_CAPTURE_UP + battleButtonFlag[2], 2);
    battleButtonDispNo[3] = StockDispBuffer(
        buttonX, buttonY, DISP_PRIO_IME3, CG_BTL_BUTTON_HELP_UP + helpFlag, 2);
    battleButtonDispNo[4] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BTL_BUTTON_GUARD_UP + battleButtonFlag[4], 2);
    battleButtonDispNo[5] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BTL_BUTTON_ITEM_UP + battleButtonFlag[5], 2);
    battleButtonDispNo[6] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BTL_BUTTON_PET_UP + battleButtonFlag[6], 2);
    battleButtonDispNo[7] =
        StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                        CG_BTL_BUTTON_ESCAPE_UP + battleButtonFlag[7], 2);
    StockDispBuffer(buttonX, buttonY, DISP_PRIO_MENU, CG_BTL_BUTTON_BASE, 0);
#endif

    if (CheckPetSuu() >= 5) {
#ifdef _BATTLESKILL // (不可开) Syu ADD 战斗技能介面
      StockDispBuffer(buttonX + 44, buttonY - 14, DISP_PRIO_IME4,
                      CG_BTL_BUTTON_CROSS, 0);
#else
      StockDispBuffer(buttonX + 28, buttonY - 18, DISP_PRIO_IME4,
                      CG_BTL_BUTTON_CROSS, 0);
#endif
    }
    ...
    if (NoHelpFlag == TRUE) {
#ifdef _BATTLESKILL // (不可开) Syu ADD 战斗技能介面
      StockDispBuffer(buttonX + 28 + 54 + 4, buttonY - 18 + 4, DISP_PRIO_IME4,
                      CG_BTL_BUTTON_CROSS, 0);
#else
      StockDispBuffer(buttonX + 28 + 54, buttonY - 18, DISP_PRIO_IME4,
                      CG_BTL_BUTTON_CROSS, 0);
#endif
    }
#ifdef _MAGIC_NOCAST
    if (NoCastFlag == TRUE) {
#ifdef _BATTLESKILL // (不可开) Syu ADD 战斗技能介面
      StockDispBuffer(buttonX + 28 - 54 - 14, buttonY - 18 + 4, DISP_PRIO_IME4,
                      CG_BTL_BUTTON_CROSS, 0);
#else
      StockDispBuffer(buttonX + 28 - 54, buttonY - 18, DISP_PRIO_IME4,
                      CG_BTL_BUTTON_CROSS, 0);
  ...
  if (battlePetMenuFlag == TRUE) {
    ...
    if (battlePetNoBak <= -1 || p_party[BattleMyNo + 5]->hp <= 0) {
      ...
      if (battleWazaTargetBak != -1) { // 上一回合的技能编号
        ...
        if (petSkill[battlePetNoBak][battleWazaTargetBak].field ==
                2 // 被标示为不可选
            ||
            petSkill[battlePetNoBak][battleWazaTargetBak].skillId ==
                0 // Change fix
                  // 人物使用针针相对或暴风雨时,若宠接着选防守或待机,则接下来宠物的选单不会出来,这里强制让选单出现
            || petSkill[battlePetNoBak][battleWazaTargetBak].skillId == 2) {
          // if( petSkill[battlePetNoBak][battleWazaTargetBak].skillId == 623 ){
          // //623:在petskill2.txt内的技能编号
          battleWazaTargetBak = -1;
          battlePetButtonFlag = TRUE;
        } else {
  ...
  if (battleMenuFlag & BATTLE_MENU_PET) {
    ...
    if (battleWazaTargetBak != -1) {
      ...
      battleButtonDispNo[8] = StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                                              CG_PET_BTL_BUTTON_CANCEL_UP, 2);

    } else {
      battleButtonDispNo[8] =
          StockDispBuffer(buttonX, buttonY, DISP_PRIO_IME3,
                          CG_PET_BTL_BUTTON_WAZA_UP + battleButtonFlag[8], 2);
    }
    StockDispBuffer(buttonX, buttonY, DISP_PRIO_MENU, CG_PET_BTL_BUTTON_BASE,
                    0);
  }
