...
void char_title_update(char *data)
{
    ...
    memset(图档索引, 0, 1024);
    memset(内容介绍, 0, 4000);
    getStringFromIndexWithDelim_body((char *) data,(char *)"#@",1,基础内容,126);
    getStringFromIndexWithDelim_body((char *) 基础内容,(char *)"|",1,临时,126);
    int titleindex = atoi(临时);
    extern char* FreeGetTitleStr(int id);
    if( titleindex > 0 )
      getCharTitleSplit(FreeGetTitleStr(titleindex),&称号.title);
    else
      memset(&称号.title, 0, sizeof(TITLE_STR));
    ...
    if(数量!=0){
        ...
        getStringFromIndexWithDelim_body((char *) data, (char *)"#@", 2, 图档内容, 1024);
        getStringFromIndexWithDelim_body((char *) data, (char *)"#@", 3, 内容介绍, 4000);
        getStringFromIndexWithDelim_body((char *) data, (char *)"#@", 4, 图档索引, 1024);
        int i=0;
        extern char* FreeGetTitleStr(int id);
        for(i;i<数量;i++){
...
void init_title_struct()
{
  memset(&称号, 0, sizeof(title_struct));
}
#endif
