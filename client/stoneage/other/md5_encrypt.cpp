...
void MD5Update(MD5_CTX_ENCRYPT *context, unsigned char *input,
               unsigned int inputLen) {
  unsigned int i, index, partLen;
  index = (unsigned int)((context->count[0] >> 3) &
                         0x3F); /* Compute number of bytes mod 64 */
...
void Decode(UINT4 *output, unsigned char *input, unsigned int len) {
  unsigned int i, j;
  for (i = 0, j = 0; j < len; i++, j += 4)
    output[i] = (((UINT4)input[j] << 0) | ((UINT4)input[j + 1] << 8) |
...
char *hmac_md5(char *text, char *key) {
  ...
  MD5Update(&context, (unsigned char *)text,
  ...
  for (i = 0; i < 16; i++) {
    ...
    sprintf_s(&(output1[2 * i + 1]), 34 - 2 * i + 1, "%02x",
