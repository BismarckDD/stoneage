...
INLINE BOOL PETSKILL_setChar(int index, PETSKILL_DATACHAR element, char *new_char) {
  ...
  strncpysafe(PETSKILL_petskill[index].string[element].string,
             sizeof(PETSKILL_petskill[index].string[element].string), new_char);
  return TRUE;
}
/*----------------------------------------------------------------------
 * 矢永玄  及醒毛襞月［
...
int PETSKILL_Use(int char_index, int havepetskill, int toindex, char *data
  ...
  petskillid = CHAR_getPetSkill(char_index, havepetskill);
...
#define _SKILLTYPE_BATTLE 0x02
  if ((CHAR_getInt(char_index, CHAR_WHICHTYPE) == CHAR_TYPEPET) &&
    ...
    int masterindex = CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX);
  ...
  if (func) {
    ...
    if (strlen(buf) > 0 && buf[0] == '$') {
      CHAR_setPetSkill(char_index, havepetskill, -1);
    }
#endif
    ret = func(char_index, toindex, array, data);
  } else {
    ret = FALSE;
  }
  return ret;
...
//----------------------------------------------------------------------
int PETSKILL_ContinuationAttack(int char_index, int toindex, int array,
                                char *data
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_RENZOKU);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  ...
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, N);

  return TRUE;
}

int PETSKILL_ChargeAttack(int char_index, int toindex, int array, char *data) {
  char *pszOption, *pszP;
  int N = 1, Per = 0;

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_CHARGE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  if (sscanf(pszOption, "%d", &N) == 1) {
  ...
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, N);
  CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM3, Per);
  return TRUE;
}

int PETSKILL_Guardian(int char_index, int toNo, int array, char *data) {
  char *pszOption, *pszP;
  ...
  if ((pszP = strstr(pszOption, "攻%")) != NULL) {
    ...
    fPer = (fPer / 100);
    strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR);
    strdef = (int)(strdef * fPer);
    CHAR_setWorkInt(char_index, CHAR_WORKATTACKPOWER,
                    (CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) + strdef));
  }

  if ((pszP = strstr(pszOption, "防%")) != NULL) {
    sscanf(pszP + 3, "%f", &fPer);
    fPer = (fPer / 100);
    strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH);
    strdef = (int)(strdef * fPer);
    CHAR_setWorkInt(char_index, CHAR_WORKDEFENCEPOWER,
                    (CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) + strdef));
  }
...
int PETSKILL_Mighty(int char_index, int toindex, int array, char *data) {
  char *pszOption, *pszP;
  int iBai = 0, iDuck = 0;
  float fBai = 2.00;
  float fPer = 0.01;
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_MIGHTY);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  ...
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, iBai);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  ...
  if ((pszP = strstr(pszOption, "敏%")) != NULL) {
    sscanf(pszP + 3, "%f", &fPer);
    fPer = (fPer / 100);
    strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX);
    strdef = (int)(strdef * fPer);
    CHAR_setWorkInt(char_index, CHAR_WORKQUICK,
                    (CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX) + strdef));
  }
...
int PETSKILL_PowerBalance(int char_index, int toindex, int array, char *data) {
  char *pszOption = "\0", *pszP;
  float fPer = 0.01;
  int strdef;

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_POWERBALANCE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
#ifdef _POWER_UP
    if (type == 2) {
      strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) + value;
      strdef = (int)(strdef * fPer);
      CHAR_setWorkInt(
          char_index, CHAR_WORKATTACKPOWER,
          (CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) + strdef + value));
    } else
#endif
    {
      strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR);
...
#ifdef _POWER_UP
    if (type == 3) {
      strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) + value;
      strdef = (int)(strdef * fPer);
      CHAR_setWorkInt(
          char_index, CHAR_WORKDEFENCEPOWER,
          (CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) + strdef + value));
    } else
#endif
    {
      strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH);
...
#ifdef _POWER_UP
    if (type == 4) {
      strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX) + value;
      strdef = (int)(strdef * fPer);
      CHAR_setWorkInt(
          char_index, CHAR_WORKQUICK,
          (CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX) + strdef + value));
    } else
#endif
    {
      strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX);
...
//----------------------------------------------------------------------
int PETSKILL_StatusChange(int char_index, int toindex, int array, char *data
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_STATUSCHANGE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  ...
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, i);
  CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM3, turn);
...
int PETSKILL_WildViolentAttack(int char_index, int toindex, int array,
                               char *data
...
#ifdef _SKILL_SACRIFICE
int PETSKILL_Sacrifice(int char_index, int toindex, int array, char *data

) {
  char buf[256];

  if (CHAR_getInt(char_index, CHAR_HP) >
      CHAR_getWorkInt(char_index, CHAR_WORKMAXHP) * 0.2) {
    CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SACRIFICE);
    CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
    CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  } else {
    ...
    CHAR_talkToCli(CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX), -1, buf,
                   CHAR_COLORYELLOW);
    return FALSE;
  }
  return TRUE;
...
#endif
#ifdef _SKILL_REFRESH
int PETSKILL_Refresh(int char_index, int toindex, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_REFRESH);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
#endif
#ifdef _SKILL_WEAKEN // vincent宠技:虚弱
int PETSKILL_Weaken(int char_index, int toindex, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_WEAKEN);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
...
#endif
#ifdef _SKILL_DEEPPOISON // vincent宠技:剧毒
int PETSKILL_Deeppoison(int char_index, int toindex, int array, char *data

) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_DEEPPOISON);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
...
#ifdef _SKILL_BARRIER // vincent宠技:魔障
int PETSKILL_Barrier(int char_index, int toindex, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_BARRIER);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
...
#ifdef _SKILL_NOCAST // vincent宠技:沉默
int PETSKILL_Nocast(int char_index, int toindex, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_NOCAST);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
...
#ifdef _SKILL_ROAR // vincent宠技:大吼
int PETSKILL_Roar(int char_index, int toindex, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ROAR);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
...
#ifdef _SKILL_GUARDBREAK2 // 破除防御2 vincent add 2002/05/20
int PETSKILL_GuardBreak2(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_GBREAK2);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}
...
int PETSKILL_NormalAttack(int char_index, int toindex, int array, char *data) {
...
#ifdef _INVERSION_PETSKILL
int PETSKILL_Inversion(int char_index, int toindex, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_INVERSION);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}
...
#ifdef _MASSAGE_PETSKILL
int PETSKILL_Massage(int char_index, int toindex, int array, char *data
) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_MASSAGE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}
...
#ifdef _PETOUT_PETSKILL
int PETSKILL_Petout(int char_index, int toindex, int array, char *data
) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_PETOUT);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
}
#endif
...
int PETSKILL_Strength(int char_index, int toNo, int array, char *data) {
  ...
  if (nums >= max(1, atoi(buf1))) {
    int toindex = CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX);
    char buf[256];
  ...
  CHAR_setWorkInt(char_index, CHAR_WORK_STRENGTH, ++nums);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_STRENGTH);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
...
int PETSKILL_NormalGuard(int char_index, int toindex, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_GUARD);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
...
int PETSKILL_None(int char_index, int toindex, int array, char *data) {
...
int PETSKILL_EarthRound(int char_index, int toNo, int array, char *data) {
  char *pszOption, *pszP;
  float fPer = 0.01;
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_EARTHROUND1);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  ...
  if ((pszP = strstr(pszOption, "攻%")) != NULL) {
    sscanf(pszP + 3, "%f", &fPer);
    CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM3, (int)fPer);
  }

  return TRUE;
}

int PETSKILL_GuardBreak(int char_index, int toNo, int array, char *data) {
  char *pszOption, *pszP;
  float fPer = 0.01;
  int strdef;

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_GBREAK);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  if ((pszP = strstr(pszOption, "攻%")) != NULL) {
...
int PETSKILL_SpeedyAttack(int char_index, int toNo, int array, char *data
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SPEEDYATTACK);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  if (pszOption == "\0") {
...
int PETSKILL_Abduct(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ABDUCT);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  ...
  {
    int badindex = CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX);
...
int PETSKILL_Steal(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_STEAL);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}
...
#ifdef _BATTLESTEAL_FIX
int PETSKILL_StealMoney(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_STEALMONEY);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}
...
// 檗  -- 失奶  丞宁岳
//
int PETSKILL_Merge(int char_index, int toNo, int array, char *data) {
  int ret, ownerindex;

  // Robin 0521 debug
  ownerindex = CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX);
  if (CHAR_getWorkInt(ownerindex, CHAR_WORKBATTLEMODE) !=
      BATTLE_CHARMODE_NONE) {
    print(" Merge_ErrB:fd=%d ", ownerindex);
    return FALSE;
  }
  ret = ITEM_mergeItem_merge(CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX),
                             CHAR_getInt(char_index, CHAR_PETID), data,
                             char_index, 0);
#ifdef _MO_LUA_MERGE_CALLBACK
  MergeCallBack(CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX), char_index,
                data, ret);
#endif
  return ret;
}
...
#ifdef _ALCHEMIST
int PETSKILL_Alchemist(int char_index, int toNo, int array, char *data) {
  int ret, ownerindex;
...
int PETSKILL_NoGuard(int char_index, int toNo, int array, char *data

) {
  char *pszOption, *pszP;
  int Duck = 0, Counter = 0, Critical = 0;

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_NOGUARD);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  if ((pszP = strstr(pszOption, "避%")) != NULL) {
    sscanf(pszP + 3, "%d", &Duck);
    CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM3, Duck);
  }
...
int PETSKILL_AttackMagic(int char_index, int toindex, int array, char *data) {
  char *pszP;
  char szMagic[] = "magic";
  ...
  */
  // 设定宠物技能为魔法
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ATTACK_MAGIC);
  // 攻击目标设定
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  // 设定所使用的是那一种魔法
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, magic);
  // 设定所使用的魔法的道具
  //	CHAR_SETWORKINT_HIGH(char_index,CHAR_WORKBATTLECOM3,item);
  // 命令下达完毕
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_FallGround(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_FALLRIDE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Explode(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  if (BattleArray[CHAR_getWorkInt(char_index, CHAR_WORKBATTLEINDEX)].type !=
      BATTLE_TYPE_P_vs_P) {
    CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_ATTACK);
    return TRUE;
  } else
    CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_EXPLODE);
  ...
  if ((pszP = strstr(pszOption, "命%")) != NULL) {
    ...
    strdef = CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR);
    strdef = (int)(strdef * fPer);
    CHAR_setWorkInt(char_index, CHAR_WORKFIXDEX,
                    (CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX) + strdef));
  }
  if ((pszP = strstr(pszOption, "防%")) != NULL) {
    sscanf(pszP + 3, "%f", &fPer);
...
int ENEMYSKILL_ReLife(int enemy_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ENEMYRELIFE);
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}

int ENEMYSKILL_ReHP(int enemy_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ENEMYREHP);
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}
...
int ENEMYSKILL_EnemyHelp(int enemy_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ENEMYHELP);
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(enemy_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  return TRUE;
}
...
int PETSKILL_DamageToHp(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_DAMAGETOHP);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录技能
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKATTACKPOWER, strdef);
...
int PETSKILL_MpDamage(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_MPDAMAGE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); ////记录技能
...
int PETSKILL_ToothCrushe(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_TOOTHCRUSHE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录技能
  /*
...
int PETSKILL_Modifyattack(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_MODIFYATT);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录技能
  /*
...
int PETSKILL_Mdfyattack(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_MDFYATTACK);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  ...
  if (getStringFromIndexWithDelim(pszOption, "|", 2, buf1, sizeof(buf1)) ==
    ...
    return FALSE;
  nums = atoi(buf1);
  CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM4, nums); // 记录数值
  return TRUE;
}
#endif
...
int PETSKILL_Inslay(int index, int toNo, int array, char *data) {
  int char_index, petid;

  int inslayindex = -1, inslayno = -1;
  int i, j, itemno;
#define MAXINSLAY 15

  int item_indexs[MAXINSLAY] = {-1, -1, -1, -1};
  ...
  for (i = 0; i < 15; i++) {
    ...
    if (getStringFromIndexWithDelim(data, "|", i + 1, buf, sizeof(buf)) ==
      ...
      break;
    itemno = atoi(buf);
    if (!CHAR_CHECKITEMINDEX(char_index, itemno))
      continue;
    ...
    if (ITEM_getInt(item_index, ITEM_CANBEPILE) == 1 &&
      ...
      CHAR_talkToCli(char_index, -1, "叠加物品无法进行精工。",
                     CHAR_COLORYELLOW);
      return FALSE;
    }
#endif
    ...
    if (i >= MAXINSLAY) {
      CHAR_talkToCli(char_index, -1, "精工每次只能选择四个物品。",
                     CHAR_COLORYELLOW);
      return FALSE;
    }
    for (j = 0; j < MAXINSLAY; j++) {
      if (item_indexs[j] == item_index) {
        print("精工物品index相同 ERROR !!\n");
        return FALSE;
      }
    }
    if ((buf1 = ITEM_getChar(item_index, ITEM_TYPECODE)) == "\0") {
      print(" ITEM_TYPECODE == NULL 错误 !!\n");
      return FALSE;
    }
    if (!strcmp(buf1, "\0") || !strcmp(buf1, "NULL")) {
      CHAR_talkToCli(char_index, -1, "这样物品不适合精工。", CHAR_COLORYELLOW);
      return FALSE;
    }
    if (strstr(buf1, "INSLAY") != 0) {
      if (!ITEM_CHECKINDEX(inslayindex)) {
        inslayindex = item_index;
        inslayno = itemno;
      } else {
        CHAR_talkToCli(char_index, -1, "精工每次只能选择一个武器或防具。",
                       CHAR_COLORYELLOW);
        return FALSE;
      }
    }
    item_indexs[i] = item_index;
    itemnos[i] = itemno;
  }

  if (!ITEM_CHECKINDEX(inslayindex)) {
    CHAR_talkToCli(char_index, -1, "精工必须选择一个武器或防具。",
                   CHAR_COLORYELLOW);
    return FALSE;
  }
#ifdef _EXPANSION_ITEM_INSLAY
  int maxinslay =
      max(3, min(atoi(PETSKILL_getChar(array, PETSKILL_OPTION)), 25));
#endif
  for (i = 0; i < MAXINSLAY; i++) {
    if (!ITEM_CHECKINDEX(item_indexs[i]))
      break;
    if (inslayindex == item_indexs[i])
      continue;

#ifdef _EXPANSION_ITEM_INSLAY
    if (PETSKILL_ITEM_inslay(char_index, inslayindex, item_indexs[i],
                             maxinslay) == FALSE) {
      return FALSE;
    }
#else
    if (PETSKILL_ITEM_inslay(char_index, inslayindex, item_indexs[i]) == FALSE) {
      return FALSE;
    }
#endif
    CHAR_DelItem(char_index, itemnos[i]);
    /*
                    CHAR_setItemIndex( char_index, itemnos[i] ,-1);
                    CHAR_sendItemDataOne( char_index, itemnos[i] );
                    ITEM_endExistItemsOne( item_indexs[i]);
    */
  }

  CHAR_sendItemDataOne(char_index, inslayno);
...
#ifdef _MAGIC_SUPERWALL
int PETSKILL_MagicStatusChange(int char_index, int toindex, int array,
                               char *data) {
  if (CHAR_CHECKINDEX(char_index) == FALSE)
    return FALSE;

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SUPERWALL);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toindex);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
...
int PETSKILL_SetDuck(int char_index, int toNo, int array, char *data) {
  if (CHAR_CHECKINDEX(char_index) == FALSE)
    return FALSE;
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SETDUCK);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
...
int PETSKILL_SetMagicPet(int char_index, int toNo, int array, char *data) {
  ...
  if (CHAR_CHECKINDEX(char_index) == FALSE)
    return FALSE;
  nums = CHAR_getWorkInt(char_index, CHAR_MAGICPETMP);
  if (nums >= 3) {
    int toindex = CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX);
    CHAR_talkToCli(toindex, -1, "此技能单场限用三次。", CHAR_COLORYELLOW);
    return FALSE;
  }
  CHAR_setWorkInt(char_index, CHAR_MAGICPETMP, nums);

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SETMAGICPET);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
...
int PETSKILL_Fixitem(int index, int toNo, int array, char *data) {
  ...
  int i, j, itemno;
#define MAXFIXITEM 2
  int item_indexs[MAXFIXITEM] = {-1, -1};
  ...
  for (i = 0; i < 15; i++) {
    ...
    if (i >= MAXFIXITEM) {
      CHAR_talkToCli(char_index, -1, "修复每次只能选择两个物品。",
                     CHAR_COLORYELLOW);
      return FALSE;
    }
    ...
    if (itemtype == ITEM_DISH) {
      CHAR_talkToCli(char_index, -1, "料理不能做修复。", CHAR_COLORYELLOW);
      return FALSE;
    }
    ...
    if ((itemtype >= ITEM_FIST && itemtype <= ITEM_AMULET) ||
      ...
      if (!ITEM_CHECKINDEX(fixindex)) {
        ...
        CHAR_talkToCli(char_index, -1, "修复每次只能选择一个武器或防具。",
                       CHAR_COLORYELLOW);
        return FALSE;
      }
    }
  ...
  if (!ITEM_CHECKINDEX(fixindex)) {
    CHAR_talkToCli(char_index, -1, "修复必须选择一个武器或防具。",
                   CHAR_COLORYELLOW);
    return FALSE;
  }

  if (PETSKILL_ITEM_FixItem(char_index, fixindex, item_indexs) == FALSE) {
    return FALSE;
  } else {
    for (i = 0; i < MAXFIXITEM; i++) {
      if (!ITEM_CHECKINDEX(item_indexs[i]))
        break;
      if (fixindex == item_indexs[i])
        continue;
      CHAR_setItemIndex(char_index, itemnos[i], -1);
      CHAR_sendItemDataOne(char_index, itemnos[i]);
      ITEM_endExistItemsOne(item_indexs[i]);
  ...
  CHAR_sendItemDataOne(char_index, fixno);
  CHAR_complianceParameter(char_index);
  CHAR_send_P_StatusString(char_index, CHAR_P_STRING_ATK | CHAR_P_STRING_DEF |
                                           CHAR_P_STRING_QUICK |
                                           CHAR_P_STRING_CHARM);
  CHAR_talkToCli(char_index, -1, "修复完成。", CHAR_COLORYELLOW);
  return TRUE;
}
#endif
...
int PETSKILL_BattleTimid(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_TIMID);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);

  CHAR_setWorkInt(char_index, CHAR_WORKATTACKPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) * 0.7));
  CHAR_setWorkInt(char_index, CHAR_WORKDEFENCEPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) * 0.4));
  CHAR_setWorkInt(char_index, CHAR_WORKQUICK,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX) * 0.8));
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
...
int PETSKILL_2BattleTimid(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_2TIMID);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  ...
  if ((pszP = strstr(pszOption, "-攻%")) != NULL) {
    ...
    fPer = (fPer / 100);
    CHAR_setWorkInt(char_index, CHAR_WORKATTACKPOWER,
                    (CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) * fPer));
  }
  if ((pszP = strstr(pszOption, "+攻%")) != NULL) {
    sscanf(pszP + 4, "%f", &fPer);
    fPer = (fPer / 100);
  ...
  if ((pszP = strstr(pszOption, "-防%")) != NULL) {
    ...
    fPer = (fPer / 100);
    CHAR_setWorkInt(char_index, CHAR_WORKDEFENCEPOWER,
                    (CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) * fPer));
  }
  if ((pszP = strstr(pszOption, "+防%")) != NULL) {
    sscanf(pszP + 4, "%f", &fPer);
    fPer = (fPer / 100);
  ...
  if ((pszP = strstr(pszOption, "-敏%")) != NULL) {
    ...
    fPer = (fPer / 100);
    CHAR_setWorkInt(char_index, CHAR_WORKQUICK,
                    (CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX) * fPer));
  }
  if ((pszP = strstr(pszOption, "+敏%")) != NULL) {
    sscanf(pszP + 4, "%f", &fPer);
    fPer = (fPer / 100);
...
#endif
#ifdef _PETSKILL_ANTINTER
int PETSKILL_AntInter(int char_index, int toNo, int array, char *data) {
  if (!CHAR_CHECKINDEX(char_index))
    return FALSE;

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ANTINTER);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Resurrection(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_RESURRECTION);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Grappling(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_GRAPPLING);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Lostlost(int char_index, int toNo, int array, char *data) {
  ...
  if (CHAR_CHECKINDEX(char_index) == FALSE)
    return FALSE;
  nums = CHAR_getWorkInt(char_index, CHAR_WORK_LOSTLOST);
  ...
  CHAR_setWorkInt(char_index, CHAR_WORK_LOSTLOST, ++nums);

  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_LOSTLOST);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_BattleProperty(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_PROPERTYSKILL);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);

  //	CHAR_setWorkInt( char_index, CHAR_WORKATTACKPOWER, ( CHAR_getWorkInt(
  //char_index, CHAR_WORKFIXSTR)*0.7) ); 	CHAR_setWorkInt( char_index,
  //CHAR_WORKDEFENCEPOWER, ( CHAR_getWorkInt( char_index,
  //CHAR_WORKFIXTOUGH)*0.5) ); 	CHAR_setWorkInt( char_index, CHAR_WORKQUICK, (
  //CHAR_getWorkInt( char_index, CHAR_WORKFIXDEX)*0.95) );
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
...
int PETSKILL_BattleTearDamage(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_PETSKILLTEAR);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);

  CHAR_setWorkInt(char_index, CHAR_WORKATTACKPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) * 0.9));
  CHAR_setWorkInt(char_index, CHAR_WORKDEFENCEPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) * 0.8));
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
}
#endif
...
int PETSKILL_Lighttakeed(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_LIGHTTAKE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKATTACKPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) * 0.7));
  CHAR_setWorkInt(char_index, CHAR_WORKDEFENCEPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) * 0.5));
  //	CHAR_setWorkInt( char_index, CHAR_WORKQUICK, (CHAR_getWorkInt(
  //char_index, CHAR_WORKFIXDEX)*0.95) );
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
}
#endif
...
int PETSKILL_AttackCrazed(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ATTCRAZED);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);

  CHAR_setWorkInt(char_index, CHAR_WORKATTACKPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXSTR) * 0.8));
  CHAR_setWorkInt(char_index, CHAR_WORKDEFENCEPOWER,
                  (CHAR_getWorkInt(char_index, CHAR_WORKFIXTOUGH) * 0.7));
  ...
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM3, atoi(pszOption));
  return TRUE;
}
#endif
...
int PETSKILL_AttackShoot(int char_index, int toNo, int array, char *data) {
  // int n = RAND ( 3 , 5 ) ;
  int n;
  int loyal = CHAR_getWorkInt(char_index, CHAR_WORKFIXAI);
  char bb1[16], bb2[16];
  char *buf = "\0";
  if (!CHAR_CHECKINDEX(char_index))
    return FALSE;
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ATTSHOOT);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  ...
  if (loyal >= 100) {
    if (RAND(1, 300) > 299)
      n = 8;
    else if ((CHAR_getInt(char_index, CHAR_HP)) < 20 && (RAND(1, 50) > 49))
      n = 8;
  }
  /*
          if ( n != 8 )
                  CHAR_setWorkInt( char_index, CHAR_WORKATTACKPOWER,
     (CHAR_getWorkInt( char_index, CHAR_WORKFIXSTR) * 1.2 / n ) ); else
                  CHAR_setWorkInt( char_index, CHAR_WORKATTACKPOWER,
     (CHAR_getWorkInt( char_index, CHAR_WORKFIXSTR) * 1.2 / 4 ) );
          CHAR_setWorkInt( char_index, CHAR_WORKDEFENCEPOWER, (CHAR_getWorkInt(
     char_index, CHAR_WORKFIXTOUGH) * 1) );
  */
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM3, n);
  return TRUE;
}
#endif
...
int PETSKILL_Sars(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SARS);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  ...
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, status);
  CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM3, turn);
...
int PETSKILL_Sonic(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SONIC);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Regret(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_REGRET);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Gyrate(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_GYRATE);
...
int PETSKILL_Acupuncture(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_ACUPUNCTURE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Retrace(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_RETRACE);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
...
int PETSKILL_Hector(int char_index, int toNo, int array, char *data) {
  ...
  int strdef;
  int d_dep;
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_HECTOR);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  if (pszOption == "\0") {
  ...
  if ((pszP = strstr(pszOption, "敏%")) != NULL) {
    sscanf(pszP + 3, "%f", &fPer);
    CHAR_setWorkInt(char_index, CHAR_SKILLDEXPOWER, fPer);
    fPer = fPer / 100;
    d_dep = CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX);
    d_dep = (int)(d_dep * fPer);
    CHAR_setWorkInt(char_index, CHAR_WORKQUICK,
                    CHAR_getWorkInt(char_index, CHAR_WORKFIXDEX) + d_dep);
  }
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
}
...
int PETSKILL_Firekill(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_FIREKILL);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
...
int PETSKILL_DamageToHp2(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_DAMAGETOHP2);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录技能
  /*	pszOption = PETSKILL_getChar( array, PETSKILL_OPTION );
...
int PETSKILL_BecomeFox(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_BECOMEFOX);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录技能
...
int PETSKILL_BecomePig(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_BECOMEPIG);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array); // 记录属性
  return TRUE;
...
#ifdef _PETSKILL_LER
int PETSKILL_BatFly(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_BAT_FLY);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
...
int PETSKILL_DivideAttack(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_DIVIDE_ATTACK);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
...
#ifdef _PETSKILL_BATTLE_MODEL
int PETSKILL_BattleModel(int char_index, int toindex, int array, char *data) {
  float fPer;
  int i, iType, iObjectNum, iValue;
  int iAddPowerType[3] = {CHAR_WORKATTACKPOWER, CHAR_WORKDEFENCEPOWER,
                          CHAR_WORKQUICK};
    ...
    for (i = 0; i < 3; i++) {
      if (getStringFromIndexWithDelim(szData, " ", i + 1, szData2,
        ...
        if (strstr(szData2, szWord[i]) != NULL) {
          iValue = CHAR_getWorkInt(char_index, CHAR_WORKATTACKPOWER);
          // 有带 "%" 的设定 ex: "攻%"
          ...
          CHAR_setWorkInt(char_index, iAddPowerType[i], iValue);
        }
      } else
        break;
    }
  ...
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_BATTLE_MODEL);
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM2, iType); // 记录攻击类型
  CHAR_SETWORKINT_HIGH(char_index, CHAR_WORKBATTLECOM2,
                       iObjectNum); // 记录攻击物件数量
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM3, array);
  return TRUE;
}
#endif
...
int PETSKILL_ShowMercy(int char_index, int toNo, int array, char *data) {
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, BATTLE_COM_S_SHOWMERCY);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM2, toNo);
  CHAR_setWorkInt(char_index, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
...
int PETSKILL_Combined(int char_index, int toNo, int array, char *data) {
  ...
  CHAR_SETWORKINT_LOW(char_index, CHAR_WORKBATTLECOM3, kill[rand() % count]);
...
int PETSKILL_PASSIVE_PET_MATCH1(int char_index, int toNo, int array,
                                char *data) {
  float fPer = 0.01;
  int strdef;
  char *pszOption = PETSKILL_getChar(array, PETSKILL_OPTION);
  char *pszP = pszOption;
  int datainfo[5] = {0, 0, 0, 0, 0};
  if ((pszP = strstr(pszOption, "攻%")) != NULL) {
    sscanf(pszP + 3, "%f", &fPer);
    datainfo[0] = (int)fPer;
    fPer = (fPer / 100);
  ...
  if ((pszP = strstr(pszOption, "命%")) != NULL) {
    ...
    datainfo[3] = (int)fPer;
    CHAR_setWorkInt(
        char_index, CHAR_WORKPASSIVE_ACURATE,
        (CHAR_getWorkInt(char_index, CHAR_WORKPASSIVE_ACURATE) + fPer));
  }
  if ((pszP = strstr(pszOption, "回%")) != NULL) {
    sscanf(pszP + 3, "%f", &fPer);
    datainfo[4] = (int)fPer;
    CHAR_setWorkInt(
        char_index, CHAR_WORKPASSIVE_DUCK,
        (CHAR_getWorkInt(char_index, CHAR_WORKPASSIVE_DUCK) + fPer));
  }
  char buf[256];
  sprintf(buf, "%s被动提升 %d%攻击 %d%防敏 %d%敏捷 %d%命中 %d%回避!!",
          CHAR_getChar(char_index, CHAR_NAME), datainfo[0], datainfo[1],
          datainfo[2], datainfo[3], datainfo[4]);
  CHAR_talkToCli(CHAR_getWorkInt(char_index, CHAR_WORKPLAYERINDEX), -1, buf,
                 CHAR_COLORYELLOW);
  return TRUE;
}
