...
int BATTLE_ai_all(int battleindex, int side, int turn) {
  ...
  for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
    int char_index = BattleArray[battleindex].Side[side].Entry[i].char_index;
    int mode;
    ...
    if (BattleArray[battleindex].Side[side].flg & BSIDE_FLG_SURPRISE) {
      ...
      if (functbl[mode] != NULL) {
        rc = functbl[mode](turn, char_index, pEntry, &result);
      ...
      if (rc) {
        ...
        if (result.command == BATTLE_COM_WARP) {
          for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
            int tindex = (pEntry + i)->char_index;
...
#endif
        CHAR_setWorkInt(char_index, CHAR_WORKBATTLECOM1, result.command);
...
static int BATTLE_ai_normal(int turn, int char_index, BATTLE_ENTRY *pEntry,
...
#ifdef _BATTLENPC_WARP_PLAYER
  if (NPC_Util_GetStrFromStrWithDelim(
          CHAR_getWorkChar(char_index, CHAR_WORKBATTLE_ACT_CONDITION), "tn",
    ...
    if (sTurn == CHAR_getWorkInt(char_index, CHAR_WORKTURN)) {
  ...
  if (mode == B_AI_ATTACKMODE ||
    ...
    if (at[2] == B_AI_NORMAL_SELECT_RANDOM) {
      ...
      for (i = 0; i < cnt; i++) {
        if (i == 0)
          ...
          int tophp = CHAR_getInt((pEntry + top)->char_index, CHAR_HP);
          int comphp = CHAR_getInt((pEntry + target[i])->char_index, CHAR_HP);
      ...
      for (i = 0; i < cnt; i++) {
        if (i == 0)
          ...
          int topstr = CHAR_getInt((pEntry + top)->char_index, CHAR_STR);
          int compstr = CHAR_getInt((pEntry + target[i])->char_index, CHAR_STR);
          if (compstr > topstr)
            top = target[i];
        }
      }
      ...
      for (i = 0; i < cnt; i++) {
        if (i == 0)
          ...
          int topdex = CHAR_getInt((pEntry + top)->char_index, CHAR_DEX);
          int compdex = CHAR_getInt((pEntry + target[i])->char_index, CHAR_DEX);
      ...
      for (i = 0; i < cnt; i++) {
        if (i == 0)
          ...
          switch (GetSubdueAttribute(char_index)) {
          case AI_ATT_EARTHAT: {
            int topatt = CHAR_getWorkInt((pEntry + top)->char_index,
                                         CHAR_WORKFIXEARTHAT);
            int compatt = CHAR_getWorkInt((pEntry + target[i])->char_index,
                                          CHAR_WORKFIXEARTHAT);
            if (compatt > topatt)
              top = target[i];
          } break;
          ...
          case AI_ATT_FIREAT: {
            int topatt =
                CHAR_getWorkInt((pEntry + top)->char_index, CHAR_WORKFIXFIREAT);
            int compatt = CHAR_getWorkInt((pEntry + target[i])->char_index,
                                          CHAR_WORKFIXFIREAT);
            if (compatt > topatt)
              top = target[i];
          ...
          case AI_ATT_WINDAT: {
            int topatt =
                CHAR_getWorkInt((pEntry + top)->char_index, CHAR_WORKFIXWINDAT);
            int compatt = CHAR_getWorkInt((pEntry + target[i])->char_index,
                                          CHAR_WORKFIXWINDAT);
            if (compatt > topatt)
              top = target[i];
    ...
    if (B_AI_WAZAMODE0 <= mode && mode <= B_AI_WAZAMODE6) {
      if (PETSKILL_Use(char_index, mode - B_AI_WAZAMODE0, result->target,
                       NULL) == TRUE) {
        result->command = CHAR_getWorkInt(char_index, CHAR_WORKBATTLECOM1);
        return TRUE;
      } else {
        return FALSE;
      }
