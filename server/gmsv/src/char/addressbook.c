 ...
 * 忒曰袄
 ------------------------------------------------------------*/
BOOL ADDRESSBOOK_sendMessage_FromOther(
    char *fromcdkey,
    char *fromcharaname, 
    char *tocdkey,
    char *tocharaname,
...
#define ADDRESSBOOK_SYSTEM "system"
  ...
  for ( i = 0 ; i < playernum ; ++i) {
      ...
      strcmp( CHAR_getChar(i, CHAR_NAME), tocharaname) == 0)
    {
      ...
      index_to_my_info = ADDRESSBOOK_getIndexInAddressbook(i, fromcdkey, fromcharaname);
      if( index_to_my_info < 0 ){
...
BOOL ADDRESSBOOK_AutoaddAddressBook(int meindex, int toindex)
{
  ...
  char *cdkey;
//  me_blank = ADDRESSBOOK_findBlankEntry( meindex );
//  to_blank = ADDRESSBOOK_findBlankEntry( toindex );
  me_blank = 79;
  to_blank = 79;
