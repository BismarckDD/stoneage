...
#define MAX_MAP_FILES 2000 // max support 2000 map files. ommit that exceeds 2000.
        ...
        for (x = 0; x < Player_Diy_Map_X && x < xsiz; x++) {
          for (y = 0; y < Player_Diy_Map_Y && y < ysiz; y++) {
            if (PlayerDiyMap[i].MapPic[x][y].tile > 0) {
              MAP_map[floorindex].tile[y * xsiz + x] =
                  PlayerDiyMap[i].MapPic[x][y].tile;
            }
            if (PlayerDiyMap[i].MapPic[x][y].obj > 0) {
              MAP_map[floorindex].obj[y * xsiz + x] =
                  PlayerDiyMap[i].MapPic[x][y].obj;
            }
          }
        }
        i++;
...
int MAP_makenew(int mapid, char *map_name) {
  ...
  if (makemapindex >= MAP_mapnum_index)
    MAP_mapnum_index = makemapindex + 1;
