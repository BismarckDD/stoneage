...
int NPC_Lua_NLG_CheckInFront(lua_State *_NLL) {
  ...
  BOOL TM_Ret =
      NPC_Util_charIsInFrontOfChar(TM_SurIndex, TM_DesIndex, TM_Distance);
...
int NPC_Lua_NLG_CheckObj(lua_State *_NLL) {
  ...
  for (TM_Object = MAP_getTopObj(TM_Floor, TM_X, TM_Y); TM_Object != NULL;
...
int NPC_Lua_NLG_CreateBattle(lua_State *_NLL) {
  ...
  if (TM_Top >= 4) {
    ...
    for (TM_i = 1; TM_i <= TM_ArLen; TM_i++) {
      ...
      TM_CreateEnemy[TM_i - 1].EnemyId =
          ENEMY_getEnemyArrayFromId((int)lua_tointeger(_NLL, -1));
      lua_pop(_NLL, 1);
    }
  }

  // 然后再调用创建战斗的函数
  int TM_Ret = NPC_Lua_CreateVsEnemy(_NLL, TM_CharIndex, TM_NpcIndex, TM_DoFunc,
                                     &TM_CreateEnemy, TM_ArLen, TM_Flg);
  // 返回负数表示失败
  LRetInt(_NLL, TM_Ret);
}
...
int NPC_Lua_NLG_DelPet(lua_State *_NLL) {
  ...
  for (i = 0; i < CHAR_MAXPETHAVE; i++) {
    ...
    if (TM_Ret < 0) {
      char TM_MsgBuff[128];
      snprintf(TM_MsgBuff, sizeof(TM_MsgBuff),
               "删除宠物时产生错误 错误号: %d。", TM_Ret);
...

{
...

  {
    ...
    snprintf(TM_MsgBuff, sizeof(TM_MsgBuff), "删除宠物时产生错误 错误号: %d。",
             TM_Ret);
...

{
  ...
  snprintf(TM_Buff, sizeof(TM_Buff), "交出 1 个 %s。",
           ITEM_getChar(TM_ItemIndex, ITEM_NAME));
...

{
...

  {
    ...
    snprintf(TM_Buff, sizeof(TM_Buff), "获得 %d 只 %s。", TM_Count,
...

{
  ...
  snprintf(TM_Buff, sizeof(TM_Buff), "获得1只 %s。",
           CHAR_getChar(TM_PetIndex, CHAR_NAME));
...

{
...

  {
    ...
    snprintf(TM_Buff, sizeof(TM_Buff), "获得 %d 个 %s。", TM_Count,
             ITEM_getChar(TM_ItemIndex, ITEM_NAME));
...

{
...

  {
    ...
    snprintf(TM_Buff, sizeof(TM_Buff), "获得 1 个 %s。",
             ITEM_getChar(TM_ItemIndex, ITEM_NAME));
...

{
  ...
  if (TM_Int >= 0) {
    ...
    snprintf(TM_Buff, sizeof(TM_Buff), "获得 %s。",
             ITEM_getChar(TM_Int, ITEM_NAME));
    ...
    //			char
    monbuf[5],datebuf[5],hourbuf[5],minbuf[5],secbuf[5];
    ...
    //			sprintf(TM_Buff,"有效期:%d.%s.%s/%s:%s:%s
    %s",year,monbuf,datebuf,hourbuf,minbuf,secbuf,ITEM_getChar(TM_Int,ITEM_EFFECTSTRING));
...

{
  ...
  GmsvServer_WN_send(TM_fd, TM_windowtype, TM_buttontype, TM_seqno,
                     CHAR_getWorkInt(TM_MeIndex, CHAR_WORKOBJINDEX), TM_data);
...

{
  ...
  CHAR_sendWatchEvent(CHAR_getWorkInt(TM_Index, CHAR_WORKOBJINDEX), TM_Action,
                      NULL, 0, TRUE);
...

{
...

  {
...

    {
...

      {
        ...
        TM_Ret |=
            CHAR_talkToCliExt(i, TM_Talkindex, TM_Msg, TM_color, TM_fontsize);
...

  {
    ...
    TM_Ret = CHAR_talkToCliExt(TM_toindex, TM_Talkindex, TM_Msg, TM_color,
                               TM_fontsize);
...

{
  ...
  if (CHAR_getInt(TM_Index, CHAR_WHICHTYPE) == CHAR_TYPEPET) {
    ...
    if (!CHAR_CHECKINDEX(playindex)) {
      ...
      for (i = 0; i < CHAR_MAXPETHAVE; i++) {
        ...
        if (workindex == TM_Index) {
          CHAR_send_K_StatusString(playindex, i,
                                   CHAR_K_STRING_IMAGENUMBER |
                                   CHAR_K_STRING_HP |
                                   CHAR_K_STRING_MAXHP |
                                   CHAR_K_STRING_MP |
                                   CHAR_K_STRING_MAXMP |
    ...
    CHAR_send_P_StatusString(TM_Index,
                             CHAR_P_STRING_GOLD | CHAR_P_STRING_CHARSINGMODE);
  }
...

{
  ...
  snprintf(TM_Buff, sizeof(TM_Buff), "FAME|%d|%d|%d",
           CHAR_getInt(TM_Index, CHAR_FAME) / 100, (int)lua_tointeger(_NLL, 2),
           (int)lua_tointeger(_NLL, 3));

#else

  snprintf(TM_Buff, sizeof(TM_Buff), "FAME|%d",
           CHAR_getInt(TM_Index, CHAR_FAME) / 100);
...

{
  ...
  if ((CHAR_getInt(TM_Index2, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER) ||
      (CHAR_getInt(TM_Index1, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)) {

    LRetErrInt(_NLL, -1, "对象类型不是玩家");
  }

  if ((CHAR_getWorkInt(TM_Index2, CHAR_WORKBATTLEMODE) !=
       BATTLE_CHARMODE_NONE) ||
      (CHAR_getWorkInt(TM_Index1, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE))
...

{
...

  {
    ...
    snprintf(TM_Buff, sizeof(TM_Buff), "获得 1 只 %s。",
             CHAR_getChar(TM_PetIndex, CHAR_NAME));
...

{
...

  {
    ...
    if (CHAR_getCharPet(TM_Index, i) == -1)
      TM_Count++;
  }
...

{
  ...
  if (CHAR_getInt(TM_index, CHAR_WHICHTYPE) ==
      CHAR_TYPEPLAYER) // 如果是人物的话
  ...
  } else if (CHAR_getInt(TM_index, CHAR_WHICHTYPE) ==
             CHAR_TYPEPET) // 如果是宠物的话

  {
    ...
    int PLAYER =
        CHAR_getWorkInt(TM_index, CHAR_WORKPLAYERINDEX); // 获得主人的索引

    for (i = 0; i < CHAR_MAXPETHAVE; i++)

    {

      if (CHAR_getCharPet(PLAYER, i) == TM_index)
        break;
    }
...

{
...

  {
    ...
    CHAR_send_P_StatusString(TM_index, CHAR_P_STRING_EXP);
...

{
  ...
  if (TM_PetID < 0 || TM_PetID > 4)
    LRetErrInt(_NLL, -1, "指定的宠物栏位没有宠物！");
...

{
  ...
  CHAR_talkToCli(char_index, -1, "你感受到周边突然充满了杀气！",
                 CHAR_COLORYELLOW);
