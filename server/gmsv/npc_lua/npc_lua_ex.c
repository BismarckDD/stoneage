...

{
...

  {
...

    {
      ...
      luaL_openlib(_NLL, _RegArray->FuncList->LibName,
                   _RegArray->FuncList->FuncRegList, 0);
...
int NPC_Lua_Create(const char *_DoFile, const char *_InitFuncName, char *_seek,
                   BOOL _IsFly)

{
...

  {
    ...
    strcpy_s(TM_char.lua_charfunctable[CHAR_INITFUNC].string,
             sizeof(TM_char.lua_charfunctable[CHAR_INITFUNC].string),
             _InitFuncName);

    strcpy_s(TM_char.charfunctable[CHAR_INITFUNC].string,
             sizeof(TM_char.charfunctable[CHAR_INITFUNC].string),
             FUNCNAME_INITCALLBACK);
  }
  ...
  TM_objindex =
      initObjectOne(&TM_obj); // 产生一个对象实例（调用完之后NPC实体产生）
  ...
  CHAR_setWorkInt(TM_char_index, CHAR_WORKACTION,
                  CHAR_getInt(TM_char_index, CHAR_ACTIONSTYLE));

  CHAR_sendWatchEvent(TM_objindex,
                      CHAR_getWorkInt(TM_char_index, CHAR_WORKACTION), NULL, 0,
                      TRUE);
