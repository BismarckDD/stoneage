...
static CharBase CharBaseColor[] = {
    {{"白色"}, CHAR_COLORWHITE},   {{"青色"}, CHAR_COLORCYAN},
    {{"紫色"}, CHAR_COLORPURPLE},  {{"蓝色"}, CHAR_COLORBLUE},
    {{"黄色"}, CHAR_COLORYELLOW},  {{"绿色"}, CHAR_COLORGREEN},
    {{"红色"}, CHAR_COLORRED},     {{"灰白色"}, CHAR_COLORGRAY},
    {{"灰蓝色"}, CHAR_COLORBLUE2}, {{"灰绿色"}, CHAR_COLORGREEN2}};
...
static int setWorkChar(lua_State *L) {
  ...
  const int index = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, CharBaseWorkChar, arraysizeof(CharBaseWorkChar));
...
static int setWorkInt(lua_State *L) {
  const int index = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, CharBaseWorkInt, arraysizeof(CharBaseWorkInt));
  const int data =
      getCharBaseValue(L, 3, CharBaseWorkValue, arraysizeof(CharBaseWorkValue));
...
static int setChar(lua_State *L) {
  ...
  const int index = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, CharBaseChar, arraysizeof(CharBaseChar));
  char *data = luaL_checklstring(L, 3, &l);
...
static int setInt(lua_State *L) {
  const int index = luaL_checkint(L, 1);
  const int element =
      getCharBaseValue(L, 2, CharBaseInt, arraysizeof(CharBaseInt));
  const int data =
      getCharBaseValue(L, 3, CharBaseValue, arraysizeof(CharBaseValue));
...
static int setFunctionPointer(lua_State *L) {
  ...
  const int index = luaL_checkint(L, 1);
  const int functype =
      getCharBaseValue(L, 2, CharBaseEvent, arraysizeof(CharBaseEvent));
...
static int TalkToCli(lua_State *L) {
  ...
  const int color =
      getCharBaseValue(L, 4, CharBaseColor, arraysizeof(CharBaseColor));
...
static int TalkToRound(lua_State *L) {
  ...
  const int color =
      getCharBaseValue(L, 3, CharBaseColor, arraysizeof(CharBaseColor));
...
static int Updata(lua_State *L) {
  ...
  const int indextable =
      getCharBaseValue(L, 2, CharBaseUpdata, arraysizeof(CharBaseUpdata));
  ...
  if (emptyitem_indexinchara >= 0) {
    ...
    if (item_index > -1) {
      ...
      LogItem(CHAR_getChar(char_index, CHAR_NAME),
              ...
              atoi(message),
#endif
              "AddItem(制作道具LUA)", CHAR_getInt(char_index, CHAR_FLOOR),
...
static int AddPet(lua_State *L) {
  ...
  LogPet(CHAR_getChar(index, CHAR_NAME), CHAR_getChar(index, CHAR_CDKEY),
         ...
         "来至LUA的char.AddPet", CHAR_getInt(index, CHAR_FLOOR),
...
static int sendWatchEvent(lua_State *L) {
  ...
  const int action =
      getCharBaseValue(L, 2, CharBaseAction, arraysizeof(CharBaseAction));
  const int flg = luaL_checkint(L, 3);
  ...
  for (i = 0; i < CHAR_MAXPETHAVE; i++) {
    if (petindex == CHAR_getCharPet(char_index, i)) {
      ...
      LogPet(CHAR_getChar(char_index, CHAR_NAME), // 平乓仿
             CHAR_getChar(char_index, CHAR_CDKEY),
             CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
             "EvnetDell(LUA任务删除)", CHAR_getInt(char_index, CHAR_FLOOR),
             CHAR_getInt(char_index, CHAR_X), CHAR_getInt(char_index, CHAR_Y),
             CHAR_getChar(petindex, CHAR_UNIQUECODE) // shan 2001/12/14
      );
...
static int dropPetFollow(lua_State *L) {
  ...
  LogPet(CHAR_getChar(char_index, CHAR_NAME),
         CHAR_getChar(char_index, CHAR_CDKEY),
         CHAR_getChar(petindex, CHAR_NAME), CHAR_getInt(petindex, CHAR_LV),
         "Follow(lua溜宠)", CHAR_getInt(char_index, CHAR_FLOOR),
         CHAR_getInt(char_index, CHAR_X), CHAR_getInt(char_index, CHAR_Y),
...
static int setCharNewTitleMode(lua_State *L) {
  ...
  const int id = luaL_checkint(L, 2);
  if (id < 32) {
    CHAR_setInt(char_index, CHAR_TITLE1,
                CHAR_getInt(char_index, CHAR_TITLE1) | (1 << id));
  } else if (id >= 32 && id < 64) {
    CHAR_setInt(char_index, CHAR_TITLE2,
                CHAR_getInt(char_index, CHAR_TITLE2) | (1 << (id - 32)));
  } else if (id >= 64 && id < 96) {
    CHAR_setInt(char_index, CHAR_TITLE3,
                CHAR_getInt(char_index, CHAR_TITLE3) | (1 << (id - 64)));
  }
  return 1;
}
...
// 2026.09.22 通过luaAB_openlibs给lua注册一个全局变量char
