...
static CharBase sEnemyTempBaseInt[] = {
    {{"编号"}, E_T_TEMPNO}, {{"初始值"}, E_T_INITNUM}, {{"成长率"}, E_T_LVUPPOINT}, {{"体力"}, E_T_BASEVITAL}, {{"腕力"}, E_T_BASESTR}, {{"耐力"}, E_T_BASETGH}, {{"速度"}, E_T_BASEDEX}, {{"智能"}, E_T_MODAI}, {{"给"}, E_T_GET}, {{"地"}, E_T_EARTHAT}, {{"水"}, E_T_WATERAT}, {{"火"}, E_T_FIREAT}, {{"风"}, E_T_WINDAT}, {{"毒抗"}, E_T_POISON}, {{"麻抗"}, E_T_PARALYSIS}, {{"睡抗"}, E_T_SLEEP}, {{"石抗"}, E_T_STONE}, {{"酒抗"}, E_T_DRUNK}, {{"混抗"}, E_T_CONFUSION}, {{"技能1"}, E_T_PETSKILL1}, {{"技能2"}, E_T_PETSKILL2}, {{"技能3"}, E_T_PETSKILL3}, {{"技能4"}, E_T_PETSKILL4}, {{"技能5"}, E_T_PETSKILL5}, {{"技能6"}, E_T_PETSKILL6}, {{"技能7"}, E_T_PETSKILL7}, {{"罕见"}, E_T_RARE}, {{"暴击率"}, E_T_CRITICAL}, {{"反击率"}, E_T_COUNTER}, {{"技能数"}, E_T_SLOT}, {{"形象"}, E_T_IMGNUMBER}, {{"等级限制"}, E_T_LIMITLEVEL}
#ifdef _PET_FUSION
    ,
...
static CharBase sEnemyTempBaseChar[] = {
    {{"名字"}, E_T_NAME}};

static int getInt(lua_State *L)
{
  const int array = luaL_checkint(L, 1);
  const int element = getCharBaseValue(L, 2, sEnemyTempBaseInt, arraysizeof(sEnemyTempBaseInt));
...
{
  const int array = luaL_checkint(L, 1);
  const int element = getCharBaseValue(L, 2, sEnemyTempBaseChar, arraysizeof(sEnemyTempBaseChar));
