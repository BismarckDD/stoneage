...
static int CreateNpc(lua_State *L) {
  ...
  char *npc_name = luaL_checklstring(L, 1, &l);
  ...
  obj_index = initObjectOne(&object);
...
static int DelItemNum(lua_State *L) {
  ...
  for (i = CHAR_STARTITEMARRAY; i < CHAR_MAXITEMHAVE; i++) {
    ...
    LogItem(
        CHAR_getChar(index, CHAR_NAME), CHAR_getChar(index, CHAR_CDKEY),
#ifdef _add_item_log_name // WON ADD 在item的log中增加item名称
        item_index,
#else
        ITEM_getInt(item_index, ITEM_ID),
#endif
        "任务收取", CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X),
        CHAR_getInt(index, CHAR_Y), ITEM_getChar(item_index, ITEM_UNIQUECODE),
