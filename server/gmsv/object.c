...
INLINE int _initObjectOne(char *file, int line, Object *ob) {
  ...
  while (1) {
    if (!first && i >= allocobjnum) {
      print("Can't allocate an object\n");
      return -1;
    }
...
INLINE int OBJECT_getType(int index) { return obj[index].type; }

INLINE int OBJECT_getchartype(int index) { return obj[index].chartype; }
...
INLINE int OBJECT_getFloor(int index) { return obj[index].floor; }
...
INLINE int OBJECT_getX(int index) { return obj[index].x; }
...
INLINE int OBJECT_getY(int index) { return obj[index].y; }
...
INLINE int OBJECT_getTime(int index) { return obj[index].time; }
