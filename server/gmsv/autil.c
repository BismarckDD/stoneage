...
// ret: (none)
#ifdef _NEW_FUNC_DECRYPT
static BOOL hexstrtostring(char *dst, const char *src, size_t len) {
  int b, tmp;
  size_t i;
  for (i = 0; i < len; i++) {
    if (src[i] >= '0' && src[i] <= '9') {
...
static const uint32_t DELTA = UINT32_C(0x9e3779b9);
#define MX                                                                     \
  (z >> 5 ^ y << 2) + (y >> 3 ^ z << 4) ^ (sum ^ y) + (k[p & 3 ^ e] ^ z);
static void DecodeTea(uint32_t *v, size_t n, const uint32_t *k) {
  uint32_t z, y, sum, e;
  z = v[n - 1];
  y = v[0];
  sum = 0;
...
BOOL util_DecodeMessageTea(char *dst, char *sac) {
  /* GetOneLine_fix() deliberately keeps the wire newline in the packet. */
  size_t encoded_len = strcspn(sac, "\r\n");
  size_t len;
  uint32_t key[4];
  ...
  if (len < sizeof(uint32_t) || (len % sizeof(uint32_t)) != 0)
    return FALSE;
  if (!hexstrtostring(dst, sac, encoded_len))
    return FALSE;
  memcpy(key, DENGLUKEY1, sizeof(key));
  DecodeTea((uint32_t *)dst, len / sizeof(uint32_t), key);
  dst[len] = '\0';
  return TRUE;
}
#endif
