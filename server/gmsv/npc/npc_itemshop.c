...
BOOL NPC_AddItemBuy(int meindex, int talker, int itemID, int kosuu, double rate,
                    int iCostFame, int iChangeItemCost);
int NPC_SellItemstrsStr(int item_index, int flg, double rate, char *argtoken,
                        int select, int sell);
...
void NPC_ItemShopTalked(int meindex, int talker, char *szMes, int color) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "sellonly_msg", token,
    ...
    strncpysafe(sellmsg, sizeof(sellmsg),
...
#ifdef _VERSION_25
    sprintf(token, "0|1|%d|%s|%s|%-72s声望:%d|",
...
void NPC_ItemStrStr(int itemID, double rate, char *name, char *token2,
  ...
  if (iCostFame < 0) {
    ...
    sprintf(token2, "%-18s%8dＦ|0|%d|%d|%d|%s|", escape,
            iCostFame < 0 ? 0 : iCostFame / 100, level, gold, graNo, info);
  }
#else
...
#ifdef _VERSION_25
    sprintf(token, "1|1|%d|%s|%s|%-72s声望%d|",
            CHAR_WINDOWTYPE_WINDOWITEMSHOP_STARTMSG,
            CHAR_getChar(meindex, CHAR_NAME), buff, buff2,
            CHAR_getInt(talker, CHAR_FAME) / 100);
#else
          ...
          for (j = 0; j < ITEM_CATEGORYNUM + 3; j++) {
            ...
            if (strcmp(TypeTable[j].arg, token) == 0) {
              ...
              if (ITEM_getInt(item_index, ITEM_TYPE) == itemtype) {
                cost =
                    NPC_GetSellItemList(item_index, 0, argstr, token3, i, sell);
                if (cost != -1)
                  return cost;
                ...
                if (8 <= ITEM_getInt(item_index, ITEM_TYPE) &&
                  ...
                  cost = NPC_GetSellItemList(item_index, 0, argstr, token3, i,
                                             sell);
                  if (cost != -1)
                    return cost;
                  strncat(token2, token3, sizeof(token3));
      ...
      if ((NPC_Util_GetStrFromStrWithDelim(argstr, "LimitItemNo", buff,
        ...
        while (getStringFromIndexWithDelim(buff, ",", k, token,
          ...
          if (strstr(token, "-") == NULL && strcmp(token, "") != 0) {
            ...
            if ((start <= ITEM_getInt(item_index, ITEM_ID)) &&
                (ITEM_getInt(item_index, ITEM_ID) < end)) {
              cost =
                  NPC_GetSellItemList(item_index, 0, argstr, token3, i, sell);
              if (cost != -1)
                return cost;
    ...
    while (getStringFromIndexWithDelim(buff, ",", k, buff2, sizeof(buff2)) !=
      ...
      if (strstr(buff2, "-") == NULL && strcmp(buff2, "") != 0) {
        if (ITEM_getInt(item_index, ITEM_ID) == atoi(buff2)) {
          cost =
              NPC_SellItemstrsStr(item_index, 0, rate, argtoken, select, sell);
          return cost;
        }
...
int NPC_SellItemstrsStr(int item_index, int flg, double rate, char *argtoken,
  ...
  sprintf(argtoken,
          ...
          "%s|%d|%d|%d|%s|%d|",
#endif
          name, flg, cost, ITEM_getInt(item_index, ITEM_BASEIMAGENUMBER),
          escapedname, select
#ifdef _ITEM_PILENUMS
          ,
...
BOOL NPC_SellNewItem(int meindex, int talker, char *data) {
  ...
  if (select < CHAR_STARTITEMARRAY || select >= CheckCharMaxItem(talker))
    return FALSE;
  k = select;
...
#endif
            "Sell(卖道具)", CHAR_getInt(talker, CHAR_FLOOR),
