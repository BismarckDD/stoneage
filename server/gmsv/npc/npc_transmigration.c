  ...
  for (i = 0; i < CHAR_MAXPETHAVE; i++) {
    ...
    if (CHAR_getInt(petindex, CHAR_PETID) == 718
      ...
      if (CHAR_getInt(talkerindex, CHAR_LV) < 80) {
        CHAR_talkToCli(talkerindex, -1, "由于你的等级小于80,没能力进行转宠！",
                       CHAR_COLORWHITE);
        return;
...
static void NPC_Transmigration_selectWindow(int meindex, int toindex, int num) {
  ...
  case 2:
    ...
    for (i = CHAR_STARTITEMARRAY; i < itemMax; i++) {
      ...
      if (ITEM_CHECKINDEX(TM_ItemIndex) == FALSE)
        continue;
...
static int questTbl[20] = {1,  2,  4,  5,  8,  12, 14, 15, 16, 17,
                           19, 22, 27, 30, 31, 34, 35, 38, 44, 45};
//*******************************************************
// 弁巨旦玄及醒毛醒尹月
//*******************************************************
...
BOOL NPC_TransmigrationAddPet(int meindex, int talker, int petid) {
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
         ...
         CHAR_getInt(petindex2, CHAR_LV), "TenseiGet",
...
BOOL NPC_TransmigrationDelPetDel(int meindex, int talker, int petsel) {
  ...
  CHAR_talkToCli(talker, -1, msgbuf, CHAR_COLORWHITE);
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿抩 */
         CHAR_getChar(talker, CHAR_CDKEY), CHAR_getChar(petindex, CHAR_NAME),
         CHAR_getInt(petindex, CHAR_LV), "TenseiDel",
         CHAR_getInt(talker, CHAR_FLOOR), CHAR_getInt(talker, CHAR_X),
         CHAR_getInt(talker, CHAR_Y),
...
#endif
                  "WarpManDelItem", CHAR_getInt(toindex, CHAR_FLOOR),
                  CHAR_getInt(toindex, CHAR_X), CHAR_getInt(toindex, CHAR_Y),
...
#endif
#endif
  int array = shiftbit / 32;
  int shift = shiftbit % 32;

  if (array >= event_num) {
    // print("错误！！所设的任务旗标编号已超过范围(0~%d)。",32*event_num-1);
    return;
  }
  int point = CHAR_getInt(char_index, CHAR_ENDEVENT + array);
  point = point | (1 << shift);
