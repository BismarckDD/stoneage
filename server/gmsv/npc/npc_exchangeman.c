  ...
  while (getStringFromIndexWithDelim(argstr, "EventEnd", i, buf, sizeof(buf)) !=
    ...
    if (strstr(buf, "GetPet") != NULL) {
      if (strstr(buf, "PetFullMsg") == NULL) {
        NPC_Util_GetStrFromStrWithDelim(buf, "EventNo", buf4, sizeof(buf4));
        print("\n### ventNo=%d %s:PetFullMsg 脱离 ###", atoi(buf4),
              CHAR_getChar(meindex, CHAR_NAME));
      }
    }
...
BOOL NPC_TypeCheck(int meindex, int talker, char *szMes) {
  ...
  if (NPC_Util_GetStrFromStrWithDelim(argstr, "TALKCHECK", buf, sizeof(buf)) !=
      NULL) {
    if (CHAR_getWorkInt(meindex, CHAR_WORK_TALKCHECK) == 0) {
      CHAR_setWorkInt(meindex, CHAR_WORK_TALKCHECK, getTalkCheck());
    }

    if (CHAR_getWorkInt(meindex, CHAR_WORK_TALKCHECK) !=
        CHAR_getWorkInt(talker, CHAR_WORK_TALKCHECK)) {
      char buf[256];
      CHAR_setWorkInt(meindex, CHAR_WORK_TALKCHECK, getTalkCheck());
      sprintf(buf, "请输入验证数字:%d",
              CHAR_getWorkInt(meindex, CHAR_WORK_TALKCHECK));
          ...
          if (NPC_Util_GetStrFromStrWithDelim(buf, "NomalMsg", token,
            ...
            if (NPC_Util_GetStrFromStrWithDelim(buf, "MISSIONOVER", buf2,
              ...
              if (mission == missiontable[mindex].mission) {
                ...
                SaacClient_ACMissionTable_send(acfd, MISSION_HERO_COMPLETE, 4,
                                               nameinfo, "");
...
void NPC_ExChangeManWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  CHAR_sendCSpecifiedObjindex(getfdFromCharaIndex(talkerindex),
...
void NPC_MsgDisp(int meindex, int talker, int num) {
  ...
  case 2:
    ...
    if (strstr(buf, "ThanksMsg2") != NULL) {
      /*仇仇匹霜耨允月*/
      GmsvServer_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_NEXT,
        ...
        for (i = 0; i < CHAR_STARTITEMARRAY; i++) { // 检查装备中道具
          if (ITEM_CHECKINDEX(CHAR_getItemIndex(talker, i))) {
            if (CHAR_getInt(talker, PROFESSION_CLASS) != 0 &&
                CHAR_getInt(talker, PROFESSION_CLASS) !=
                    ITEM_getInt(CHAR_getItemIndex(talker, i),
                                ITEM_NEEDPROFESSION) &&
                ITEM_getInt(CHAR_getItemIndex(talker, i), ITEM_NEEDPROFESSION) >
                    0) {
              p_class = 1;
  ...
  if (num != 8)
#endif
    GmsvServer_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, buttontype, windowtype,
                       CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX), token);
}
...
BOOL NPC_EventAdd(int meindex, int talker, int mode) {
  ...
  evcnt = NPC_EventFile(meindex, talker, buff, sizeof(buff));
...
#ifdef _GLORY_POINT
  if (NPC_Util_GetStrFromStrWithDelim(buff, "AddGlory", buff2, sizeof(buff2)) !=
      NULL) {
...
#endif
        "EventAddItem(任务需求所得到的道具)", CHAR_getInt(talker, CHAR_FLOOR),
...
BOOL NPC_AcceptDel(int meindex, int talker, int mode) {
  ...
  int evcnt;
  int b_flg = 0;
  evcnt = NPC_EventFile(meindex, talker, buf, sizeof(buf));
  if (evcnt == -1)
    return FALSE;
...
BOOL NPC_EventDelPet(int meindex, int talker, int petsel) {
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
         ...
         CHAR_getInt(petindex, CHAR_LV), "EvnetDell(任务删除)",
...
BOOL NPC_EventAddPet(int meindex, int talker, char *buff2, int mode) {
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
         CHAR_getChar(talker, CHAR_CDKEY), CHAR_getChar(petindex, CHAR_NAME),
         CHAR_getInt(petindex, CHAR_LV), "EventGet(任务得到)",
         CHAR_getInt(talker, CHAR_FLOOR), CHAR_getInt(talker, CHAR_X),
         CHAR_getInt(talker, CHAR_Y),
         CHAR_getChar(petindex, CHAR_UNIQUECODE) // shan 2001/12/14
  );
...
BOOL NPC_EventAddEgg(int meindex, int talker, char *buff2, int mode) {
  ...
  LogPet(CHAR_getChar(talker, CHAR_NAME), /* 平乓仿   */
         CHAR_getChar(talker, CHAR_CDKEY), CHAR_getChar(petindex, CHAR_NAME),
         CHAR_getInt(petindex, CHAR_LV), "EventGetEgg(任务得到)",
         CHAR_getInt(talker, CHAR_FLOOR), CHAR_getInt(talker, CHAR_X),
         CHAR_getInt(talker, CHAR_Y),
         CHAR_getChar(petindex, CHAR_UNIQUECODE) // shan 2001/12/14
  );
 ...
 *椭瘀毛  凶仄化中月及毛  五请允
 ------------- ---------------------*/
int NPC_EventFile(int meindex, int talker, char *arg, int arg_size) {
...
void NPC_EventPetSkill(int meindex, int talker, char *data) {
  ...
  evcnt = NPC_EventFile(meindex, talker, argstr, sizeof(argstr));
