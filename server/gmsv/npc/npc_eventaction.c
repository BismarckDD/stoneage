...
BOOL ActionNpc_WarpPoint(int meindex, int talker, char *npcarg)
{
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "NPC_POINT", data, sizeof(data)) != NULL)
  {
    ...
    i = 1;
    j = 0;
    while (getDelimitedField(data, ";", i, buf1, sizeof(buf1)) != FALSE)
    {
      i++;
  ...
  if (NPC_Util_GetStrFromStrWithDelim(npcarg, "WARPIPPOINT", data, sizeof(data)) != NULL)
  {
    ...
    i = 1;
    j = 0;
    if (getDelimitedField(data, ";", i, buf1, sizeof(buf1)) != FALSE)
    {
      getDelimitedField(buf1, ",", 1, buf2, sizeof(buf2));
      fl = atoi(buf2);
      getDelimitedField(buf1, ",", 2, buf2, sizeof(buf2));
      ...
      getDelimitedField(buf1, ",", 3, buf2, sizeof(buf2));
      y = atoi(buf2);
      getDelimitedField(buf1, ",", 4, buf2, sizeof(buf2));
      ipnum = atoi(buf2);
    }
...
BOOL ActionNpc_CheckMenuFree(int meindex, int toindex, char *npcarg, int nums)
{
  ...
  if (getDelimitedField(npcarg, "}", nums, buf1, sizeof(buf1)) == FALSE)
    return FALSE;
...
BOOL Action_RunDoEventAction(int meindex, int toindex, char *buf1)
{
  ...
  if (NPC_Util_GetStrFromStrWithDelim(buf1, "GetRandItem", buf2, sizeof(buf2)) != NULL)
  {
    int j = 1, rand_cnt = 1;
    char buf3[256];
    while (getDelimitedField(buf2, ",", j, buf3, sizeof(buf3)) != FALSE)
    {
      j++;
      rand_cnt++;
    }
...
BOOL NPC_ActionIntSet(int talker, char *buf)
{
  char key[128];
  char value[64];
  getDelimitedField(buf, ",", 1, key, sizeof(key));
  int i;
  for (i = 0; i < CHAR_DATAINTNUM; i++)
  {
    if (strcmp(key, CHAR_IntDataName[i]) == 0)
    {
      getDelimitedField(buf, ",", 2, value, sizeof(value));
...
BOOL NPC_ActionDP(int talker, char *buf)
{
  ...
  GmsvServer_RD_send(getfdFromCharaIndex(talker), szBuffer);
...
BOOL NPC_ActionPlayerDiyMapBuy(int talker, char *buf)
{
  ...
  int fame = 0;
  char token[256];
  if (getDelimitedField(buf, ",", 1, token, sizeof(token)) == TRUE)
  {
    ff = atoi(token);
  }
  if (getDelimitedField(buf, ",", 2, token, sizeof(token)) == TRUE)
  {
    day = max(0, atoi(token));
  }
  if (getDelimitedField(buf, ",", 3, token, sizeof(token)) == TRUE)
  {
    playernum = max(0, atoi(token));
  }
  if (getDelimitedField(buf, ",", 4, token, sizeof(token)) == TRUE)
  {
    point = max(0, atoi(token));
  }
  if (getDelimitedField(buf, ",", 5, token, sizeof(token)) == TRUE)
  {
...
BOOL NPC_ActionPlayerDiyMapLetter(int talker, char *buf)
{
  ...
  ITEM_setChar(item_index, ITEM_ARGUMENT, CHAR_getChar(talker, CHAR_CDKEY));
  ITEM_setChar(item_index, ITEM_USEFUNC, "PLAYERDIYMAPLETTER");
  sprintf(token, "由 %s 签发的地图 %d 邀请函", PlayerDiyMap[i].cdkey, PlayerDiyMap[i].ff);
  ITEM_setChar(item_index, ITEM_EFFECTSTRING, token);
  int ret = CHAR_addItemSpecificItemIndex(talker, item_index);
  ...
  for (i = 0; i < Player_Diy_Map_NUM; i++)
  {
    if (PlayerDiyMap[i].ff == ff)
    {
      ...
      for (j = CHAR_STARTITEMARRAY; j < CheckCharMaxItem(talker); j++)
      {
        ...
        {
          if (strcmp(ITEM_getChar(item_index, ITEM_USEFUNC), "PLAYERDIYMAPLETTER") == 0)
          {
            if (strcmp(ITEM_getChar(item_index, ITEM_ARGUMENT), PlayerDiyMap[i].cdkey) == 0)
            {
              int k;
              int playernum = 0;
...
BOOL NPC_ActionLotteryChange(int talker, char *buf)
{
  ...
  if (now.tm_year == tm1.tm_year && now.tm_mon == tm1.tm_mon && now.tm_mday == (tm1.tm_mday + 1))
  {
    ...
    for (i = 0; i < 7; i++)
    {
      if (getDelimitedField(lotteryvalue, ",", i + 1, token, sizeof(token)) == TRUE)
      {
        value[i] = atoi(token);
        int k;
        for (j = 0; j < 7; j++)
    ...
    if (num > 0)
    {
      for (i = 0; i < 7; i++)
      {
        if (getDelimitedField(buf, ",", i + 1, token, sizeof(token)) == TRUE)
        {
          award[i] = atoi(token);
        }
        else
...
BOOL NPC_ActionLotteryAddTime(int talker, char *buf)
{
  ...
  {
    GmsvServer_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OKCANCEL, -1, -1, "你未购买彩票");
  }
  else
  {
    char token[256];
    ...
    GmsvServer_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OKCANCEL, -1, -1, token);
  }
  return TRUE;
}
...
BOOL NPC_ActionShowItemName(int talker, char *buf)
{
  ...
  int item_index = CHAR_getItemIndex(talker, CHAR_EQUIPPLACENUM);
  if (!ITEM_CHECKINDEX(item_index))
  {
    CHAR_talkToCli(talker, -1, "请将道具放在第一栏上", CHAR_COLORRED);
    return TRUE;
  }
  if (ITEM_getInt(item_index, ITEM_ID) == id)
  {
    ITEM_setChar(item_index, ITEM_FORUSERNAME, CHAR_getChar(talker, CHAR_NAME));
    ITEM_setChar(item_index, ITEM_FORUSERCDKEY, CHAR_getChar(talker, CHAR_CDKEY));
...
BOOL NPC_ActionDelPet(int talker, char *buf)
{
  ...
  while (getDelimitedField(buf, ",", k, buff1, sizeof(buff1)) != FALSE)
  {
    k++;
    if (strstr(buff1, "-") != NULL)
    {
      getDelimitedField(buff1, "-", 2, buff3, sizeof(buff3));
      getDelimitedField(buff1, "-", 1, buf2, sizeof(buf2));

      if (strstr(buf2, "!") != NULL)
      {
        flg = 0;
        getDelimitedField(buf2, "!=", 2, buff2, sizeof(buff2));
      }
...
BOOL NPC_ActionNewDelPet(int toindex, char *msg)
{
  ...
  // 等级
  if (getDelimitedField(msg, "-", 1, buf, sizeof(buf)))
  {
    char cmp[12];
    if (flg == 0)
      strcpy(cmp, "=");
      ...
      strcpy(cmp, ">");
    if (getDelimitedField(buf, cmp, 2, buf2, sizeof(buf2)))
    {
      petlev = atoi(buf2);
    }
    else
  ...
  // 宠物编号
  if (getDelimitedField(msg, "-", 2, buf, sizeof(buf)))
  {
    if (getDelimitedField(buf, "*", 1, buf2, sizeof(buf2)))
    {
      petnum = atoi(buf2);
    }
    else
  ...
  if (getDelimitedField(msg, "-", 2, buf, sizeof(buf)))
  {
    if (getDelimitedField(buf, "*", 2, buf2, sizeof(buf2)))
    {
      petcount = atoi(buf2);
    }
    else
...
{
  int item_index, pilenum;

  item_index = CHAR_getItemIndex(toindex, ti);
  if (!ITEM_CHECKINDEX(item_index))
    return FALSE;
...
{
  int i, item_index, finditem = 0, pilenum = 0, onenum = 0;
  char token[256];
  ...
  for (i = CHAR_STARTITEMARRAY; i < CheckCharMaxItem(toindex) && (finditem < num || num == -1); i++)
  {
    ...
    ITEM_setInt(item_index, ITEM_USEPILENUMS, pilenum);
    sprintf(token, "交出%d个%s", onenum, ITEM_getChar(item_index, ITEM_NAME));
    CHAR_talkToCli(toindex, -1, token, CHAR_COLORYELLOW);
    if (pilenum <= 0)
    {
      LogItem(
...
BOOL NPC_ActionDelItem(int talker, char *buf)
{
  ...
  {
    neednum = 1;
    if (getDelimitedField(buff3, "*", 1, buf2, sizeof(buf2)) == FALSE)
      continue;
    itemid = atoi(buf2);
    if (getDelimitedField(buff3, "*", 2, buf2, sizeof(buf2)) == TRUE)
    {
      neednum = atoi(buf2);
    ...
    if (strstr(buff3, "*") != NULL)
    {
      ...
      for (i = CHAR_STARTITEMARRAY; i < CheckCharMaxItem(talker); i++)
      {
        ...
        if (ITEM_CHECKINDEX(item_index))
        {
          ...
          if (itemno == id)
          {
            ItemArray[cnt].item_index = item_index;
      ...
      for (i = 0; i < 20; i++)
      {
        item_index = ItemArray[i].item_index;
        if (!ITEM_CHECKINDEX(item_index))
          break;
        ...
        LogItem(
            ...
            CHAR_getInt(talker, CHAR_Y), ITEM_getChar(item_index, ITEM_UNIQUECODE),
            ITEM_getChar(item_index, ITEM_NAME),
            ITEM_getInt(item_index, ITEM_ID));
      }
    }
...
BOOL NPC_ActionVipItem(int talker, char *buf)
{
  ...
  item_index = CHAR_findEmptyItemBox(talker);
  if (item_index < 0)
  {
  ...
  if (getDelimitedField(buf, ",", 2, buff, sizeof(buff)) != FALSE)
    VipPoints = atoi(buff);
  if (VipPoints < 0)
    VipPoints = 0;
...
BOOL NPC_ActionVipPet(int talker, char *buf)
{
  ...
  if (getDelimitedField(buf, ",", 2, buf2, sizeof(buf2)) != FALSE)
    VipPoints = atoi(buf2);
  if (VipPoints < 0)
    VipPoints = 0;
...
{
  char buf1[1024];
  int item_index = CHAR_findEmptyItemBox(talker);
  if (item_index < 0)
  {
    CHAR_talkToCli(talker, -1, "物品栏空间不足！！", CHAR_COLORYELLOW);
    return FALSE;
  ...
  int Points = 0;
  if (getDelimitedField(buf, ",", 2, buf1, sizeof(buf1)) != FALSE)
    Points = atoi(buf1);
  if (Points < 0)
    Points = 0;
  int fd = getfdFromCharaIndex(talker);
...
BOOL NPC_ActionSetEend(int talkerindex, char *buf)
{
  ...
  while (getDelimitedField(buf, ",", talkNo, buf1, sizeof(buf1)) != FALSE)
  {
    talkNo++;
    eveno = atoi(buf);
...
BOOL NPC_ActionPassCheck(int meindex, int talker, char *buf)
{
  ...
  int i = 1, j = 1;
  int loop = 0;
  while (getDelimitedField(buf, ",", i, buff2, sizeof(buff2)) != FALSE)
  {
    i++;
    if (strstr(buff2, "&") != NULL)
    {
      j = 1;
      loop = 0;
      while (getDelimitedField(buff2, "&", j, buff3, sizeof(buff3)) != FALSE)
      {
        j++;
...
BOOL NPC_ActionBigSmallCheck(int meindex, int talker, char *buf)
{
  ...
  if (strstr(buf, "<") != NULL)
  {
    getDelimitedField(buf, "<", 2, buff3, sizeof(buff2));
    kosuu = atoi(buff3);
...
BOOL NPC_ActionWarpManReduce2(int meindex, int talker, char *buf)
{
  ...
  for (i = CHAR_STARTITEMARRAY; i < CheckCharMaxItem(talker); i++)
  {
    ...
    if (ITEM_CHECKINDEX(item_index))
    {
      ...
      if (itemno == id)
      {
        nums += ITEM_getInt(item_index, ITEM_USEPILENUMS); // 数量
      }
    }
  }
...
BOOL NPC_ActionFreeIfCheck(int meindex, int talker, char *buf, char *opt, int kosuu, int flg, int temp)
{
  ...
  if (strcmp(buf, "YEAR") == 0 || strcmp(buf, "MON") == 0 || strcmp(buf, "DAY") == 0 || strcmp(buf, "HOUR") == 0 || strcmp(buf, "MIN") == 0 || strcmp(buf, "SEC") == 0)
  {
  ...
  if (strcmp(buf, "MANOR") == 0)
  {
    ...
    if (kosuu > -1)
    {
      ...
      getDelimitedField(fmpointlist.pointlistarray[kosuu], "|", 5, token, sizeof(token));
...
BOOL NPC_ActionCleanProfession(int toindex, char *buf)
{
  ...
  k = 1;
  while (getDelimitedField(buf, ",", k, buf1, sizeof(buf1)) != FALSE)
  {
    k++;
...
BOOL NPC_ActionChangePlayerBBI(int meindex, int charindex, char *Img)
{
  ...
  {
    int armtype;
    int item_index = CHAR_getItemIndex(charindex, CHAR_ARM);
    if (ITEM_CHECKINDEX(item_index))
    {
...
#endif
  if (CHAR_getInt(charindex, CHAR_RIDEPET) != -1)
  { // 骑宠不换图
  ...
  else if (strstr(Img, ",") != NULL)
  {
    ...
    while (getDelimitedField(Img, ",", imgindex, buf1, sizeof(buf1)) != FALSE)
    {
      imgindex++;
      maxImg++;
    }
    ...
    if (getDelimitedField(Img, ",", imgindex, buf1, sizeof(buf1)) == FALSE)
      return FALSE;
    cBBI = atoi(buf1);
    sprintf(buf1, "你样子改变了。");
  }
...
BOOL NPC_getTimeXYPoint(int meindex, int Mode)
{
  ...
  {
    if (getDelimitedField(buf, "&", points, buf1, sizeof(buf1)) != FALSE)
    {
...
void GetRand_WarpPoint(char *buf, int *fl, int *x, int *y)
{
  ...
  while (getDelimitedField(buf, ";", point, buff1, sizeof(buff1)) != FALSE)
  {
    point++;
    getDelimitedField(buff1, ",", 1, buff2, sizeof(buff2));
    RandPoint[i].m_fl = atoi(buff2);
    getDelimitedField(buff1, ",", 2, buff2, sizeof(buff2));
    RandPoint[i].m_x = atoi(buff2);
    getDelimitedField(buff1, ",", 3, buff2, sizeof(buff2));
...
int NPC_EmenyChanceCheck(int meindex, int char_index, char *arg)
{
  ...
  {
    char data[256];
    if (getDelimitedField(buf1, ",", 1, data, sizeof(data)) == FALSE)
      return char_index;
    fl = atoi(data);
    if (getDelimitedField(buf1, ",", 2, data, sizeof(data)) == FALSE)
      return char_index;
    x = atoi(data);
    if (getDelimitedField(buf1, ",", 3, data, sizeof(data)) == FALSE)
      return char_index;
    y = atoi(data);
  }
  return NPC_DischargePartyForSex(meindex, char_index, fl, x, y, sex);
}
...
int NPC_DischargePartyForSex(int meindex, int char_index, int fl, int x, int y, int fsex)
{
  ...
  CHAR_talkToCli(char_index, -1, "团队重组！", CHAR_COLORYELLOW);

  for (i = 0; i < getPartyNum(char_index); i++)
  {
    int index = CHAR_getWorkInt(char_index, CHAR_WORKPARTYINDEX1 + i);
...
int PROFESSION_RESTORE_POINT(int char_index)
{
  ...
  if (!CHAR_CHECKINDEX(char_index))
    return -1;
  if ((fd = getfdFromCharaIndex(char_index)) == -1)
    return -1;

  vital = CHAR_getInt(char_index, CHAR_VITAL); // 体
  str = CHAR_getInt(char_index, CHAR_STR);     // 攻
  tough = CHAR_getInt(char_index, CHAR_TOUGH); // 防
  dex = CHAR_getInt(char_index, CHAR_DEX);     // 敏
  MySp = CHAR_getInt(char_index, CHAR_SKILLUPPOINT);
  ...
  */
  ...
  if (sum2 < 20)
    vital = vital + (20 - sum2);
  CHAR_setInt(char_index, CHAR_VITAL, vital * 100); // 体
  CHAR_setInt(char_index, CHAR_STR, str * 100);     // 攻
  CHAR_setInt(char_index, CHAR_TOUGH, tough * 100); // 防
  CHAR_setInt(char_index, CHAR_DEX, dex * 100);     // 敏

  sum /= 100;
  MySp = (sum + CHAR_getInt(char_index, CHAR_SKILLUPPOINT)) - 20;
  CHAR_setInt(char_index, CHAR_SKILLUPPOINT, MySp);

  GmsvServer_SKUP_send(fd, MySp);
  ...
  { // 有无信物
    for (i = CHAR_STARTITEMARRAY; i < CheckCharMaxItem(talker); i++)
    {
      ...
      if (ITEM_CHECKINDEX(item_index))
      {
        ...
        if (itemid == ANGELITEM)
        {
          if (!strcmp(ITEM_getChar(item_index, ITEM_ANGELINFO), missiontable[mindex].angelinfo) && !strcmp(ITEM_getChar(item_index, ITEM_HEROINFO), missiontable[mindex].heroinfo))
            break;
        }
      }
    }
