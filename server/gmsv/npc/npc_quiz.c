...
BOOL NPC_QuizInit(int meindex) {
  ...
  if (CHAR_getWorkInt(meindex, CHAR_WORK_QUIZNUM) > (tbl[0] - 1)) {
    print("%s?符合条件,圆圈quiz的问题数(%d?)比出题数(%d题)不足",
...
static void NPC_Quiz_selectWindow(int meindex, int talker, int num) {
  ...
  case 2:
    /*--丢奶件--*/
    {
      ...
      /*--仇及???卞涩烂今木化中月椭瘀及杽锁?寞毛?五请允--*/
      tbl = NPC_GetQuestion(meindex);

      /*--杽锁醒及民尼永弁 tbl[0]卞反椭瘀毛坢凶仄凶杽锁醒互反中匀化中月 --*/
      if (CHAR_getWorkInt(meindex, CHAR_WORK_QUIZNUM) > (tbl[0] - 1)) {
        /*--杽锁互箫曰卅中--*/
        /*--丢乒伉恄啖钒旵--*/
        NPC_QuizPlayerRelease(point);
  ...
  /*-仇仇匹霜耨允月--*/
  GmsvServer_WN_send(fd, messagetype, buttontype, windowno,
...
void NPC_QuizWindowTalked(int meindex, int talkerindex, int seqno, int select,
  ...
  case CHAR_WINDOWTYPE_QUIZ_MAIN: {
    ...
    if (CHAR_getWorkInt(meindex, CHAR_WORK_PLAYER1) == -1) {
      //		print("\n谜题log->玩家帐号:%s,玩家名称:%s\n ",
      //CHAR_getChar( talkerindex, CHAR_CDKEY ), CHAR_getChar( talkerindex,
      //CHAR_NAME) );
      break;
    }
    ...
    //CHAR_getWorkInt( meindex, CHAR_WORK_PLAYER1), p_no, CHAR_getChar(
    //talkerindex, CHAR_CDKEY ) );
    point = CHAR_getWorkInt(meindex, CHAR_WORK_PLAYER1 + p_no);
    pl_ptr = NPC_QuizPlayerGet(point);
    if (pl_ptr == NULL)
      return;
