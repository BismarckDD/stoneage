...
struct {
  ...
  int battle;
  int gotowin;
} buttonproc[13]; /* ok,cancel, yes,no,prev,next の時の処理  (按键对应的处理) */
...
static void NPC_Bankman_selectWindow(int meindex, int toindex, int num) {
  ...
  if (fd != -1) {
    GmsvServer_WN_send(fd, w.windowtype, w.buttontype, w.windowno + 100,
...
static BOOL NPC_Bankman_readData(int meindex, int windowno, BOOL chkflg) {
  ...
  while (readflg == TRUE) {
    ...
    while (1) {
      ...
      /* ウィンドウNo が決まっていない時の行は無視する  (窗口No未确定时的行忽略) */
      if (winno == -1) {
      ...
      /* ウィンドウNo が一致した時は条件を読む。  (窗口No一致时读取条件)
       * それ以外は無視する  (其他情况忽略) */
          ...
          if (strcasecmp(firstToken, "gotowin") == 0) {
            ...
            if (buttonproc[b_mode].gotowin == -1) {
              if (buttonproc[b_mode].checkhaveitem == -1 &&
                ...
                /* どっちかかたっぽだけでも設定されていれば  (只要任意一侧已设定即可) */
                if (!((buttonproc[b_mode].checkhaveitem != -1 &&
...
/* buttontype=で指定した文字列を数値に変換する。  (将 buttontype= 指定的字符串转换为数值) */
static int NPC_Bankman_restoreButtontype(char *data) {
  int ret = 0;
  int rc;
  int i;
