  ...
  case NPC_AUTOPK_JOINPK:
    if (autopktime > 2) {
      ...
      for (i = 0; i < GMMAXNUM; i++) {
        if (strcmp(CHAR_getChar(talkerindex, CHAR_CDKEY), gGmInfo[i].cdkey) ==
            0) {
...
void AutoPk_PKSystemInfo() {
  ...
  if (autopktime > 0) {
    ...
    if (awardpetid != -1 && awarditemid == -1)
      sprintf(
          buf2,
          "距离截止参加PK比赛时间还剩%d分钟，本次比赛奖品为:编号:%d,宠物:%s",
          autopktime - 2, awardpetid, awardpet);
    else if (awardpetid == -1 && awarditemid != -1)
      sprintf(
          buf2,
          "距离截止参加PK比赛时间还剩%d分钟，本次比赛奖品为:编号:%d,物品:%s",
          autopktime - 2, awarditemid, awarditem);
    else if (awardpetid != -1 && awarditemid != -1)
      sprintf(buf2,
              "距离截止参加PK比赛时间还剩%d分钟，本次比赛奖品为:编号:%d,宠物:%"
              "s和编号:%d,物品:%s",
              autopktime - 2, awardpetid, awardpet, awarditemid, awarditem);
    else
    ...
    if (num > 1) {
      ...
      AutoPk_PKSystemTalk("由于参加比赛人数过少，所以取消比赛！",
                          "由于参加比赛人数过少，所以取消比赛！");
      baward = FALSE;
    }
  }
