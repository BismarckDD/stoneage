...
void NPC_DuelrankingWindowTalked(int meindex, int talkerindex, int seqno,
  ...
  case CHAR_WINDOWTYPE_DUELRANKING_START:
    if (atoi(data) == 3) {
      SaacClient_DBGetEntryByCount_send(
    ...
    if (atoi(data) == 5) {
      ...
      SaacClient_DBGetEntryRank_send(
...
void NPC_Duelranking_PrintRanking(char *data, int count_start, int fdid,
  ...
  for (i = 1;; i++) {
    ...
    }
    {
      ...
      snprintf(msgbuf, sizeof(msgbuf), "%s %5d%s      %10s     %3s     %s\n",
               hosi, atoi(rank) + 1, engnum[dsprank], score, lv, charaname);
    }
    strncatsafe(message, msgbuf, sizeof(message));
    cnt++;
  }
  if (cnt == 0) {
    strcpy(message, "\n      没有其他的资料了。");
  ...
  GmsvServer_WN_send(fd, WINDOW_MESSAGETYPE_WIDEMESSAGE, button,
