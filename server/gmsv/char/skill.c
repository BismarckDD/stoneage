...
static void SKILL_setitemlimit(int char_index, Skill *sk);
static void SKILL_setmerchant(int char_index, Skill *sk);
static void SKILL_setlevel(int char_index, Skill *sk);
#endif
...
static void SKILL_setmerchant(int char_index, Skill *sk) {
  ...
  CHAR_setInt(char_index, CHAR_MERCHANTLEVEL, merchantlevel);
}
...
static void SKILL_setlevel(int char_index, Skill *sk) {
  ...
  CHAR_setInt(char_index, skvalset[index].charadataindex, level);
}
#endif
...
char *SKILL_makeSkillStatusString(Skill *skill, int char_index, int skill_num) {
  ...
  // 耗费MP
  if ((cost_mp = PROFESSION_MAGIC_COST_MP(char_index, skill_num)) == -1)
...
int SKILL_getLevelFromSkillID(int char_index, SKILL_ID id) {
  ...
  for (i = 0; i < CHAR_SKILLMAXHAVE; i++) {
    CHAR_HaveSkill *sk;
    sk = CHAR_getCharHaveSkill(char_index, i);
...
BOOL SKILL_getUpableSkillID(int char_index, char *buf, int buflen) {
  ...
  for (i = 0; i < CHAR_SKILLMAXHAVE; i++) {
    CHAR_HaveSkill *chsk;
    chsk = CHAR_getCharHaveSkill(char_index, i);
...
void SKILL_skillEffect(int char_index) {
  ...
  for (i = 0; i < CHAR_SKILLMAXHAVE; i++) {
    ...
    cskill = CHAR_getCharHaveSkill(char_index, i);
...
int PROFESSION_MAGIC_COST_MP(int char_index, int skill_num) {
  ...
  // 人物技能
  skillid = CHAR_getCharSkill(char_index, skill_num);
  ...
  // 技能等级
  hskill = CHAR_getCharHaveSkill(char_index, skill_num);
