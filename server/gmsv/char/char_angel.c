...
char *getMissionNameInfo(int char_index, char *nameinfo) {
  sprintf(nameinfo, "%s:%s", CHAR_getChar(char_index, CHAR_CDKEY),
...
void selectAngel(int char_index, int heroindex, int mission, int gm_cmd) {
  ...
  sprintf(msg, " 使者资格检查: %s %s ", CHAR_getChar(char_index, CHAR_CDKEY),
          CHAR_getChar(char_index, CHAR_NAME));
  print(msg);
  LogAngel(msg);
  ...
  if (gm_cmd == FALSE) {
    ...
    if (CHAR_getInt(char_index, CHAR_LV) < 30 ||
        !NPC_EventCheckFlg(char_index, 4)) {
      print(" ANGEL资格不符 ");
      return;
    }
...

  {
    sprintf(
        ...
        "hi=%d ",
        gm_cmd ? ("(GM指令)") : (" "), CHAR_getChar(char_index, CHAR_CDKEY),
        CHAR_getChar(char_index, CHAR_NAME), CHAR_getInt(char_index, CHAR_LV),
        CHAR_getChar(heroindex, CHAR_CDKEY), CHAR_getChar(heroindex, CHAR_NAME),
        CHAR_getInt(heroindex, CHAR_LV), mission, char_index, heroindex);
    print(msg);
    LogAngel(msg);
  }
}
...
int AngelCreate(int angelindex) {
  ...
  if (angeltokenindex != -1) {
    ...
    LogItem(CHAR_getChar(angelindex, CHAR_NAME),
            ...
            "AddItem(制作道具 使者信物)", CHAR_getInt(angelindex, CHAR_FLOOR),
  ...
  if (herotokenindex != -1) {
    ...
    LogItem(CHAR_getChar(angelindex, CHAR_NAME),
            ...
            "AddItem(制作道具 勇者信物)", CHAR_getInt(angelindex, CHAR_FLOOR),
            CHAR_getInt(angelindex, CHAR_X), CHAR_getInt(angelindex, CHAR_Y),
...
void Use_AngelToken(int char_index, int toindex, int haveitem_index) {
  ...
  if (mindex == -1 ||
      (strcmp(nameinfo, ITEM_getChar(item_index, ITEM_ANGELINFO)) &&
       strcmp(nameinfo, ITEM_getChar(item_index, ITEM_HEROINFO)))) {
    // 路人甲使用时
    CHAR_talkToCli(char_index, -1, "这并不是属於你的信物，不可随便使用喔。",
                   CHAR_COLORRED);
    return;
  }

  if (strcmp(ITEM_getChar(item_index, ITEM_ANGELINFO),
             missiontable[mindex].angelinfo) ||
      strcmp(ITEM_getChar(item_index, ITEM_HEROINFO),
             missiontable[mindex].heroinfo)) {

    CHAR_talkToCli(char_index, -1, "这是无用的信物，还是丢掉吧。",
                   CHAR_COLORRED);
    return;
  }
  ...
  if (!strcmp(nameinfo, missiontable[mindex].angelinfo)) { // 使者使用时
    ...
    if (checkUnlawWarpFloor(CHAR_getInt(char_index, CHAR_FLOOR))) {
      CHAR_talkToCli(char_index, -1, "你所在的地方无法传送。",
                     CHAR_COLORYELLOW);
      return;
    }
    ...
    for (i = 0; i < maxchar; i++) {
      ...
      if (checkUnlawWarpFloor(fl)) {
        CHAR_talkToCli(char_index, -1, "对象所在的地方无法传送。",
                       CHAR_COLORYELLOW);
        return;
      }
...
void Use_HeroToken(int char_index, int toindex, int haveitem_index) {
  ...
  if (!strcmp(nameinfo, missiontable[mindex].heroinfo)) { // 勇者使用时
    ...
    if (CheckDropatLogout(char_index) == TRUE) {
      CHAR_talkToCli(char_index, -1, "携带登出後消失的道具时无法使用。",
                     CHAR_COLORYELLOW);
      return;
    }
    ...
    for (i = 0; i < maxchar; i++) {
      ...
      if (checkUnlawWarpFloor(fl)) {
        CHAR_talkToCli(char_index, -1, "对象所在的地方无法传送过去。",
                       CHAR_COLORYELLOW);
        return;
      }
