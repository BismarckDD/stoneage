...
void CHAR_recoveryStatus(int char_index)
{
  ...
  for (i = 0; i < arraysizeof(pShow); i++)
  {
    ...
    {
      int old = CHAR_setInt(char_index, pShow[i].paramindex,
         ...
         j < centery + CHAR_CLAIRVOYANCEWIDTH / 2; j++)
    {
           ...
           object = NEXT_OBJECT(object))
      {
        ...
        if (OBJECT_getType(objindex) == OBJTYPE_NOUSE)
          continue;
        if (OBJECT_getType(objindex) == OBJTYPE_CHARA && OBJECT_getIndex(objindex) == char_index)
          continue;
...
int CHAR_die(int char_index)
{
              ...
              CHAR_getInt(char_index, CHAR_DEADCOUNT) + 1);
  {
    ...
    if (CHAR_setInt(char_index, CHAR_POISON, 0) > 0)
      old = TRUE;
    if (old)
