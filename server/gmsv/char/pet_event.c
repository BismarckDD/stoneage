...
//		CHAR_CharSaveLostPet( pindex, 0);
    ...
    LogPet("系统", "Watchfunc", CHAR_getChar(pindex, CHAR_NAME),
           CHAR_getInt(pindex, CHAR_LV), "timeout_lost(系统清除-地上自由宠)",
...
//	CHAR_CharSaveLostPet( petindex, 0);
  ...
  LogPet("系统", "Watchfunc", CHAR_getChar(petindex, CHAR_NAME),
         CHAR_getInt(petindex, CHAR_LV), "timeout_lost(系统清除-地上自由宠)",
...
#ifdef _PET_TALKBBI
BOOL PET_CheckPlayerBBI(int meindex, int to_index, int BBI, int flg) {
  if (!CHAR_CHECKINDEX(to_index))
    return FALSE;
  if (BBI < 0)
    return FALSE;
