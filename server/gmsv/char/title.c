...
BOOL TITLE_addtitle(int char_index, int titleindex) {
  ...
  for (i = 0; i < CHAR_TITLEMAXHAVE; i++) {
    if (CHAR_getCharHaveTitle(char_index, i) == titleindex)
      /*  允匹卞  匀化中月井日窒手仄卅中  */
      return FALSE;
    if (firstfindempty == -1 && CHAR_getCharHaveTitle(char_index, i) == -1) {
      firstfindempty = i;
    }
  }
...
BOOL TITLE_initTitleConfig(char *filename) {
  ...
  if (TITLE_ConfigTable == NULL) {
    printEx("Can't allocate Memory %d\n",
  ...
  while (1) {
...

    {
      ...
      for (i = 1;; i++) {
        ...
        if (strncmp("TITLE", token, 5) == 0) {
          ...
          if (j == arraysizeof(TITLE_param)) {
            printEx("头衔配置语法错误：%s 第%d条，未知字段：%s\n", filename,
                   linenum, token);
            TITLE_initTitleData(titlecfg_readlen);
            errflg = TRUE;
            break;
      ...
      if (TITLE_ConfigTable[titlecfg_readlen].title == -1) {
        printEx("头衔配置语法错误：%s 第%d条缺少 TITLE 字段\n", filename,
               linenum);
        TITLE_initTitleData(titlecfg_readlen);
      } else {
        titlecfg_readlen++;
 ...
 *  FALSE:窒手  井匀凶［
 *------------------------------------------------------------*/
static BOOL TITLE_TitleCheck_Main(int char_index, BOOL mode, int *addcnt,
                                  int *delcnt) {
  int i, j, k, ret;
    ...
    for (j = 0; j < arraysizeof(TITLE_ConfigTable[i].param) &&
      ...
      if (TITLE_param[index].checkfunc != NULL) {
        rc = TITLE_param[index].checkfunc(
            char_index, TITLE_param[index].element,
 ...
 *  FALSE:窒手  井匀凶［
 *------------------------------------------------------------*/
BOOL TITLE_TitleCheck_Nomsg(int char_index, BOOL mode, int *addcnt,
                            int *delcnt) {
...
static int TITLE_IntCheck(int char_index, int elem, int *data, int flg) {
  ...
  for (i = 0; i < TITLE_PARAMSIZE && *(data + i) != -1; i++) {
    switch (flg) {
    case 0: /* "<=" */
      if (CHAR_getInt(char_index, elem) <= *(data + i))
        rc = TRUE;
      break;
    case 1: /* ">=" */
      if (CHAR_getInt(char_index, elem) >= *(data + i))
...
static int TITLE_WorkIntCheck(int char_index, int elem, int *data, int flg) {
  ...
  for (i = 0; i < TITLE_PARAMSIZE && *(data + i) != -1; i++) {
    switch (flg) {
    case 0: /* "<=" */
      if (CHAR_getWorkInt(char_index, elem) <= *(data + i))
        rc = TRUE;
      break;
    case 1: /* ">=" */
      if (CHAR_getWorkInt(char_index, elem) >= *(data + i))
 ...
 * -------------------------------------*/
static int TITLE_ItemCheckMain(int char_index, int itemhaveindex, int *data,
                               int flg) {
  int i, j;
  int rc = FALSE;
  ...
  for (j = 0; j < TITLE_PARAMSIZE && *(data + j) != -1; j++) {
    for (i = 0; i < itemhaveindex; i++) {
      ...
      if (ITEM_CHECKINDEX(item_index)) {
        switch (flg) {
        case 0: /* "<=" */
          if (ITEM_getInt(item_index, ITEM_ID) <= *(data + j))
            rc = TRUE;
          break;
        case 1: /* ">=" */
          if (ITEM_getInt(item_index, ITEM_ID) >= *(data + j))
