...
int CHAR_getNewImagenumberFromEquip(int index, int base_image_number,
...
BOOL CHAR_initAppearPosition(char *filename) {
  ...
  print("CharAppearNumber %d...", CHAR_appearnum);
  return TRUE;
}
    ...
    if (CHAR_getInt(char_index, CHAR_WHICHTYPE) == CHAR_TYPEPET
      ...
      if (level >= CHAR_getInt(char_index, CHAR_LIMITLEVEL) &&
        ...
        if (CHAR_getInt(char_index, CHAR_PETID) == 718
          ...
          nextexp = CHAR_GetLevelExp(char_index,
                                     CHAR_getInt(char_index, CHAR_LIMITLEVEL));
...
#define RAND(x, y)                                                             \
  ...
  if (CHAR_getInt(petindex, CHAR_PETID) == 718
    ...
    if ((level % 20) == 0) {
      ...
      LogPetPointChange(
          CHAR_getChar(toindex, CHAR_NAME), CHAR_getChar(toindex, CHAR_CDKEY),
          CHAR_getChar(petindex, CHAR_NAME), petindex, 3,
          CHAR_getInt(petindex, CHAR_LV), "level%20",
          CHAR_getInt(toindex, CHAR_FLOOR), CHAR_getInt(toindex, CHAR_X),
          CHAR_getInt(toindex, CHAR_Y));
    }
  }
...
int CHAR_PetTakeDrop(int petindex, int floor, int ox, int oy)
{
  int x = rand() % 10;
  int y = rand() % 10;
  int objindex = PET_dropPetAbsolute(petindex, floor, x, y, FALSE);
  if (objindex == -1) {
    return -1;
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (j >= CHAR_MAXPETHAVE) {
      ...
      LogPet(CHAR_getChar(toindex, CHAR_NAME),
             CHAR_getChar(toindex, CHAR_CDKEY),
             CHAR_getChar(petindex[i], CHAR_NAME),
             CHAR_getInt(petindex[i], CHAR_LV), "TenseiDel",
             CHAR_getInt(toindex, CHAR_FLOOR), CHAR_getInt(toindex, CHAR_X),
             CHAR_getInt(toindex, CHAR_Y),
...
int PETFUSION_Evolution(int char_index, int petindex) {
  ...
  LogPetFeed(
      ...
      CHAR_getInt(char_index, CHAR_Y), CHAR_getChar(petindex, CHAR_UNIQUECODE));
...
void CHAR_CheckWaterStatus(int char_index) {
  ...
  if (CHAR_getWorkInt(char_index, CHAR_WORKMAPFLOORTYPE) == 1 &&
    ...
    if (myhp <= 0) {
      myhp = 1;
      if (CHAR_getInt(char_index, CHAR_FLOOR) == 151 ||
          CHAR_getInt(char_index, CHAR_FLOOR) == 160 ||
          CHAR_getInt(char_index, CHAR_FLOOR) == 161)
        CHAR_warpToSpecificPoint(char_index, 702, 213, 27);
      else
        CHAR_warpToSpecificPoint(char_index, 200, 102, 1021);
    }
    CHAR_setInt(char_index, CHAR_HP, myhp);
    CHAR_send_P_StatusString(char_index, CHAR_P_STRING_HP);
  }
...
void CHAR_ReMetamo(int char_index) {
  ...
  if (oldMetamo >= 100700 && oldMetamo < 100819) {
    int newMetamo;
    int hbMetamo[] = {
        100000, 100010, 100025, 100035, 100055, 100045, 100060, 100070,
        100095, 100090, 100100, 100110, 100135, 100125, 100145, 100150,
