...
static CHAR_ChatMagicTable CHAR_cmtbl[] = {
    ...
    {"gmkick", CHAR_CHAT_DEBUG_gmkick, TRUE, 0, 1,
     "帐号 LSLOCK/KICK/DEUNLOCK/UNLOCKALL/LOCK/TYPE/UNLOCK"},
#endif
...
#endif
#ifdef _DEF_NEWSEND
    {"newsend", CHAR_CHAT_DEBUG_newsend, TRUE, 0, 2,
     "floor x y 帐号 无/要说的话"},
#endif
...
//	{ "fukuwa",		CHAR_CHAT_Fukuwa,      TRUE,   0,  3, "" },
    ...
    {"petmaketrans", CHAR_CHAT_DEBUG_petmake2, TRUE, 0, 2,
     "转生数 宠物ID 等级 数量"},
#endif
    /////////////////////////////////////////////////////////////////////////
};
...
static BOOL CHAR_useChatMagic(int char_index, char *data, BOOL isDebug) {
  ...
  if (func) {
    LogGM(CHAR_getUseName(char_index), CHAR_getChar(char_index, CHAR_CDKEY),
          data, CHAR_getInt(char_index, CHAR_FLOOR),
...
void OneByOneTkChannel(int fd, char *tmp1, char *tmp2, int color) {
    ...
    if ((int)NowTime.tv_sec - lastTalkTime > 5) {
      ...
      if (CHAR_getInt(myindex, CHAR_TRANSMIGRATION) >= getTheWorldTrans() &&
        ...
        if (CHAR_getInt(myindex, CHAR_FAME) >= getTheWorldSend()) {
          sprintf(token, "[世界] %s 说：%s", CHAR_getChar(myindex, CHAR_NAME),
                  tmp2);
          ...
          CHAR_setInt(myindex, CHAR_FAME,
                      CHAR_getInt(myindex, CHAR_FAME) - getTheWorldSend());
          CHAR_setWorkInt(myindex, CHAR_WORKWORLDSENDTIME, (int)NowTime.tv_sec);
        } else {
          sprintf(token, "您的声望不足%d，无法使用世界频道功能！",
                  getTheWorldSend());
      ...
      CHAR_talkToCli(myindex, -1, "每次使用世界频道需间隔五秒才能使用世界频道!",
                     CHAR_COLORRED);
      return;
    }
  }
  ...
  if (strcmp(tmp1, "星球") == 0) {
    ...
    if (CHAR_getInt(myindex, CHAR_TRANSMIGRATION) >= getAllServTrans() &&
      ...
      if (CHAR_getInt(myindex, CHAR_FAME) >= getAllServSend()) {
        sprintf(token, "[星球%s] %s 说：%s", getGameservername(),
                CHAR_getChar(myindex, CHAR_NAME), tmp2);
        SaacClient_AllServSend_send(token);

        CHAR_setInt(myindex, CHAR_FAME,
                    CHAR_getInt(myindex, CHAR_FAME) - getAllServSend());

      } else {
        sprintf(token, "您的声望不足%d，无法使用星球频道功能！",
                getAllServSend());
        CHAR_talkToCli(myindex, -1, token, CHAR_COLORRED);
      }
      return;
  ...
  if (IndexCount == 1) {
    ...
    CHAR_talkToCli(myindex, -1, buf, color);
    snprintf(buf, sizeof(buf) - 1, "%s告诉你：%s",
             CHAR_getChar(myindex, CHAR_NAME), tmp2);
    CHAR_talkToCli(IndexList[0], -1, buf, color);
#else
    snprintf(buf, sizeof(buf) - 1, "P|M|你告诉%s：%s", tmp1, tmp2);
    GmsvServer_TK_send(fd, CHAR_getWorkInt(myindex, CHAR_WORKOBJINDEX), buf,
                       color);
    snprintf(buf, sizeof(buf) - 1, "P|M|%s告诉你：%s",
             CHAR_getChar(myindex, CHAR_NAME), tmp2);
    GmsvServer_TK_send(getfdFromCharaIndex(IndexList[0]),
    ...
    if ((addr = strstr(tmp2, "/T")) != NULL) {
      ...
      if (strcmp(tmp1, CHAR_getChar(IndexList[target], CHAR_NAME)) == 0 &&
        ...
        snprintf(buf, sizeof(buf) - 1, "P|M|%s告诉你：%s",
                 CHAR_getChar(myindex, CHAR_NAME), addr);
        GmsvServer_TK_send(
            getfdFromCharaIndex(IndexList[target]),
            CHAR_getWorkInt(IndexList[target], CHAR_WORKOBJINDEX), buf, color);
#endif
        TalkCount++;
      }
    } else {
...
void CHAR_Talk(int fd, int index, char *message, int color, int area) {
    ...
    if ((int)NowTime.tv_sec - lastTalkTime > 5) {
      ...
      if (CHAR_getInt(index, CHAR_TRANSMIGRATION) >= getTheWorldTrans() &&
        ...
        if (CHAR_getInt(index, CHAR_FAME) >= getTheWorldSend()) {
          sprintf(token, "[世界] %s 说：%s", CHAR_getChar(index, CHAR_NAME),
          ...
          CHAR_setInt(index, CHAR_FAME,
                      CHAR_getInt(index, CHAR_FAME) - getTheWorldSend());
      ...
      CHAR_talkToCli(index, -1, "每次使用世界频道需间隔五秒才能使用世界频道!",
                     CHAR_COLORRED);
      return;
    }
  }
        ...
        if (CHAR_getCharUse(i) != FALSE) {
          if (strcmp(CHAR_getChar(i, CHAR_NAME), name) == 0) {
            ...
            for (i = CHAR_STARTITEMARRAY; i < CheckCharMaxItem(index); i++) {
              ...
              if (strcmp(ITEM_getChar(item_index, ITEM_USEFUNC),
                         "ITEM_TALKIP") == 0) {
                char buf[256];
                ...
                if (itemmaxuse < 1) {
                  ...
                  sprintf(buf, "%s，可使用次数剩余%d次。",
                          ITEM_getChar(item_index, ITEM_NAME), itemmaxuse);
                ...
                sprintf(buf, "onlinetalkip/%02d-%02d-%02d.txt",
                        1900 + now.tm_year, 1 + now.tm_mon, now.tm_mday);

                FILE *f1 = fopen(buf, "a+");
                sprintf(token,
                        "[%02d-%02d-%02d %02d:%02d:%02d]玩家 %s 人物名 %s "
                        "在线举报对方 %s 人物名 %s  \n",
                        1900 + now.tm_year, 1 + now.tm_mon, now.tm_mday,
            ...
            if (i == CheckCharMaxItem(index)) {
              CHAR_talkToCli(index, -1, "必需身上持有举报道具才可以举报玩家!",
                             CHAR_COLORYELLOW);
            }
          }
        }
  ...
  if (messageeraseescape[0] == '|' && messageeraseescape[1] == 'G' &&
    ...
    int playernum = CHAR_getPlayerMaxNum();
    char token[256];
    sprintf(token, "收到来自玩家 %s 人物名 %s 的消息：%s",
  ...
  if (messageeraseescape[0] == '/' && messageeraseescape[1] == 's' &&
    ...
    if (CHAR_getInt(index, CHAR_FAME) >= getSaveFame()) {
      ...
      if (NowTime.tv_sec - OldTimes > 3) {
        if (fd != -1) {
          ...
          if (CHAR_charSaveFromConnect(index, FALSE)) {
            ...
            CHAR_setInt(index, CHAR_FAME,
                        CHAR_getInt(index, CHAR_FAME) - getSaveFame());
          }
      ...
      CHAR_talkToCli(index, -1, "你已经没有足够的声望进行保存了！",
                     CHAR_COLORRED);
    }
    return;
  }
    ...
    for (i = 0; i < getFdnum(); i++) {
      ...
      if (TalkTo > -1) {
        ...
        if (CHAR_getFlg(TalkTo, CHAR_ISOCC) == TRUE) {
          ...
          snprintf(buf, sizeof(buf) - 1, "P|O|[职]%s",
                   makeEscapeString(messageeraseescape, escapebuf,
                                    sizeof(escapebuf)));
          GmsvServer_TK_send(getfdFromCharaIndex(TalkTo),
        ...
        if (CHAR_getInt(index, CHAR_GMTIME) > 0) {
          ...
          if (!strcmp(CHAR_getChar(index, CHAR_GMFUNCTION), magicname)) {
            ...
            print("\n账号:%s "
                  "名字:%s\n来自ip:%d.%d.%d.%d\n使用物品命令%s]次数剩下%d\n",
                  CHAR_getChar(index, CHAR_CDKEY),
                  CHAR_getChar(index, CHAR_NAME), a, b, c, d,
                  messageeraseescape, CHAR_getInt(index, CHAR_GMTIME) - 1);
            CHAR_setInt(index, CHAR_GMTIME,
                        CHAR_getInt(index, CHAR_GMTIME) - 1);
            if (CHAR_getInt(index, CHAR_GMTIME) > 0) {
              sprintf(token, "你还能使用%s权限%d次!",
                      CHAR_getChar(index, CHAR_GMFUNCTION),
                      CHAR_getInt(index, CHAR_GMTIME));
              CHAR_talkToCli(index, -1, token, CHAR_COLORRED);
            } else {
              sprintf(token, "你已经没有使用%s权限了!",
                      CHAR_getChar(index, CHAR_GMFUNCTION));
              CHAR_talkToCli(index, -1, token, CHAR_COLORRED);
            }
  ...
  if (getPMove() > -1) {
    if (messageeraseescape[0] == '/' && messageeraseescape[1] == 'g' &&
      ...
      if (point >= getPMove() || getPMove() == 0) {
        ...
        CHAR_warpToSpecificPoint(index, CHAR_getInt(index, CHAR_FLOOR), atoi(x),
                                 atoi(y));
        CHAR_setInt(index, CHAR_AMPOINT, point - getPMove());
#ifdef _AMPOINT_LOG
        if (getPMove() > 0) {
          LogAmPoint(CHAR_getChar(index, CHAR_NAME),
                     CHAR_getChar(index, CHAR_CDKEY), -getPMove(),
                     CHAR_getInt(index, CHAR_AMPOINT), "(玩家移动)",
                     CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X),
                     CHAR_getInt(index, CHAR_Y));
        }
#endif
      } else
        CHAR_talkToCli(index, -1, "你已经没有足够的积分点数顺移了！",
                       CHAR_COLORRED);
      return;
    }
  }
    ...
    if (messageeraseescape[0] == '/' && messageeraseescape[1] == 'g' &&
      ...
      if (point >= getPAnnounce() && getPAnnounce() >= 0) {
        ...
        for (i = 0; i < playernum; i++) {
          if (CHAR_getCharUse(i) != FALSE) {
            char token[256];
            sprintf(token, "<小喇叭> %s 说：%s", CHAR_getChar(index, CHAR_NAME),
                    buff);
        ...
        if (getPAnnounce() > 0) {
          LogAmPoint(CHAR_getChar(index, CHAR_NAME),
                     CHAR_getChar(index, CHAR_CDKEY), -getPAnnounce(),
                     CHAR_getInt(index, CHAR_AMPOINT), "(玩家喇叭)",
                     CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X),
                     CHAR_getInt(index, CHAR_Y));
        }
#endif
        ...
        CHAR_talkToCli(index, -1, "你已经没有足够的积分点数小喇叭了！",
                       CHAR_COLORRED);
      }
      return;
    }
...
#ifdef _SPECIAL_SUIT
  if (messageeraseescape[0] == '/' && messageeraseescape[1] == 'f' &&
      messageeraseescape[2] == 'l' && messageeraseescape[3] == 'y') {
    char token[255];
    int suit;
  ...
  if (lastvolume == -1) {
      ...
      for (object = MAP_getTopObj(fl, xx[j], yy[j]); object;
        ...
        if (OBJECT_getType(objindex) == OBJTYPE_CHARA && toindex != index) {
          if (CHAR_Talk_check(index, toindex, 0)) {
            ...
            if (talkedfunc) {
              CHAR_CHAT_DEBUG_SAVESHH(index, index, messageeraseescape, color,
                                      channel);
    ...
    if (channel > -1 && fmindexi >= 0) {
      ...
      {
        char buf[512];
        sprintf(buf, "[族长广播]%s: %s", CHAR_getChar(index, CHAR_NAME),
                messageeraseescape);
        SaacClient_ACFMAnnounce_send(acfd, CHAR_getChar(index, CHAR_FMNAME),
            ...
            if (OBJECT_getType(objindex) == OBJTYPE_CHARA && toindex != index) {
              ...
              if (CHAR_Talk_check(index, toindex, micflg)) {
                ...
                talkedfunc =
                    (TALKF)CHAR_getFunctionPointer(toindex, CHAR_TALKEDFUNC);
                if (talkedfunc) {
                  CHAR_CHAT_DEBUG_SAVESHH(index, index, messageeraseescape,
                                          color, channel);
                  talkedfunc(toindex, index, messageeraseescape, color, -1);
                }
              }
...
#ifdef _FONT_SIZE
BOOL CHAR_talkToCliExt(int talkedchar_index, int talkchar_index, char *message,
  ...
  snprintf(lastbuf, sizeof(lastbuf), "P|%s|%d",
           makeEscapeString(CHAR_appendNameAndTitle(talkchar_index, message,
...
BOOL CHAR_talkToCli(int talkedchar_index, int talkchar_index, char *message,
                    CHAR_COLOR color) {
  return CHAR_talkToCliExt(talkedchar_index, talkchar_index, message, color, 0);
}
#else
BOOL CHAR_talkToCli(int talkedchar_index, int talkchar_index, char *message,
                    CHAR_COLOR color) {
...
void TalkAction(int char_index, char *message) {
  ...
  tagTlakAction TlakAction[13] = {
      {"坐下", 11}, {"挥手", 12}, {"点头", 18}, {"高兴", 13}, {"生气", 14},
      {"悲伤", 15}, {"站立", 19}, {"走动", 17}, {"晕倒", 10}, {"攻击", 2},
      {"防御", 16}, {"受伤", 4},  {"投掷", 3}};
  for (i = 0; i < 13; i++)
