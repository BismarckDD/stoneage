...
int CheckFMMember(int char_index) {
  if (CHAR_getInt(char_index, CHAR_FMINDEX) > 0)
    return -1;
  if (strcmp(CHAR_getChar(char_index, CHAR_FMNAME), "") != 0)
    return -1;
#ifdef _FMVER21
...
void FAMILY_Add(int fd, int meindex, char *message) {
  ...
  SaacClient__ACAddFM_send(acfd, fmname, charname, charid, charlv, petname,
  ...
  //   Connectfd:%d fd:%d\n", 	acfd, meindex, fmname, charname,
  //   CHAR_getInt(meindex, CHAR_FAME), CONNECT_getFdid(fd), fd);
#else
  SaacClient__ACAddFM_send(acfd, fmname, charname, charid, charlv, petname,
                           petattr, fmrule, fmsprite, chargrano,
                           CONNECT_getFdid(fd));
...
void ACJoinFM(int fd, int result, int recv) {
  ...
  if (result == 1) {
    GmsvServer_WN_send(
        fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
        makeEscapeString(
            "\n谢谢你的加入申请！请先等族长对你的审核通过之後，才算正式加入。",
            buf, sizeof(buf)));
...
#endif
    {
      SaacClient__ACDelFM_send(acfd, fmname, fmindex, index, charname, charid,
                               CONNECT_getFdid(fd));
    } else {
      SaacClient__ACLeaveFM_send(acfd, fmname, fmindex, charname, charid, index,
                                 CONNECT_getFdid(fd));
    }
  }
}
...
void FAMILY_Detail(int fd, int meindex, char *message) {
  ...
  if (strcmp(token, "P") == 0) {
    ...
    if (CHAR_getInt(meindex, CHAR_BOUNDTIME) == 0) {
      CHAR_talkToCli(meindex, -1, "你目前的特殊权限期限未启用！",
                     CHAR_COLORYELLOW);
      ...
      CHAR_talkToCli(meindex, -1, "你目前的特殊权限期限已过期！",
                     CHAR_COLORYELLOW);
    }
#endif
#ifdef _VIP_RIDE
    ...
    if (CHAR_getInt(meindex, CHAR_VIPTIME) == -1) {
      CHAR_talkToCli(meindex, -1, "你目前的会员权限期限永久！",
                     CHAR_COLORYELLOW);
    } else if (CHAR_getInt(meindex, CHAR_VIPTIME) == 0) {
      CHAR_talkToCli(meindex, -1, "你目前的会员权限期限未启用！",
                     CHAR_COLORYELLOW);
      ...
      CHAR_talkToCli(meindex, -1, "你目前的会员权限期限已过期！",
                     CHAR_COLORYELLOW);
    }
#endif

#ifdef _CAMEO_MONEY
    sprintf(sendbuf, "你目前的个人贝壳数目为：%d",
            CHAR_getInt(meindex, CHAR_CAMEO));
    CHAR_talkToCli(meindex, -1, sendbuf, CHAR_COLORYELLOW);
#endif
  ...
  if (strcmp(token, "D") == 0) {
    ...
    SaacClient__ACFMDetail_send(acfd, fmname, fmindex, tempindex,
                                CONNECT_getFdid(fd));
  }
...
void FAMILY_CheckMember(int fd, int meindex, char *message) {
...
#ifdef _FMVER21
  {
    ...
    SaacClient__ACMemberJoinFM_send(
        acfd, CHAR_getChar(meindex, CHAR_FMNAME),
...
#else
  {
    SaacClient__ACMemberJoinFM_send(acfd, CHAR_getChar(meindex, CHAR_FMNAME),
                                    CHAR_getInt(meindex, CHAR_FMINDEX),
                                    charname, charindex,
                                    CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
...
#endif
  {
...
#ifdef _FMVER21
    {
      ...
      LogFamily(CHAR_getChar(meindex, CHAR_FMNAME),
                ...
                CHAR_getChar(meindex, CHAR_CDKEY), "CheckMember", sbuf);
      // shan end
      SaacClient__ACMemberLeaveFM_send(
          acfd, CHAR_getChar(meindex, CHAR_FMNAME),
          CHAR_getInt(meindex, CHAR_FMINDEX), charname, charindex,
          CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
          CHAR_getWorkInt(meindex, CHAR_WORKFMCHARINDEX), CONNECT_getFdid(fd));
  ...
  if (strcmp(token, "G") == 0) {
    ...
    if (((cash - toBank) >= 0) && ((cash - toBank) <= MaxGold) &&
      ...
      if (toBank >= 0) {
        ...
        LogFamilyBankStone(
            CHAR_getChar(meindex, CHAR_NAME), CHAR_getChar(meindex, CHAR_CDKEY),
            toBank, CHAR_getInt(meindex, CHAR_GOLD),
            "myBank(存款)(家族个人银行)", CHAR_getInt(meindex, CHAR_FLOOR),
        ...
        LogFamilyBankStone(
            CHAR_getChar(meindex, CHAR_NAME), CHAR_getChar(meindex, CHAR_CDKEY),
            toBank, CHAR_getInt(meindex, CHAR_GOLD),
            "myBank(提款)(家族个人银行)", CHAR_getInt(meindex, CHAR_FLOOR),
            CHAR_getInt(meindex, CHAR_X), CHAR_getInt(meindex, CHAR_Y),
            CHAR_getInt(meindex, CHAR_BANKGOLD));
      }
      // Syu ADD 新增家族个人银行存取Log (不含家族银行)
      LogStone(-1, CHAR_getChar(meindex, CHAR_NAME), /* 平乓仿   */
               ...
               -toBank,                              /* 嗯喊 */
               CHAR_getInt(meindex, CHAR_GOLD), "myBank(家族个人银行)",
...
void FAMILY_SetPoint(int fd, int meindex, char *message) {
  ...
  for (i = 1; i <= MANORNUM; i++) {
    if (((strcmp(fmpks[i * MAX_SCHEDULE + 1].guest_name,
      ...
      GmsvServer_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK,
                         -1, -1, makeEscapeString(tmpbuf, buf, sizeof(buf)));
      return;
    }
  }
  if (getStringFromIndexWithDelim(message, "|", 2, token, sizeof(token)) ==
  ...
  /*
     ...
     fl, x, y);
  */
  SaacClient__ACSetFMPoint_send(acfd, CHAR_getChar(meindex, CHAR_FMNAME),
                                CHAR_getInt(meindex, CHAR_FMINDEX),
                                CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
...
void ACSetFMPoint(int ret, int r, int clifd) {
  ...
  GmsvServer_WN_send(clifd, WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK,
                     -1, -1, makeEscapeString(message, buf, sizeof(buf)));
}
...
void FAMILY_SetAcceptFlag(int fd, int meindex, char *message) {
  ...
  if ((result == 0) || (result == 1)) {
    SaacClient__ACFixFMData_send(acfd, CHAR_getChar(meindex, CHAR_FMNAME),
                                 CHAR_getInt(meindex, CHAR_FMINDEX),
                                 CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI),
...
void FAMILY_FixRule(int fd, int meindex, char *message) {
  ...
  if (strcmp(token, "B") == 0) {
    ...
    SaacClient__ACFixFMData_send(
        ...
        CHAR_getWorkInt(meindex, CHAR_WORKFMCHARINDEX), CONNECT_getFdid(fd));
    SaacClient__ACShowMemberList_send(
...
int FAMILY_RidePet(int fd, int meindex, char *message) {
  ...
  printf(
      "[RIDE] request: fd=%d char=%d name='%s' message='%s' "
      "rideSlot=%d learnRide=%d lv=%d baseGra=%d battleMode=%d tradeMode=%d\n",
...
void FAMILY_LeaderFunc(int fd, int meindex, char *message) {
  ...
  if (strcmp(token, "CHANGE") == 0) {
    ...
    if (strcmp(token2, "Q") == 0) {
      ...
      GmsvServer_WN_send(CHAR_getWorkInt(toindex, CHAR_WORKFD),
...
void ACFMJob(int fd, int ret, char *data1, char *data2) {
  ...
  if (!CHAR_CHECKINDEX(leaderindex))
    return;
  // if( CHAR_getWorkInt( leaderindex, CHAR_WORKLEADERCHANGE ) != char_index )
  // return;
  CHAR_setWorkInt(leaderindex, CHAR_WORKLEADERCHANGE, 0);

  if (ret == 0) {
    CHAR_talkToCli(char_index, -1, "族长让位失败！", CHAR_COLORYELLOW);
  ...
  // || CHAR_getInt( leaderindex, CHAR_FMLEADERFLAG) != 1
    ...
    sprintf(buf, "leaderindex:%d:%s\n", leaderindex,
            CHAR_getChar(leaderindex, CHAR_NAME));
    LogFamily(CHAR_getChar(char_index, CHAR_FMNAME),
              CHAR_getInt(char_index, CHAR_FMINDEX),
  ...
  sprintf(buf2, "\n辛苦你了！你已经将族长的位子交给%s了。",
          CHAR_getChar(char_index, CHAR_NAME));
  GmsvServer_WN_send(CHAR_getWorkInt(leaderindex, CHAR_WORKFD),
                     WINDOW_MESSAGETYPE_MESSAGE, WINDOW_BUTTONTYPE_OK, -1, -1,
