...
// failed to exchange cards due to some reasons.
#define ADDRESSBOOK_CANTADD "那里没有任何人。"
#define ADDRESSBOOK_CANTADD2 "无法交换名片。"
// succeed to exchange cards with others.
#define ADDRESSBOOK_ADDED "和%s交换名片 。"
// begin to exchange cards with others.
...
// after exchange cards, the card in mine is delete.
#define ADDRESSBOOK_RETURNED1 "从%s寄来信件，但由於没有%s的名片又将信件退回。"
// after exchange cards, the card in others is deleted.
...
BOOL ADDRESSBOOK_sendMessage(int cindex, int aindex, char *text, int color) {
  ...
  for (i = 0; i < playernum; i++) {
    if (CHAR_CHECKINDEX(i) &&
      ...
      if (fd != -1) {
        ...
        LogHelper(LOG_TALK, "CD=%s\tNM=%s\tT=%s", mycd, mycharaname,
                  textbuffer);
      }
  ...
  SaacClient_Message_send(acfd, mycd, mycharaname, ae->cdkey, ae->charname,
                          textbuffer, color);
  CHAR_setInt(cindex, CHAR_SENDMAILCOUNT,
              CHAR_getInt(cindex, CHAR_SENDMAILCOUNT) + 1);
    ...
    if (CHAR_CHECKINDEX(i) &&
      ...
      if (index_to_my_info < 0) {
        ...
        if (fd != -1) {
          ...
          LogHelper(LOG_TALK, "CD=%s\tNM=%s\tT=%s", fromcdkey, fromcharaname,
                    text);
        }
      }
      break;
...
BOOL ADDRESSBOOK_addEntry(int meindex) {
  ...
  if (cnt == 1) {
    ADDRESSBOOK_addAddressBook(meindex, CONNECT_getTradecardchar_index(fd, 0));
    return TRUE;
  } else if (cnt > 1) {
    int strlength;
    char msgbuf[1024];
    ...
    for (i = 0; CONNECT_getTradecardchar_index(fd, i) != -1 &&
                i < CONNECT_WINDOWBUFSIZE;
         i++) {
      char *a = CHAR_getChar(CONNECT_getTradecardchar_index(fd, i), CHAR_NAME);
      char buf[256];
...
static BOOL ADDRESSBOOK_makeEntryFromCharaindex(int char_index,
  ...
  if (cdkey == "\0") {
    print("ADDRESSBOOK_makeEntryFromCharIndex: strange!"
...
void ADDRESSBOOK_notifyLoginLogout(int cindex, int flg) {
  ...
  if (!CHAR_CHECKINDEX(cindex))
    return;
  char *cd = CHAR_getChar(cindex, CHAR_CDKEY);
...
BOOL ADDRESSBOOK_AutoaddAddressBook(int meindex, int toindex) {
  ...
  int to_blank = 79;
  // ???
  if (to_blank < 0 || me_blank < 0) { // "名片匣已满."
    return FALSE;
  }
  char *cdkey = CHAR_getChar(toindex, CHAR_CDKEY);
  int myaddindex = ADDRESSBOOK_getIndexInAddressbook(
      meindex, cdkey, CHAR_getChar(toindex, CHAR_NAME));
  cdkey = CHAR_getChar(meindex, CHAR_CDKEY);
  int toaddindex = ADDRESSBOOK_getIndexInAddressbook(
      toindex, cdkey, CHAR_getChar(meindex, CHAR_NAME));
  if (myaddindex < 0) {
    ADDRESSBOOK_entry me_entry;
    ADDRESSBOOK_entry to_entry;
    if (ADDRESSBOOK_makeEntryFromCharaindex(toindex, &to_entry) == FALSE ||
        ADDRESSBOOK_makeEntryFromCharaindex(meindex, &me_entry) == FALSE) {
      return FALSE;
    }
    CHAR_setAddressbookEntry(meindex, me_blank, &to_entry);
    CHAR_setAddressbookEntry(toindex, to_blank, &me_entry);
    ADDRESSBOOK_sendAddressbookTableOne(meindex, me_blank);
    ADDRESSBOOK_sendAddressbookTableOne(toindex, to_blank);
  }
  return TRUE;
}
#endif
