...
static int readSqlConfig(const char *filename) {
  char buf[255];
  FILE *fp;
  fp = fopen(filename, "r");
  if (fp == NULL) {
...
int sasql_query(char *nm, char *pas) {
  char sqlstr[256];
  sprintf(sqlstr, "select `%s`, `salt` from %s where %s=BINARY'%s'",
          config.sql_PASS, config.sql_Table, config.sql_NAME, nm);
...
BOOL sasql_online(char *ID, char *NM, char *IP, int flag) {
  if (config.openbackground == 1) {
    ...
    if (flag == 0)
      ...
      sprintf(sqlstr,
              "update %s set LoginTime=NOW(), MAC='%s', IP='%s', Online=1 "
              "where %s=BINARY'%s'",
              config.sql_Table, MAC, IP, config.sql_NAME, ID);
    else if (flag == 2)
      sprintf(sqlstr,
              "update %s set LoginTime=NOW(), OnlineName='%s', Online=1, "
              "Path='char/0x%x'where %s=BINARY'%s'",
              config.sql_Table, NM, getHash(ID) & 0xff, config.sql_NAME, ID);
    else if (flag == 3)
...
BOOL sasql_register(char *id, char *ps) {
  ...
  sprintf(sqlstr,
          ...
          config.sql_Table, config.sql_NAME, config.sql_PASS, id, MD5String(ps),
          getHash(id) & 0xff);
#else
  sprintf(sqlstr,
          "INSERT INTO %s (%s,%s,RegTime,Path) VALUES "
...
char *sasql_ItemPetLocked_Passwd(char *id, char *safepasswd) {
  char sqlstr[256];
  sprintf(sqlstr, "update %s set SafePasswd=BINARY'%s' where %s=BINARY'%s'",
...
char *sasql_OnlineCost(char *id, char *costpasswd) {
  ...
  sprintf(sqlstr,
          "select `CostVal`, `check` from `OnlineCost` where CostPasswd = "
          "BINARY'%s'",
          costpasswd);
  if (!sasql_mysql_query(sqlstr)) {
    int num_row = 0;
...
void sasql_OnlineCost_add(int cost, int num) {
  ...
  for (i = 0; i < num; i++) {
    ...
    sprintf(
        sqlstr,
        "INSERT INTO OnlineCost (CostPasswd, CostVal) VALUES (BINARY'%s', %d)",
        CostPasswd, cost);
    if (!sasql_mysql_query(sqlstr)) {
      k++;
    } else {
...
void sasql_OldpsToMd5ps() {
  ...
  while ((mysql_row = mysql_fetch_row(mysql_result))) {
    ...
    if (strlen(password) <= 16) {
      ...
      printf("%16s%16s%32s: ", name, password, md5str);
