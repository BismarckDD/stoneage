  ...
  if (kindflag == 1) {
    ...
    for (i = 0; i < TOPFMLISTNUM; i++) {
      ...
      sprintf(t1, "%d|%d|%s|%s|%d", fmadvindex[i], i + 1,
              family[fmadvindex[i]].fmname, family[fmadvindex[i]].fmleadername,
    ...
    for (i = 0; i < TOPFMLISTNUM; i++) {
      ...
      sprintf(t1, "%d|%d|%s|%s|%d", fmpkindex[i], i + 1,
              family[fmpkindex[i]].fmname, family[fmpkindex[i]].fmleadername,
...
void AddFMMaintainSort(int index) {
  ...
  j = MAX_FAMILY;
  k = 0;
  while ((family[fmsynthesizeindex[i]].fmsynthesize >=
  ...
  k = 0;
  while (
      (family[fmtotalfameindex[i]].fmtotalfame >= family[index].fmtotalfame) &&
...
int readFamily(const char *dir) {
  char dirname[256];
  DIR *p_dir;
  /*搭配使用dirent64和readdir64,解决32位应用读取64位操作系统文件夹的问题*/
...
int writeFamily(const char *dir) {
  ...
  for (i = 0; i < MAX_FAMILY; i++) {
    ...
    if ((family[i].fmindex != -1) && (strcmp(family[i].fmname, "") != 0)) {
      fprintf(
          fp,
          "fmindex=%d|fmname=%s|fmleadername=%s|fmleaderid=%s|"
          ...
          family[i].fmgold, family[i].fmmemberindexmaxnum, family[i].apply_time,
          family[i].setup_time, family[i].predel_time,
      ...
      for (k = 0; k < family[i].fmmemberindexmaxnum; k++) {
        if ((strcmp(family[i].fmmemberindex[k].charname, "") != 0) &&
          ...
          fprintf(fp,
                  "|%s%d=charname=%s charid=%s charlv=%d charflag=%d "
                  "onlineflag=%d predeltime=%d popular=%d eventflag=%d",
#else
          fprintf(
              fp,
              "|%s%d=charname=%s charid=%s charlv=%d charflag=%d onlineflag=%d "
              "predeltime=%d popular=%d eventflag=%d momentum=%d",
#endif
#else
          fprintf(fp,
                  "|%s%d=charname=%s charid=%s charlv=%d charflag=%d "
                  "onlineflag=%d predeltime=%d popular=%d",
#endif
...
int ACFixFMData(int index, char *fmname, int fmindex, int kindflag,
  ...
  if (kindflag == 1)
    ...
    family[index].fmmemberindex[charindex].charlv =
        family[index].fmmemberindex[0].charlv;
...
#endif
  if ((kindflag == 2) || (kindflag == 7) || (kindflag == 8) ||
      (kindflag == 9) || (kindflag == 10)
#ifdef _NEW_MANOR_LAW
      || (kindflag == 12) || (kindflag == 13)
#endif
  ) {
    ...
    family[index].fmpopular = family[index].fmadv + family[index].fmfeed +
                              family[index].fmsynthesize +
                              family[index].fmdealfood + family[index].fmpk;
    {
      int j;
...
//%d\n",family[index].fmmomentum);
    ...
    if (family[index].fmpointindex >= 0) {
      fmpoint[family[index].fmpointindex].hadfmpopular =
    ...
    if (family[index].fmmemberindex[0].onlineflag > 0) {
      ...
      saacproto_ACFMCharLogin_send(
          ...
          family[index].fmtotalfame,
#ifdef _FMVER21
          family[index].fmmemberindex[0].charflag, family[index].fmsetupflag, 1,
          0,
#else
...
int ACFixFMPK(int winindex, char *winfmname, int winfmindex, int loseindex,
...

  {
    family[winindex].fmpopular =
        family[winindex].fmadv + family[winindex].fmfeed +
...
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
    {
      ...
      for (j = 0; j < MAX_MEMBERNUM; j++) {
        if ((family[winindex].fmmemberindex[j].charflag != FMMEMBER_NONE) &&
          ...
          family[winindex].fmtotalfame +=
              family[winindex].fmmemberindex[j].popular;
      }
    }
#endif
    ...
    family[loseindex].fmpopular =
        family[loseindex].fmadv + family[loseindex].fmfeed +
...
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
    {
      ...
      for (j = 0; j < MAX_MEMBERNUM; j++) {
        if ((family[loseindex].fmmemberindex[j].charflag != FMMEMBER_NONE) &&
          ...
          family[loseindex].fmtotalfame +=
              family[loseindex].fmmemberindex[j].popular;
      }
    }
#endif
...
#ifdef _FMVER21
          family[loseindex].fmmemberindex[0].charflag,
          family[loseindex].fmsetupflag, 1, 0,
#else
...
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
    fmpoint[family[winindex].fmpointindex].hadfmpopular =
        family[winindex].fmtotalfame;
#else
    fmpoint[family[winindex].fmpointindex].hadfmpopular =
        family[winindex].fmpopular;
...
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
    fmpoint[family[loseindex].fmpointindex].hadfmpopular =
        family[loseindex].fmtotalfame;
#else
    fmpoint[family[loseindex].fmpointindex].hadfmpopular =
        family[loseindex].fmpopular;
...
int ACDelFM(int index, char *fmname, int fmindex) {
  ...
  if (family[index].memonum > 0) {
    for (i = 0; i < family[index].memonum; i++)
      memset(family[index].memolist[i], 0, sizeof(family[index].memolist[i]));
  }
...
int ACShowFMList(char *data) {
  ...
  for (i = 0; i <= fmindexmaxnum; i++) {
    ...
    if ((strcmp(family[i].fmname, "") != 0) && (family[i].fmindex != -1)) {
#ifdef _PERSONAL_FAME // Arminius: 家族个人声望
      sprintf(msg, "%d %s %s %d %d %d %d %d|", family[i].fmindex,
              family[i].fmname, family[i].fmleadername, family[i].fmtotalfame,
              family[i].fmjoinnum, i, family[i].fmsetupflag,
              family[i].fmacceptflag);
#else
      sprintf(msg, "%d %s %s %d %d %d %d %d|", family[i].fmindex,
              family[i].fmname, family[i].fmleadername, family[i].fmpopular,
              family[i].fmjoinnum, i, family[i].fmsetupflag,
              family[i].fmacceptflag);
#endif
      strcat(data, msg);
...
int ACFMDetail(int index, char *fmname, int fmindex, char *data) {
  ...
  strcpy(data, "");
  sprintf(data,
          "%d|%d|%s|%s|%d|%s|%s|%d|%d|%s|%d"
#ifdef _FAMILYBADGE_
          "|%d"
#endif
          ,
          index, family[index].fmindex, family[index].fmname,
          family[index].fmleadername, family[index].fmleadergrano,
          family[index].petname, family[index].petattr, family[index].fmjoinnum,
          family[index].fmacceptflag, family[index].fmrule,
          family[index].fmsprite
#ifdef _FAMILYBADGE_
          ,
...
#endif
{
  ...
  if (strcmp(family[index].fmmemberindex[charindex].charname, charname) == 0) {
    ...
    if (result != FMMEMBER_MEMBER) {
      if (result == FMMEMBER_ELDER &&
          family[index].fmmemberindex[charindex].eventflag != 1)
        return -1;
      if (ACFMAssignOcp(index, fmname, fmindex, charname, charindex, result) >=
          0)
        return 0;
      else
        return -1;
    ...
    if (family[index].fmmemberindex[charindex].onlineflag > 0) {
...
#ifdef _NEW_MANOR_LAW
      {
        ...
        // 家族气势 = (家族人数平方) / 10 + 1 + 家族成员个人气势加总
        family[index].fmmomentum +=
            ((family[index].fmjoinnum * family[index].fmjoinnum) / 10 + 1) *
            100;
        if (family[index].fmmomentum > 2000000000)
          family[index].fmmomentum = 2000000000;
      }
...
#endif
            i, fmpoint[i].fl, fmpoint[i].x, fmpoint[i].y, fmpoint[i].hadfmindex,
            fmpoint[i].hadfmname, fmpoint[i].hadfmpopular,
            fmpoint[i].hadfmindex - 1, fmpoint[i].village
#ifdef _FIX_FMPOINTTIME
            ,
            fmpoint[i].fmpointtime
#endif
...
// 申请庄园
int ACSetFMPoint(int index, char *fmname, int fmindex, int fmpointindex, int fl,
                 int x, int y) {
  int i = 0;
  if (CheckFM(&index, fmname, fmindex) < 0)
    return -1;
...
int ACFixFMPoint(int winindex, char *winfmname, int winfmindex, int loseindex,
  ...
  int i = 0;
  // printf("\n修复庄园信息 %d|%s|%d|%d|%s|%d|%d",winindex, winfmname,
  // winfmindex, loseindex, 		losefmname,  losefmindex,  village);
#ifdef _SET_FMPOINT
  if (winindex == -1 || winfmindex == -1 || loseindex == -1 ||
      losefmindex == -1) {
  ...
  if (fmpoint[village - 1].hadfmindex == winfmindex) {
    ...
    iTotalGetGold = (family[winindex].fmmomentum / 100 * 10000) -
                    (iTotalMamberMomentum / 100 * 5000);
...
int ACGMFixFMData(int index, char *fmname, char *charid, char *cmd,
  ...
  if (charid[0] == '-' && charid[1] == '1') {
    ...
    for (i = 0; i < MAX_MEMBERNUM; i++) {
      if (strcmp(family[index].fmmemberindex[i].charid, charid) == 0 &&
          family[index].fmmemberindex[i].onlineflag > 0) {
        charindex = i;
        break;
      }
        ...
        case 5:
          ...
          for (i = 0; i < MAX_MEMBERNUM; i++) {
            if ((family[index].fmmemberindex[i].charflag != FMMEMBER_NONE) &&
                (family[index].fmmemberindex[i].charflag != FMMEMBER_APPLY))
              family[index].fmtotalfame +=
                  family[index].fmmemberindex[i].popular;
          }
          break;
        case 6:
...
int ChangeFMLeader(int index, char *fmname, int fmindex) {
  ...
  ACMemberLeaveFM(index, fmname, fmindex,
                  family[index].fmmemberindex[tmpindex].charname, 0, tmpindex,
                  0);
  return 0;
}
...
int readOneFamily(char *filename, int i) {
  ...
  chop(line);
  for (j = 1;
       j <= familymaxint + familymaxchar + MAX_MEMBERNUM + FAMILY_MEMONUM;
       j++) {
